"""Voice and audio system for SynROV AiBot."""
...
def _parse_device_index(device: Any) -> Optional[int]:
    """Convert a device index to int or None (system default)."""
    if device is None:
        return None
...
def _safe_device_label(device: Any) -> str:
    """Return a readable description of the selected device."""
...
class RichVoiceParser:
    """Intent parser backed by the three SynROV robot profiles."""
    ...
    def parse_full(self, text: Any) -> Dict[str, Any]:
        """Return the full dictionary with text, normalized text, robot, kind, intent, and confidence."""
...
class GoogleVoiceAudioEngine:
    """Complete voice and audio engine for SynROV AiBot.

    Features:
    - Audio-level measurement in an isolated subprocess (does not block the UI)
    - Speech recognition through Google Speech Recognition
    - Multiple-language support through a dynamic getter
    - Audio features (RMS, peak, pulse) for the learning pipeline
    - Rich context with backend, diagnostics, and state

    Audio capture and speech recognition run in separate daemon threads.
    start() returns immediately and never blocks Tkinter.
    """
    ...
    def set_language_getter(self, language_getter: Any) -> None:
        """Register a callable that returns the current language (for example, lambda: lang_var.get())."""
        if callable(language_getter):
            self.language_getter = language_getter


    def start(self) -> bool:
        """Start audio capture and speech recognition in daemon threads.

        Never blocks. Returns True if already running or started successfully.
        """
        if self.running:
            return True
    ...
    def inject_text(self, text: str) -> None:
        """Inject text directly as if it had been recognized from the microphone."""
    ...
    def context(self) -> Dict[str, Any]:
        """Return the complete voice and audio system state."""
        with self._audio_lock:
    ...
    def _pyaudio_meter_loop(self) -> None:
        """Open an isolated Python subprocess for audio-level measurement.

        Isolation prevents PyAudio failures (stalled driver, denied permission,
        incompatible format) from affecting the main UI. The subprocess
        automatically tries different devices, rates, and formats.
        """
    ...
    def _speech_loop(self) -> None:
        """Continuous listening and recognition loop using Google Speech Recognition."""
        ...
        for mic_index in candidates:
            ...
            try:
                ...
                with _sr_lib.Microphone(device_index=mic_index) as source:
                    ...
                    try:
                        # Recognition uses a fixed threshold to avoid auto-adjustment that can mask commands
                        pass
                    except Exception:
                        pass
    ...
    def _recognize_async(self, recognizer: Any, audio: Any) -> None:
        """Send recognition to a separate thread to avoid blocking the capture loop."""
