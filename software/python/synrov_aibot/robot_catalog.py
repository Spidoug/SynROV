...
from .ui_i18n import text as ui_text
...
class CommandSpec:
    ...
    def command(self, language: str = "pt") -> str:
        label = ui_text(language, self.command_pt, self.command_en)
        return f"{label} [{self.key_hint}]" if self.key_hint else label

    def description(self, language: str = "pt") -> str:
        return ui_text(language, self.description_pt, self.description_en)
...
class MissionSpec:
    ...
    def command(self, language: str = "pt") -> str:
        return ui_text(language, self.command_pt, self.command_en)
...
COMMANDS: Dict[str, Tuple[CommandSpec, ...]] = {
    ...
    "Drone": (
        ...
        _c("camera_left", "câmera drone esquerda", "drone camera left", ("camera drone esquerda",), "Move o gimbal do Drone para a esquerda.", "Moves the Drone gimbal left.", "X"),
        _c("camera_right", "câmera drone direita", "drone camera right", ("camera drone direita",), "Move o gimbal do Drone para a direita.", "Moves the Drone gimbal right.", "Z"),
        ...
        _c("camera_stream_toggle", "alternar câmera drone", "toggle drone camera", ("alternar camera drone",), "Alterna a transmissão da câmera do Drone.", "Toggles the Drone camera stream."),
    ),
}
...
MISSIONS: Dict[str, Tuple[MissionSpec, ...]] = {
    ...
    "Vehicle": (
        ...
        _m("return_home", "voltar base", "return home", ("retornar home",), "Retorna à posição de origem usando navegação.", "Returns to the origin using navigation."),
    ...
    "Drone": (
        ...
        _m("return_home", "voltar base", "return home", ("retornar home",), "Retorna à origem usando GPS e rumo da telemetria.", "Returns to origin using GPS and telemetry heading."),
