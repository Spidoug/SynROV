...
void captureMotorCalibrationLimit2() {
  ...
  if (motorOneTurnSpanRaw < MOTOR_CAL_RECOMMENDED_SPAN_RAW) {
    ...
    Serial.println(F("WARNING: THIS IS A REAL A0 RESOLUTION LIMIT; SPANRAW WILL NOT BE ARTIFICIALLY ENLARGED"));
  }
...
void printMotorCalibrationStatus() {
  ...
  if (motorCal.limit1Adc != 0 && motorCal.limit2Adc != 0) {
    ...
    Serial.print(F("LIMITMOTORCALIB CANDIDATE_SPANRAW=")); Serial.println(candidateSpan);
...
void autoMotorStart() {
  ...
  if (motorOneTurnSpanRaw < MOTOR_CAL_MIN_SPAN_RAW) {
...
void printMotorConfigPanel() {
  ...
  Serial.println(F("  SPANRAW=<v>     -> ADC width of that one selected turn"));
...
void handleConfigCommand(char* cmd) {
  ...
  *eq = '\0';
  ...
  if      (strcmp(key, "PWMADDR") == 0)   pca9685Addr = (uint8_t)clampValue((int)value, 1, 127);
    ...
    long maxZero = (long)MOTOR_ADC_MAX - (long)MOTOR_CAL_MIN_SPAN_RAW;
    ...
    long requested = (long)roundf(value);
    long maxSpan = (long)MOTOR_ADC_MAX - (long)motorCalibrationZeroADC;
    if (requested < (long)MOTOR_CAL_MIN_SPAN_RAW || requested > maxSpan) {
      Serial.print(F("INVALID SPANRAW - VALID RANGE=")); Serial.print(MOTOR_CAL_MIN_SPAN_RAW); Serial.print(F("..")); Serial.println(maxSpan);
      printConfigInvalidEcho(rawCmd);
      return;
    }
    motorOneTurnSpanRaw = (int32_t)requested;
    motorCalibrationChanged = true;
  }
...
void handleCommand(char* cmd) {
  ...
  // 0x17 text frames use the V1 #CMD envelope. Reject a malformed V1
