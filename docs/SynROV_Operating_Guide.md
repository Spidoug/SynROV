...
The Python AiBot is a Tkinter application/package that connects to the Processing WebSocket bridge. It includes helper modules for state, bridge communication, safety filtering, orchestration, replay, voice parsing, media/vision, learning, and UI. The main Tkinter compatibility implementation is kept in `core.py`, while the smaller modules expose stable public APIs for specific subsystems.
...
| Module | Purpose |
| --- | --- |
| `actuators.py` | Actuator and safety adapter exports |
| `auto_torque.py` | Auto Torque bridge and Tkinter integration |
| `bridge.py` | WebSocket bridge classes |
| `config.py` | Constants, paths, robot keys, and telemetry keys |
...
| `learning.py` | Dataset collection, feature extraction, training, and inference helpers |
| `media.py` | Vision, webcam, image/audio feature helpers |
| `modern_runtime.py` | Dedicated WebSocket AI runtime without the Tkinter UI |
| `orchestrator.py` | Runtime arbitration/orchestration layer |
| `perception.py` | Voice, vision, and telemetry snapshot builders |
| `policy.py` | Policy output normalization |
| `primitives.py` | JSON, numeric, and text primitives |
| `replay.py` | Replay buffer utilities |
| `robot_ai.py` | Robot profiles, intents, missions, and learned commands |
| `runtime.py` | Runtime compatibility exports |
| `safety.py` | Safety clamps, limits, smoothing, and adapters |
...
The large `core.py` file preserves the original Tkinter behavior and inheritance order. The smaller modules import, wrap, or re-export stable pieces of that compatibility core and the newer dedicated runtime.

### 17.3 Data folders

The Python package creates runtime data folders under:
...
These folders are used for multimodal data, captured frames, datasets, and models. They may not exist in a fresh checkout until the AiBot saves data for the first time.
...
Useful follow-up improvements for a public hardware release include:

- Screenshots and hardware photos.
- Circuit diagrams.
- Exact Arduino and Processing library version list.
