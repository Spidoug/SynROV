...
String formatSensorKeyLabel(String key) {
  ...
  if (isDroneSelected) {
    ...
    if (k.equals("sonar_vertical_cm")) return tr("Sonar inferior vertical", "Downward sonar vertical");
    if (k.equals("sonar_ground_ref_cm")) return tr("Referência do chão do sonar", "Sonar ground reference");
    if (k.equals("sonar_height_cm") || k.equals("drone_height_from_sonar_cm")) return tr("Altura sonar/MPU", "Sonar/MPU height");
...
void drawDiagnosticsSlider2D(int x, int y, int w, String label, int channel, int targetValue, int liveValue) {
  ...
  // The slider position represents the value actually reported by the Arduino.
  // While the operator is dragging, temporarily show the requested command so
  // the control remains responsive under the mouse. Cmd and Live stay visible
  // independently in the label above the bar.
  int displayValue = active ? targetValue : liveValue;
