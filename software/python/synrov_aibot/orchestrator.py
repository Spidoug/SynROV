...
HUMAN_SOURCES = {"manual", "operator", "ui", "teacher", "keyboard", "joystick", "leap", "web", "voice", "text", "typed"}
...
class SynROVOrchestrator:
    ...
    def __init__(self, arbiter: Optional[PriorityArbiter] = None) -> None:
        ...
        self.state = RuntimeState()  # aggregate diagnostics
    ...
    def sync_input_context(self, robot: Any, context: Optional[Dict[str, Any]]) -> None:
        """Mirror Processing's current input owner into the priority arbiter.

        Keyboard, joystick and Leap activity are measured in Processing, where
        those devices actually own the command path.  Registering that owner
        here prevents autonomous/model/music actions from fighting a human.
        """
        if not isinstance(context, dict):
            return
        control = context.get("control") if isinstance(context.get("control"), dict) else {}
        if not bool(control.get("human_override", False)):
            return
        source = normalize_identifier(control.get("active_source", "manual")) or "manual"
        if source == "leap_motion":
            source = "leap"
        self.register_external_command(source, robot=robot, command="human_control")
