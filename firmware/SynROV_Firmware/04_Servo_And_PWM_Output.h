// =====================================================================
// SynROV Firmware V1 - Servo and PWM outputs
// ---------------------------------------------------------------------
// Purpose:
//   External PWM mapping, fallback servo control, smooth output updates and
//   output safety logic.
// =====================================================================

// Utility: angle to external PWM ticks.
...
uint16_t angleToFallbackServoMicros(uint8_t ch, float angleDeg) {
  uint16_t ticks = angleToExternalPwmTicks(ch, angleDeg);
...
// Returns the PCA9685 phase offset for a servo channel.
uint16_t pcaServoPhaseOnTicks(uint8_t ch) {
  if (!SERVO_PCA_PHASE_STAGGER_ENABLED || !isServoChannel(ch)) return 0;
  uint8_t idx = ch - FIRST_SERVO_CH;
  return (uint16_t)((SERVO_PCA_PHASE_BASE_TICKS +
                     (uint16_t)idx * SERVO_PCA_PHASE_SPACING_TICKS) & 0x0FFFu);
}

// Writes a servo pulse to the PCA9685 with per-channel phase staggering.
bool externalPwmSetServoPulseTicks(uint8_t ch, uint16_t widthTicks) {
  widthTicks = clampValue(widthTicks, (uint16_t)0, (uint16_t)4095);
  uint16_t on = pcaServoPhaseOnTicks(ch);
  uint16_t off = (uint16_t)((on + widthTicks) & 0x0FFFu);
  return externalPwmSetPWM(ch, on, off);
}
...
void writeChannel(uint8_t ch, float value) {
  ...
  if (isPwmChannel(ch)) {
    ...
    bool writtenExternally = false;
    if (rt.pwmReady) {
      uint16_t pwmValue = map(percent, 0, 100, 0, 4095);
      writtenExternally = externalPwmSetPWM(ch, 0, pwmValue);
    }
    // externalPwmSetPWM() switches rt.pwmReady off on an I2C failure.  Retry
    // the exact same command immediately so the failover loses no output cycle.
    ...
    if (ch == FIRST_PWM_CH && motorType == MOTOR_TYPE_DC_MOTOR) {
  ...
  if (getServoWriteValid(ch) && fabsf(angle - (float)servoLastWritten[ch]) < servoWriteEpsilonDeg) {
    return;
  }

  bool outputWritten = false;
  if (rt.pwmReady) {
    outputWritten = externalPwmSetServoPulseTicks(ch, angleToExternalPwmTicks(ch, angle));
  }

  // If the PCA was absent at boot, or failed during the external write above,
  // emit this same target immediately through the Arduino Servo fallback.
  if (!outputWritten) {
    uint8_t idx = ch - FIRST_SERVO_CH;
    if (idx < 11) {
      if (!isFallbackServoAttached(idx)) {
        ...
        // Servo::attach() can initialize a 16-bit timer. Keep the dedicated
        // motor PWM timer healthy even if library allocation changes later.
...
void applyBootPwmDefaults() {
  // CH12..CH15 boot percentages were already loaded from EEPROM into
  // pwmBootPercent[] by applyConfigToRuntime(). Apply those values now.
...
void applySafeStartupPose() {
  float safeBaseDeg = clampValue(getManipulatorMemberStartupSafeDeg(MANIP_MEMBER_BASE), MOTOR_INPUT_MIN, MOTOR_INPUT_MAX);
  uint8_t baseCh = getManipulatorMemberChannel(MANIP_MEMBER_BASE);
  if (baseCh == 0) {
    servos.target[0] = safeBaseDeg;
    servos.actual[0] = safeBaseDeg;
    motor.targetAngle = safeBaseDeg;
    motor.currentAngle = safeBaseDeg;
