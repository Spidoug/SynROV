...
// Vertical descent is allowed whenever the drone is measurably clear of the
// landed zone. This lower threshold keeps manual descent responsive during the
// takeoff ramp without ever commanding negative throttle from the ground.
boolean droneCanDescendNow() {
  return droneCurrentPhysicalLift() > DRONE_LANDED_LIFT_EPS;
}

// Returns whether the Drone has reached the normal physical flight-height threshold.
// This remains the completion criterion for automatic takeoff and the airborne phase.
boolean droneReachedPlanarUnlockHeight() {
  return droneCurrentPhysicalLift() >= DRONE_PLANAR_UNLOCK_LIFT;
}

// Horizontal/yaw commands become valid as soon as a takeoff/climb sequence has
// started. Waiting for the altitude threshold here caused Web/AiBot commands to
// be discarded even after TAKEOFF or a positive climb command had been accepted.
boolean dronePlanarMotionReady() {
  if (droneAutoLandingActive || droneFlightPhase == DRONE_FLIGHT_LANDING) return false;
  return droneReachedPlanarUnlockHeight()
    || droneAutoTakeoffActive
    || droneFlightPhase == DRONE_FLIGHT_TAKING_OFF
    || droneFlightPhase == DRONE_FLIGHT_AIRBORNE
    || droneThrottleCmd > 0.02f;
}
...
// this gate so takeoff/climb command authority and landing safety stay
...
void setDroneFlightNormalized(float throttle, float yaw, float pitch, float roll, float strafe, float forward) {
  ...
  // A deliberate manual command in the opposite vertical direction takes
  // authority from the automatic sequence. The rule is source-independent:
  // Web, AiBot, ROS, keyboard and physical joystick all reach this same gate.
  if (droneAutoTakeoffActive && boundedThrottle < -0.02f) {
    droneAutoTakeoffActive = false;
    droneFlightPhase = droneReachedPlanarUnlockHeight() ? DRONE_FLIGHT_AIRBORNE : DRONE_FLIGHT_LANDING;
  } else if (droneAutoLandingActive && boundedThrottle > 0.02f) {
    droneAutoLandingActive = false;
    droneFlightPhase = droneReachedPlanarUnlockHeight() ? DRONE_FLIGHT_AIRBORNE : DRONE_FLIGHT_TAKING_OFF;
  }

  boolean flightReady = !droneAutoLandingActive && (dronePlanarMotionReady() || boundedThrottle > 0.02f);
  ...
  } else if (boundedThrottle < 0.0f && !droneCanDescendNow()) {
...
boolean droneRuntimeMotionBlockedByCollision() {
  ...
  if (!dronePlanarMotionReady()) return false;
...
void triggerDroneTakeOff() {
  if (droneReachedPlanarUnlockHeight()) {
    droneAutoTakeoffActive = false;
    droneAutoLandingActive = false;
    droneFlightPhase = DRONE_FLIGHT_AIRBORNE;
    resetDroneInputState();
...
void updateDronePhysics() {
  ...
  // Rebuild after altitude transitions so command readiness and the physical
  // phase stay synchronized with takeoff/climb/landing state.
...
void drawDrone3D(boolean advanceCamera) {
  ...
  pushMatrix();
  translate(0, 0, 1.8);
  fill(isRobotCameraViewEnabled(ROBOT_MODE_DRONE) ? color(255, 220, 0) : color(80, 80, 70));
  sphere(0.4);
