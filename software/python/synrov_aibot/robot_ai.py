                ...
                if isinstance(entries, dict):
                    for phrase, item in entries.items():
                        ...
                        if kind in {"intent", "mission"} and name:
                            clean[r][norm_text(phrase)] = {"kind": kind, "name": name}
        self._data = clean
...
MANIPULATOR_PROFILE = RobotProfile(
    ...
    missions={
        ...
        "scan_workspace":     ("scan 360", "varredura 360", "girar 360", "varrer bancada"),
...
class DedicatedRobotAI:
    ...
    def _intent_manipulator(self, name: str, bridge: Any) -> CommandResult:
        ...
        step = 24.0
        # The physical SynROV base is centred at 180° and the operator-left
        # command moves in the positive direction around that centre.
    ...
    def tick(
        ...
        if done is None:
            # Mission ended abnormally (unknown name).
            self.active_mission = None
    ...
    def _tick_manipulator(
        ...
        if name == "return_home":
            bridge.manip_home()
            self.active_mission = None
            bridge.command_manipulator_pose(
                self.safety.sanitize_pose(pose, source="mission", smooth=True)
            )
        ...
        if name == "hold_pose":
            ...
            pose["base"] = (pose["base"] + 2.4) % 360
            pose["wrist_pitch"] = clamp(112 + 14 * math.sin(t * 0.9), 0, 359)
            done = 18.0
    ...
    def __init__(
        ...
        *,
        ...
        self.ais: Dict[str, DedicatedRobotAI] = {
            "Manipulator": DedicatedRobotAI(MANIPULATOR_PROFILE, self.safety, model_root=model_root, memory=self.memory),
            "Vehicle":     DedicatedRobotAI(VEHICLE_PROFILE,     self.safety, model_root=model_root, memory=self.memory),
            "Drone":       DedicatedRobotAI(DRONE_PROFILE,       self.safety, model_root=model_root, memory=self.memory),
        }
