...
boolean applyManipulatorPoseTargets(int[] requestedAngles, boolean[] activeMask) {
  ...
  // With hardware connected, angles[] remains measured/visual feedback. In
  // simulation there is no physical telemetry, so commit the command pose to the
  // visual state immediately.
  if (simulationMode || myPort == null || !systemReady) {
    for (int i = 0; i < manipCount; i++) {
