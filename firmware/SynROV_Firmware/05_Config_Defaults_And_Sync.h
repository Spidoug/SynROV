// =====================================================================
// SynROV Firmware V1 - Configuration defaults and runtime synchronization
// ---------------------------------------------------------------------
// Purpose:
...
void fillDefaultConfig(PersistentConfig& cfg) {
  ...
  cfg.payload.kp = MOTOR_DC_KP_DEFAULT;
  cfg.payload.ki = MOTOR_DC_KI_DEFAULT;
  cfg.payload.kd = MOTOR_DC_KD_DEFAULT;

  cfg.payload.zeroAdc = MOTOR_BASE_DEFAULT_ZERO_ADC;
  cfg.payload.spanRaw = MOTOR_BASE_DEFAULT_SPAN_RAW;

  cfg.payload.minStepDelayUs = 1500;
  ...
  cfg.payload.angleFilterAlpha = MOTOR_ANGLE_FILTER_ALPHA_DEFAULT;
  ...
  cfg.payload.controlModes = encodeControlModesForStore(MOTOR_MODE_HALFSTEP_UNIPOLAR, MOTOR_TYPE_DC_MOTOR);
  cfg.payload.manipMemberChannel[MANIP_MEMBER_BASE] = DEFAULT_MANIP_BASE_CH;
  cfg.payload.manipMemberChannel[MANIP_MEMBER_UPPER] = DEFAULT_MANIP_UPPER_CH;
  cfg.payload.manipMemberChannel[MANIP_MEMBER_FORE] = DEFAULT_MANIP_FORE_CH;
  cfg.payload.manipMemberChannel[MANIP_MEMBER_FOREARM_ROLL] = DEFAULT_MANIP_FOREARM_ROLL_CH;
  cfg.payload.manipMemberChannel[MANIP_MEMBER_WRIST_PITCH] = DEFAULT_MANIP_WRIST_PITCH_CH;
  cfg.payload.manipMemberChannel[MANIP_MEMBER_WRIST_ROLL] = DEFAULT_MANIP_WRIST_ROT_CH;
  cfg.payload.manipMemberChannel[MANIP_MEMBER_GRIPPER] = DEFAULT_MANIP_GRIPPER_CH;
  cfg.payload.baseA0DeadbandCentiDeg = encodeBaseA0DeadbandDeg(MOTOR_BASE_A0_DEADBAND_DEFAULT_DEG);
  cfg.payload.dcPidStopBandDeciDeg = encodeDcPidBandDeg(MOTOR_DC_PID_STOP_BAND_DEFAULT_DEG);
  cfg.payload.dcPidExitBandDeciDeg = encodeDcPidBandDeg(MOTOR_DC_PID_EXIT_BAND_DEFAULT_DEG);
  ...
  cfg.payload.servoMinLimit[DEFAULT_MANIP_GRIPPER_CH] = 0.0f;
  cfg.payload.servoMaxLimit[DEFAULT_MANIP_GRIPPER_CH] = MANIP_GRIPPER_MAX_DEG;
  ...
  cfg.payload.pidDirectionInverted = MOTOR_DC_PID_OUTPUT_INVERTED_DEFAULT ? 1 : 0;
  ...
  for (uint8_t i = 0; i < 4; i++) {
    cfg.payload.pwmBoot[i] = getDefaultPwmBootPercent(i);
  }
  cfg.payload.armStabilizeEnable = false;
  cfg.payload.armStabilizeGain = 0.10f;
  cfg.payload.armStabilizeThreshold = 5.0f;
  cfg.payload.gripPressureEnable = true;
  cfg.payload.gripPressureGain = 1.00f;
  memcpy_P(&cfg.payload.vehicleProfile, &kDefaultVehicleProfile, sizeof(cfg.payload.vehicleProfile));
  memcpy_P(&cfg.payload.droneProfile, &kDefaultDroneProfile, sizeof(cfg.payload.droneProfile));
  memcpy_P(&cfg.payload.vehicleControlProfile, &kDefaultVehicleControlProfile, sizeof(cfg.payload.vehicleControlProfile));
  memcpy_P(&cfg.payload.droneControlProfile, &kDefaultDroneControlProfile, sizeof(cfg.payload.droneControlProfile));
...
void applyConfigToRuntime(const PersistentConfig& cfg) {
  ...
  motorCalibrationSpanRaw = cfg.payload.spanRaw;
  ...
  motorBaseA0DeadbandDeg = decodeBaseA0DeadbandDeg(cfg.payload.baseA0DeadbandCentiDeg);
  motorDcPidStopBandDeg = decodeDcPidBandDeg(cfg.payload.dcPidStopBandDeciDeg, MOTOR_DC_PID_STOP_BAND_DEFAULT_DEG);
  motorDcPidExitBandDeg = decodeDcPidBandDeg(cfg.payload.dcPidExitBandDeciDeg, MOTOR_DC_PID_EXIT_BAND_DEFAULT_DEG);
  motorDcPidOutputInverted = cfg.payload.pidDirectionInverted != 0;
  ...
  motorDriveMode = decodeStoredMotorDriveMode(cfg.payload.controlModes);
  motorType = decodeStoredMotorType(cfg.payload.controlModes);
...
void syncRuntimeToConfig(PersistentConfig& cfg) {
  ...
  cfg.payload.spanRaw = motorCalibrationSpanRaw;
  ...
  cfg.payload.controlModes = encodeControlModesForStore(motorDriveMode, motorType);
  ...
  cfg.payload.baseA0DeadbandCentiDeg = encodeBaseA0DeadbandDeg(motorBaseA0DeadbandDeg);
  cfg.payload.dcPidStopBandDeciDeg = encodeDcPidBandDeg(motorDcPidStopBandDeg);
  cfg.payload.dcPidExitBandDeciDeg = encodeDcPidBandDeg(motorDcPidExitBandDeg);
  ...
  cfg.payload.pidDirectionInverted = motorDcPidOutputInverted ? 1 : 0;
  sanitizeVehicleProcessingProfile(vehicleProfile);
  sanitizeDroneProcessingProfile(droneProfile);
  sanitizeVehicleControlProfile(vehicleControlProfile);
  sanitizeDroneControlProfile(droneControlProfile);
...
bool loadConfigExtrasFromEEPROM() {
  ...
  if (persistentBlockValid(cfg, CONFIG_EXTRAS_MAGIC, CONFIG_EXTRAS_SCHEMA_ID, calculateConfigExtrasCRC(cfg))) {
...
bool loadConfigFromEEPROM() {
  ...
  if (persistentBlockValid(cfg, EEPROM_MAGIC, EEPROM_SCHEMA_ID, calculateConfigCRC(cfg))) {
  ...
  loadDroneEmergencyHomeFromEEPROM();
...
void eraseEEPROMRegion() {
  ...
  for (unsigned int i = 0; i < sizeof(PersistentDroneEmergencyHome); i++) {
    EEPROM.write(EEPROM_ADDR_DRONE_EMERGENCY_HOME + i, 0xFF);
  }
}

// Utility: rebind I2C devices through the generic guarded I2C layer.
void rebindI2CDevices() {
  // PWM expander / external PWM device.  initExternalPwmControllerDirect()
  // already performs a guarded presence probe, so do not probe the PCA twice.
  // One bounded attempt is enough: any failure switches immediately to the
  // native Arduino fallback and setup() continues.
  rt.pwmReady = initExternalPwmControllerDirect(pca9685Addr);
  if (rt.pwmReady) {
    detachFallbackServos();
    // Force the next servo update onto the newly selected output backend.
    for (uint8_t ch = FIRST_SERVO_CH; ch <= LAST_SERVO_CH; ch++) {
      setServoWriteValid(ch, false);
      servoLastWritten[ch] = 0;
    }
  } else {
...
// The Mega core exposes SDA/SCL as constants rather than macros, therefore
// #if defined(SDA) / defined(SCL) must not be used here.
static bool i2cBusLooksIdle() {
  pinMode(SYNROV_I2C_SDA_PIN, INPUT_PULLUP);
  pinMode(SYNROV_I2C_SCL_PIN, INPUT_PULLUP);
  delayMicroseconds(5);
  return digitalRead(SYNROV_I2C_SDA_PIN) == HIGH &&
         digitalRead(SYNROV_I2C_SCL_PIN) == HIGH;
}

// Releases a stuck I2C slave by clocking SCL and generating a STOP condition.
// On AVR the TWI peripheral is temporarily disabled so GPIO really owns pins
// 20/21 during recovery.  The previous TWI enable state is restored afterwards.
static void recoverI2CBusPins() {
#if defined(TWCR) && defined(TWEN)
  const bool twiWasEnabled = (TWCR & _BV(TWEN)) != 0;
  TWCR &= (uint8_t)~_BV(TWEN);
#else
  const bool twiWasEnabled = false;
#endif
  ...
  // Up to 18 clocks covers the standard 9-clock slave-release sequence with
  // one retry. Stop early as soon as SDA is released.
  for (uint8_t i = 0; i < 18 && digitalRead(SYNROV_I2C_SDA_PIN) == LOW; i++) {
    pinMode(SYNROV_I2C_SCL_PIN, OUTPUT);
    digitalWrite(SYNROV_I2C_SCL_PIN, LOW);
    delayMicroseconds(8);
    pinMode(SYNROV_I2C_SCL_PIN, INPUT_PULLUP);
    delayMicroseconds(8);
  }

  // Generate STOP only when SCL can actually be released high.
  pinMode(SYNROV_I2C_SCL_PIN, INPUT_PULLUP);
  delayMicroseconds(8);
  if (digitalRead(SYNROV_I2C_SCL_PIN) == HIGH) {
    pinMode(SYNROV_I2C_SDA_PIN, OUTPUT);
    digitalWrite(SYNROV_I2C_SDA_PIN, LOW);
    delayMicroseconds(8);
    pinMode(SYNROV_I2C_SDA_PIN, INPUT_PULLUP);
    delayMicroseconds(20);
  }
...
#if defined(TWCR) && defined(TWEN)
  if (twiWasEnabled) TWCR |= _BV(TWEN);
#endif
}
...
static bool i2cTransactionOk(uint8_t wireErr) {
  ...
  if (wireErr != 0) {
    // NACK is a normal "device absent" result.  If either physical line is
    // nevertheless stuck, recover it before the next peripheral is probed.
    if (!i2cBusLooksIdle()) recoverI2CBusPins();
    return false;
  }
  return true;
...
// Small GPIO helpers used by the bounded I2C probe.
static inline void i2cProbeReleaseLine(uint8_t pin) {
  pinMode(pin, INPUT_PULLUP);
}

static inline void i2cProbeDriveLow(uint8_t pin) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

static bool i2cProbeWaitHigh(uint8_t pin) {
  i2cProbeReleaseLine(pin);
...
// Performs a bounded GPIO-level I2C address probe.
// IMPORTANT: an absent device must never be discovered through Wire.endTransmission(),
// because a wedged AVR TWI state can block before the firmware gets a chance to select
// the Arduino fallback. This probe owns SDA/SCL temporarily, emits one START + address
// byte, samples ACK, then emits STOP. Every wait is explicitly bounded.
static bool i2cDevicePresent(uint8_t addr) {
  ...
  i2cProbeReleaseLine(SYNROV_I2C_SDA_PIN);
  i2cProbeReleaseLine(SYNROV_I2C_SCL_PIN);
  delayMicroseconds(5);

  bool ack = false;
  bool busOk = i2cProbeWaitHigh(SYNROV_I2C_SCL_PIN) && digitalRead(SYNROV_I2C_SDA_PIN) == HIGH;
  if (busOk) {
    // START: SDA high->low while SCL is high.
    i2cProbeDriveLow(SYNROV_I2C_SDA_PIN);
    delayMicroseconds(4);
    i2cProbeDriveLow(SYNROV_I2C_SCL_PIN);

    uint8_t byteOut = (uint8_t)(addr << 1); // write direction
    for (int8_t bit = 7; bit >= 0 && busOk; --bit) {
      if (byteOut & (1u << bit)) i2cProbeReleaseLine(SYNROV_I2C_SDA_PIN);
      else i2cProbeDriveLow(SYNROV_I2C_SDA_PIN);
      delayMicroseconds(2);
      busOk = i2cProbeWaitHigh(SYNROV_I2C_SCL_PIN);
      delayMicroseconds(2);
      i2cProbeDriveLow(SYNROV_I2C_SCL_PIN);
      delayMicroseconds(2);
    }
    ...
    if (busOk) {
      // Ninth clock: slave ACKs by pulling SDA low.
      i2cProbeReleaseLine(SYNROV_I2C_SDA_PIN);
      delayMicroseconds(2);
      busOk = i2cProbeWaitHigh(SYNROV_I2C_SCL_PIN);
      if (busOk) ack = (digitalRead(SYNROV_I2C_SDA_PIN) == LOW);
      delayMicroseconds(2);
      i2cProbeDriveLow(SYNROV_I2C_SCL_PIN);
    }

    // STOP, also bounded if SCL is being held low.
    i2cProbeDriveLow(SYNROV_I2C_SDA_PIN);
    delayMicroseconds(2);
    if (i2cProbeWaitHigh(SYNROV_I2C_SCL_PIN)) {
      delayMicroseconds(2);
      i2cProbeReleaseLine(SYNROV_I2C_SDA_PIN);
      delayMicroseconds(4);
    } else {
...
static void externalPwmFailoverToFallback() {
  ...
  // Keep fallback servos detached until the first armed write for each joint.
  // This avoids 50 Hz startup pulses and unexpected movement during boot.
  detachFallbackServos();
  configureFallbackOutputs();

  // If the PCA disappears while already running, restore the latest PWM
  // percentages immediately. At boot rt.active is false, so outputs remain at
  // their safe zero level until applyBootPwmDefaults() runs.
  if (rt.active) {
    for (uint8_t ch = FIRST_PWM_CH; ch <= LAST_PWM_CH; ch++) {
      uint8_t idx = ch - FIRST_PWM_CH;
      if (idx < 4) {
        uint8_t percent = (uint8_t)clampValue((int)servos.actual[ch], 0, 100);
        analogWrite(FALLBACK_PWM_PINS[idx], map(percent, 0, 100, 0, 255));
      }
    }
  }

  // Backend changed: invalidate the write cache so an unchanged target is
  // still emitted once on the Arduino fallback pins.
  for (uint8_t ch = FIRST_SERVO_CH; ch <= LAST_SERVO_CH; ch++) {
    setServoWriteValid(ch, false);
    servoLastWritten[ch] = 0;
  }
...
void disarmRuntimeOutputsAfterLinkTimeout() {
  ...
  // The generic reset already clears vehicle/drone motion, lights, camera,
  // flight automation and safe-return state. Failsafe only needs to override
  // the few runtime latches whose safe timeout value differs from normal reset.
