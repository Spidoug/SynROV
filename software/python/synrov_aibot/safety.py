    ...
    def install(self, bridge: Any) -> Any:
        ...
        if callable(original_control):
            def safe_send_control(payload: Dict[str, Any], *args: Any, **kwargs: Any) -> Any:
                safety.enabled = adapter._enabled()
                try:
                    safe_payload = adapter._sanitize_control_payload(payload)
                except Exception as exc:
                    emit = getattr(bridge, "_emit", None)
                    if callable(emit):
                        try:
                            emit("log", f"[safety] dropped malformed control payload: {exc}")
                        except Exception:
                            pass
                    return None
                return original_control(safe_payload, *args, **kwargs)
