...
uint32_t calculateDroneEmergencyHomeCRC(const PersistentDroneEmergencyHome& home) {
  return calculateCRC32((const uint8_t*)&home, sizeof(PersistentDroneEmergencyHome) - sizeof(uint32_t));
}

// Applies the same persistence rule to every magic/schema/CRC block.
// Keeping validation in one place prevents individual EEPROM loaders from
// drifting into different validation rules.
template <typename T>
