...
boolean applyManipulatorPoseTargets(int[] requestedAngles, boolean[] activeMask) {
  ...
  if (localCollisionFilteringActive) {
    ...
    if (!blocked && environmentCollisionEnabled) {
...
void setDuty(int channel, int percentage) {
  ...
  if (idx >= 0) {
    ...
    dutyInitialHardwareSyncDone[idx] = true;
  ...
  if (isManipulatorSelected && !directPwmSent) {
    ...
    updateMessage(tr("PWM auxiliar enviado diretamente ao firmware do manipulador.", "Aux PWM sent directly to the manipulator firmware."));
  }
...
void goHome() {
  ...
  try {
    ...
    if (allJointsAtHome && homeConfirmedByTelemetry) {
      ...
      updateMessage(tr("Homing aguardando confirmação da telemetria; proteção local ainda não ativada.",
        "Homing waiting for telemetry confirmation; local guard not enabled yet."));
    }
  }
  finally {
    commandContextSource = previousSource;
...
void resetConnectionRuntimeState() {
  ...
  dutyTargetsInitialized = true;
  dutyTargetsFollowHardware = manipulatorAutoPwmPowerControl;
  for (int i = 0; i < dutyInitialHardwareSyncDone.length; i++) {
    dutyInitialHardwareSyncDone[i] = false;
...
void finishSerialConnection(Serial confirmedPort, boolean requestRuntimeAfterConnect) {
  ...
  serialMonitorActivatedAtMillis = 0;
  hardwareStreamStoppedAtMillis = 0;
  for (int i = 0; i < dutyInitialHardwareSyncDone.length; i++) dutyInitialHardwareSyncDone[i] = false;
  dutyHardwareSnapshotReceived = false;
...
void resetTelemetryReadoutsForDisconnect() {
  ...
  for (int i = 0; i < dutyPercentages.length; i++) {
    // Keep the last known/commanded value visible while disconnected. The first
    // live pwm_12..pwm_15 snapshot replaces it as soon as the Arduino reconnects.
    dutyPercentages[i] = constrain(dutyTargetPercentages[i], 0, 100);
    if (i < dutyInitialHardwareSyncDone.length) dutyInitialHardwareSyncDone[i] = false;
    if (i < dutyPendingAckPercentages.length) dutyPendingAckPercentages[i] = -1;
    if (i < dutyPendingAckUntilMs.length) dutyPendingAckUntilMs[i] = 0;
  }
  dutyHardwareSnapshotReceived = false;
...
String[] manipulatorTorqueSensorKeys(int index) {
  if (index == 0) return new String[] {"an2", "an_2", "an_1", "base_torque", "base_torque_raw"};
  if (index == 1) return new String[] {"an3", "an_3", "an_2", "upper_torque", "upper_arm_torque_raw"};
  if (index == 2) return new String[] {"an4", "an_4", "an_3", "forearm_torque", "forearm_torque_raw"};
  return new String[] {"an5", "an_5", "an_4", "forearm_roll_torque", "forearm_roll_torque_raw"};
}
...
void applyManipulatorSensorState() {
  ...
  boolean pwmCommandShadowChanged = false;
  long pwmNow = millis();
  for (int i = 0; i < 4; i++) {
    int feedbackPct = constrain(dutyPercentages[i], 0, 100);
    boolean hasPwmTelemetry = latestSensors != null && latestSensors.hasKey("pwm_" + (12 + i));
    if (hasPwmTelemetry) {
      feedbackPct = constrain(round(getSensorFloat("pwm_" + (12 + i), feedbackPct)), 0, 100);
      pwmSnapshotPresent = true;
    }

    int pendingPct = (i < dutyPendingAckPercentages.length) ? dutyPendingAckPercentages[i] : -1;
    boolean awaitingAck = pendingPct >= 0 && pwmNow <= dutyPendingAckUntilMs[i];
    if (awaitingAck && hasPwmTelemetry && abs(feedbackPct - pendingPct) <= 1) {
      dutyPendingAckPercentages[i] = -1;
      dutyPendingAckUntilMs[i] = 0;
      awaitingAck = false;
    }
    if (awaitingAck) {
      dutyPercentages[i] = constrain(pendingPct, 0, 100);
    } else {
      if (pendingPct >= 0 && pwmNow > dutyPendingAckUntilMs[i]) {
        dutyPendingAckPercentages[i] = -1;
        dutyPendingAckUntilMs[i] = 0;
      }
    ...
    boolean channelHasNoLocalOverride = diagnosticsActiveSlider != i && dutyPendingAckPercentages[i] < 0;

    // The Arduino is the authoritative source for the auxiliary-force sliders
    // whenever the operator is not actively changing that channel. This keeps the
    // slider position synchronized with the real CH12..CH15 outputs continuously,
    // not only on the first telemetry snapshot. Auto Torque retains its local target
    // because it is itself the active command source; dutyPercentages[] still shows
    // the real feedback value beside it.
    boolean shouldMirrorHardwareToSlider = hasPwmTelemetry && channelHasNoLocalOverride &&
      !manipulatorAutoPwmPowerControlEnabled();

    if (shouldMirrorHardwareToSlider) {
      if (dutyTargetPercentages[i] != dutyPercentages[i]) {
        dutyTargetPercentages[i] = dutyPercentages[i];
        pwmCommandShadowChanged = true;
      }
      dutyInitialHardwareSyncDone[i] = true;
    } else if (hasPwmTelemetry && !dutyInitialHardwareSyncDone[i] && dutyPendingAckPercentages[i] < 0) {
      dutyInitialHardwareSyncDone[i] = true;
    }
  }
  if (pwmSnapshotPresent) {
    dutyHardwareSnapshotReceived = true;
  }
  dutyTargetsInitialized = true;
  if (pwmCommandShadowChanged) {
    for (int i = 0; i < dutyChannels.length; i++) {
      sendDutyStatePacket(dutyChannels[i]);
    }
  }
