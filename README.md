...
The Manipulator provides joint-level control, HOME/action playback, optional Leap Motion and joystick input, Auto Torque support, collision checking, base heading feedback, and workspace-oriented AiBot perception and missions. HOME always targets the V1 initial pose `180,150,70,90,95,130,0`; Processing, AiBot and firmware use that same seven-member pose. A HOME request cancels pending Manipulator interpolation/playback before the homing trajectory starts, so AiBot, Web, ROS and local actions cannot leave a stale target competing with HOME.
...
Vertical throttle is handled by one Processing gate for every producer. The HTML held descend control sends canonical negative throttle without depending on browser `flightReady`; Processing blocks descent only inside the landed zone. Once measurable lift exists, a manual negative command can cancel automatic takeoff and descend, while a manual positive command can cancel automatic landing and climb.

## 3D camera behavior

Vehicle and Drone use one camera-enable state. Enabling CAMERA from Processing diagnostics, the HTML console, or AiBot turns the camera indicator sphere yellow, starts the robot-camera channel, and activates the same **rear chase view** for the selected robot. Vehicle and Drone use the same third-person camera-mode zoom reference, and the operator camera rotation/zoom are restored when CAMERA is disabled. In every Drone 3D orbit/zoom view, the Drone itself is the vertical scene reference: climb/descent moves the ground, grid, and imported world relative to the aircraft, keeping the Drone centered instead of allowing altitude changes to push it out of frame.

Camera image data is transported on port `9002`. Processing places the camera monitor in 3D from the robot's actual camera origin and gimbal direction. Vehicle and Drone share the same mechanical gimbal safety envelope:
...
ROS commands are accepted only through `/synrov/control` using the canonical SynROV JSON envelope inside `std_msgs/msg/String`. `/cmd_vel` and `/joint_targets` are intentionally not parallel command inputs. The rosbridge topic registrations use full ROS 2 interface names, stable operation IDs and explicit `keep_last / reliable / volatile` QoS so behavior does not depend on rosbridge defaults. `/synrov/joint_states` publishes all seven Manipulator articulations as angular positions in radians, including the modeled gripper-finger articulation.
...
SynROV has one canonical runtime coordinate system for every control source. Web, AiBot, ROS, joystick, Leap Motion and local Processing controls all send the same fields with the same numeric meaning; only the human/visual control mapping changes before transmission. Vehicle and Drone share one camera-button convention: `Z = left (+)` and `X = right (-)` for pan, plus `R = down (-)` and `F = up (+)` for tilt. The joystick schema is `synrov.joystick`; Vehicle and Drone share one camera-tilt visual calibration, while digital `POS/NEG` bindings retain their explicit numeric direction. The Web yaw/pivot buttons and AiBot Drone yaw phrases use the operator visual reference without redefining the protocol yaw/pivot fields. Vehicle steering and in-place pivot remain separate commands. See `docs/CONTROL_DIRECTION_AND_SAFE_HOME.md` for the complete coordinate and visual-reference tables.
