...
float readMotorAngle() {
  ...
  const float maxRaw = minRaw + rawSpan;
  ...
  // The old implementation clamped raw A0 before converting it to degrees.
...
float getMotorRawSpan() {
  int32_t maxSpan = (int32_t)MOTOR_ADC_MAX - (int32_t)motorCalibrationZeroADC;
  if (maxSpan < MOTOR_CAL_MIN_SPAN_RAW) maxSpan = MOTOR_CAL_MIN_SPAN_RAW;
  int32_t safeSpan = clampValue<int32_t>(motorOneTurnSpanRaw, MOTOR_CAL_MIN_SPAN_RAW, maxSpan);
  return (float)safeSpan;
}
...
void sanitizeMotorRuntimeParams() {
  // Keep the complete selected one-turn window inside the Mega2560 10-bit ADC.
  // This does NOT select the potentiometer electrical extremes; it only validates
  // the user-selected ZEROADC..ZEROADC+SPANRAW interval.
  ...
  int32_t maxSpan = (int32_t)MOTOR_ADC_MAX - (int32_t)motorCalibrationZeroADC;
  motorOneTurnSpanRaw = clampValue<int32_t>(
    motorOneTurnSpanRaw,
    MOTOR_CAL_MIN_SPAN_RAW,
    maxSpan);
...
void getMotorAdaptiveBands(float &holdBand, float &enterBand, float &exitBand, float &nearBand, float &farBand) {
  ...
  if (motorType == MOTOR_TYPE_STEPPER) {
    ...
    holdBand  = max((float)motorDcPidEnterBandDeg, degPerCount * 0.85f);
