...
Each robot now owns a real persistent policy at `runtime_data/models/<robot>/<robot>_policy.pkl` and an experience trail at `runtime_data/datasets/<robot>_training.jsonl`. The learner uses a bounded UCB strategy selector: during trainable search/inspection missions it changes strategy only at controlled time segments, tries each approved variant, scores the result from available vision, clearance/collision information, battery/link state and mission success, and reuses higher-performing variants in later segments and later mission runs.
