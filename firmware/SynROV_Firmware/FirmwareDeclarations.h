...
// This avoids Arduino .ino auto-prototype issues.
enum StepperDriveMode : uint8_t;
enum Ch0DriveMode : uint8_t;
...
// emit broken auto-generated prototypes before these types exist.
...
// ---------------- Hardware ----------------
const uint8_t STEPPER_COIL_PINS[4]  = {42, 43, 44, 45};
const uint8_t DC_MOTOR_DIR_PINS[2]   = {42, 43};
const uint8_t STEPPER_POWER_PWM_PIN = 46;
...
const unsigned long I2C_CLOCK_SPEED    = 400000UL;
...
const unsigned long STEPPER_CONTROL_US = 1000UL;
...
const uint16_t SERIAL_RX_BYTE_BUDGET = 128;
const uint8_t SERIAL_RX_LINE_BUDGET = 3;
const uint8_t STEPPER_FEEDBACK_SAMPLES = 6;
const unsigned long STEPPER_FEEDBACK_MIN_INTERVAL_US = 2500UL;

const uint8_t MAX_CHANNELS   = 16;
const uint8_t SERIAL_TEXT_BUFFER_SIZE = 96;
...
const float STEPPER_INPUT_MIN = 0.0f;
const float STEPPER_INPUT_MAX = 359.0f;
...
const bool STEPPER_NATIVE_SENSOR_INVERTED = true;
const bool STEPPER_NATIVE_DIRECTION_INVERTED = true;
...
enum StepperDriveMode : uint8_t {
  STEPPER_MODE_FULLSTEP_BIPOLAR = 0,
...
// Default stepper electrical mode. This can also be changed inside CFG=1234
// with STEPMODE=<0..3> or a mode name, then persisted with SAVE.
// Pin order in STEPPER_COIL_PINS:
//   [0] = A+
//   [1] = A-
//   [2] = B+
//   [3] = B-
...
StepperDriveMode stepperDriveMode = STEPPER_MODE_HALFSTEP_UNIPOLAR;
Ch0DriveMode ch0DriveMode = CH0_MODE_STEPPER;
...
const uint8_t STEPMODE_STORE_MAGIC = 0xA0;
...
unsigned long lastStepperFeedbackReadUs = 0;
...
// output for the stepper power stage. If any Servo object lands on Timer5,
// the library reconfigures Timer5 for 50 Hz servo pulses and PWM on pin 46
// appears "dead".
//
// To keep Timer5 exclusively for the stepper driver we reserve all Timer5
...
// This removes the library/timer conflict without touching the Servo library.
...
class ManualStepperPulse {
  public:
    typedef void (*StepCallback)();

    ManualStepperPulse(StepCallback forwardCb = NULL, StepCallback backwardCb = NULL)
...
struct AutoStepperBest {
  bool valid;
  float score;
  AutoStepperCandidate params;
};

struct AutoStepperState {
  bool active;
  bool stopRequested;
  AutoStepperStage stage;
  AutoStepperStage cachedStage;
  ...
  AutoStepperCandidate seed;
  AutoStepperCandidate current;
  AutoStepperCandidate candidates[4];
  AutoStepperBest best;
  float testMotions[5];
  uint8_t modeAttemptIndex;
  StepperDriveMode originalMode;
};

enum StepperCalStage : uint8_t {
  STEPPER_CAL_IDLE = 0,
  STEPPER_CAL_WAIT_LIMIT1,
  STEPPER_CAL_WAIT_LIMIT2,
  STEPPER_CAL_DONE
};

struct StepperCalState {
  bool active;
  StepperCalStage stage;
  int16_t limit1Adc;
  int16_t limit2Adc;
};

struct DirectStepperTestState {
  bool active;
  bool forward;
  uint16_t totalSteps;
...
DirectStepperTestState directStepperTest = {false, true, 0, 0, 0, 0, 0};
...
ManualStepperPulse stepperMotor(accelStepForward, accelStepBackward);

int16_t  stepperCalibrationZeroADC = 450;
int32_t  stepperOneTurnSpanRaw     = 300;
uint16_t minStepDelayUs            = 2000;
uint16_t maxStepDelayUs            = 20000;
float    stepperStopEnter          = 4.5f;
float    stepperStopExit           = 7.5f;
uint16_t movePwmFar                = 270;
uint16_t movePwmNear               = 250;
uint16_t holdPwm                   = 200;
float    angleFilterAlpha          = 0.300f;
float    speedFilterAlpha          = 0.500f;
uint32_t controlIntervalUs         = 8000UL;
float    servoMoveStepDeg          = 0.40f;
float    servoRampSmoothing       = 0.60f;
...
const int16_t COLLISION_POSE_EPS_CENTI = 5;
const unsigned long COLLISION_GUARD_INTERVAL_MS = 120UL;
...
bool gripPressureEnable = false;
float gripPressureGain = 0.02f;
...
bool     stepperFeedbackInsideWindow = true;
uint8_t  stepperRecoveryPwm          = 0;
...
AutoStepperState autoStepper;
StepperCalState stepperCal;
...
float convertManipulatorMemberToServoDeg(uint8_t memberIdx, float memberDeg) {
  ...
  if (memberIdx == MANIP_MEMBER_BASE) {
    if (ch == 0) return clampValue(armBaseMemberToServoDeg(memberDeg), STEPPER_INPUT_MIN, STEPPER_INPUT_MAX);
...
static const float ARM_COLLISION_TOLERANCE = 1.0f;
...
float armOBBProjectRadius(const ArmOOBB &b, RuntimeVec3 axis) {
  ...
  return b.ex * fabsf(ArmCollisionMath::dot(b.ux, axis)) +
         b.ey * fabsf(ArmCollisionMath::dot(b.uy, axis)) +
         b.ez * fabsf(ArmCollisionMath::dot(b.uz, axis));
}
...
bool armOBBOverlapOnAxis(const ArmOOBB &a, const ArmOOBB &b, RuntimeVec3 axis, float tol) {
  ...
  float ra = armOBBProjectRadius(a, axis);
  float rb = armOBBProjectRadius(b, axis);
...
bool armCylinderOBBIntersects(const ArmCylinderY &c, const ArmOOBB &b, float tol) {
  ...
  float rY = armOBBProjectRadius(b, Y);
  float bMin = b.c.y - rY;
  float bMax = b.c.y + rY;
  ...
  float hx = armOBBProjectRadius(b, ArmCollisionMath::v3(ax.x, 0, ax.z));
  float hz = armOBBProjectRadius(b, ArmCollisionMath::v3(az.x, 0, az.z));
...
bool armPoseHasOOBBCollision(float baseDeg, float upperDeg, float foreDeg, float forearmRollDeg, float wristPitchDeg, float wristRotDeg, float gripDeg) {
  ...
  for (uint8_t i = 0; i < count; i++) {
    for (uint8_t j = i + 1; j < count; j++) {
      ...
      if (armColliderIntersects(cols[i], cols[j], ARM_COLLISION_TOLERANCE)) return true;
    }
  }
  return false;
}
...
uint8_t poseCollisionSeverity(float baseDeg, float upperDeg, float foreDeg, float forearmRollDeg, float wristPitchDeg, float wristRotDeg, float gripDeg) {
  ...
  for (uint8_t i = 0; i < count; i++) {
    for (uint8_t j = i + 1; j < count; j++) {
      if (armShouldIgnoreCollisionPair(cols[i].type, cols[j].type)) continue;
      if (armColliderIntersects(cols[i], cols[j], ARM_COLLISION_TOLERANCE)) {
...
uint8_t encodeControlModesForStore(StepperDriveMode stepMode, Ch0DriveMode ch0Mode);
StepperDriveMode decodeStoredStepperDriveMode(uint8_t rawMode);
