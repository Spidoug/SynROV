// =====================================================================
// SynROV Firmware V1 - Initialization helpers
// ---------------------------------------------------------------------
// Purpose:
...
void initMotorState() {
  motor.kp = MOTOR_DC_KP_DEFAULT;
  motor.ki = MOTOR_DC_KI_DEFAULT;
  motor.kd = MOTOR_DC_KD_DEFAULT;
  motor.targetAngle = 180.0f;
  motor.controlTargetAngle = 180.0f;
  motor.currentAngle = 180.0f;
  motor.filteredPot = 0.0f;
  motor.filteredAngle = 180.0f;
  motor.controlFeedbackAngle = 180.0f;
  motor.filteredSpeed = 0.0f;
  motor.lastAngle = 180.0f;
  motor.integral = 0.0f;
  motor.pwmValue = holdPwm;
  motor.inHoldBand = false;
  motor.directionForward = true;
...
void initRuntimeState() {
  ...
  directMotorTest.forward = true;
  directMotorTest.totalSteps = 0;
  directMotorTest.stepsRemaining = 0;
  directMotorTest.pwmRaw = 0;
  directMotorTest.stepDelayUs = 0;
  directMotorTest.lastStepUs = 0;
}

// Checks whether motor PWM timer healthy.
...
void setupHighFreqPwmPin46() {
  pinMode(MOTOR_POWER_PWM_PIN, OUTPUT);
...
// Ensures motor PWM timer ready.
void ensureMotorPwmTimerReady() {
  if (!isMotorPwmTimerHealthy()) {
...
// Re-arms only the channels that belong to the active manipulator profile.
// Non-manipulator modes intentionally neutralize all arm outputs, so switching
// back to MANIPULATOR must clear those neutral flags or the next ARMJ/0x18
// command is accepted but no servo/motor output is written.
void prepareManipulatorHardwareForActiveProfile() {
  sanitizeManipulatorProfile();
  enforceManipulatorGripperRange();

  for (uint8_t i = 0; i < MANIP_MEMBER_COUNT; i++) {
    uint8_t ch = getManipulatorMemberChannel(i);
    if (ch > LAST_SERVO_CH) continue;
    setServoNeutral(ch, false);
    setServoWriteValid(ch, false);
  }

  if (getManipulatorMemberChannel(MANIP_MEMBER_BASE) != 0) {
    rt.motorArmed = false;
    releaseMotorCoils();
    motorSetDriverPWM(0);
  }
...
void configureRobotModeHardware() {
  ...
  if (robotControlMode == ROBOT_MODE_MANIPULATOR) {
    ...
    // Fallback Servo objects are attached lazily by writeChannel().  Do not
    // attach all joints during boot: that would emit pulses before outputs are
    // armed and can cause an unexpected startup movement.
    if (!rt.pwmReady) detachFallbackServos();
    return;
  }
  ...
  if (robotControlMode == ROBOT_MODE_VEHICLE) {
    ...
    vehicleCameraTiltDeg = 0;
    vehicleCameraPanServo.write(90);
    vehicleCameraTiltServo.write(90);
  ...
  if (robotControlMode == ROBOT_MODE_DRONE) {
    ...
    droneCameraPanDeg = 0;
    droneCameraTiltDeg = 0;
    droneCameraPanServo.write(90);
    droneCameraTiltServo.write(90);
...
void updateVehicleHardwareOutputs() {
  ...
  int panDeg = (int)round(clampValue((float)vehicleCameraPanDeg, CAMERA_PAN_MIN_DEG, CAMERA_PAN_MAX_DEG));
  int tiltDeg = (int)round(clampValue((float)vehicleCameraTiltDeg, CAMERA_TILT_MIN_DEG, CAMERA_TILT_MAX_DEG));
  int panCmd = constrain(90 + panDeg, CAMERA_PAN_SERVO_MIN_DEG, CAMERA_PAN_SERVO_MAX_DEG);
  int tiltCmd = constrain(90 + tiltDeg, CAMERA_TILT_SERVO_MIN_DEG, CAMERA_TILT_SERVO_MAX_DEG);
...
void updateDroneHardwareOutputs() {
  ...
  int panCmd = constrain(90 + constrain(droneCameraPanDeg, (int)CAMERA_PAN_MIN_DEG, (int)CAMERA_PAN_MAX_DEG), CAMERA_PAN_SERVO_MIN_DEG, CAMERA_PAN_SERVO_MAX_DEG);
  int tiltCmd = constrain(90 + constrain(droneCameraTiltDeg, (int)CAMERA_TILT_MIN_DEG, (int)CAMERA_TILT_MAX_DEG), CAMERA_TILT_SERVO_MIN_DEG, CAMERA_TILT_SERVO_MAX_DEG);
...
// Returns motor drive mode name.
const __FlashStringHelper* getMotorDriveModeName(MotorDriveMode mode) {
  switch (mode) {
    case MOTOR_MODE_FULLSTEP_BIPOLAR:  return F("FULLSTEP_BIPOLAR");
    case MOTOR_MODE_HALFSTEP_BIPOLAR:  return F("HALFSTEP_BIPOLAR");
    case MOTOR_MODE_FULLSTEP_UNIPOLAR: return F("FULLSTEP_UNIPOLAR");
    case MOTOR_MODE_HALFSTEP_UNIPOLAR: return F("HALFSTEP_UNIPOLAR");
    default: return F("HALFSTEP_UNIPOLAR");
  }
}

// Sanitizes and clamps motor drive mode.
MotorDriveMode sanitizeMotorDriveMode(uint8_t rawMode) {
  if (rawMode > (uint8_t)MOTOR_MODE_HALFSTEP_UNIPOLAR) {
    return MOTOR_MODE_HALFSTEP_UNIPOLAR;
  }
  return (MotorDriveMode)rawMode;
}

// Utility: encode control modes for store.
uint8_t encodeControlModesForStore(MotorDriveMode mode, MotorType motorTypeValue) {
  uint8_t packed = 0;
  packed |= ((uint8_t)sanitizeMotorDriveMode((uint8_t)mode) & 0x03);
  packed |= ((uint8_t)sanitizeMotorType((uint8_t)motorTypeValue) & 0x01) << 2;
  return (uint8_t)(STEPPERMODE_STORE_MAGIC | (packed & 0x0F));
}

// Utility: decode the stored motor drive mode.
MotorDriveMode decodeStoredMotorDriveMode(uint8_t rawMode) {
  if ((rawMode & 0xF0) != STEPPERMODE_STORE_MAGIC) {
    return MOTOR_MODE_HALFSTEP_UNIPOLAR;
  }
  return sanitizeMotorDriveMode(rawMode & 0x03);
}

// Sanitizes and clamps motor type.
MotorType sanitizeMotorType(uint8_t rawMode) {
  return (rawMode == (uint8_t)MOTOR_TYPE_DC_MOTOR) ? MOTOR_TYPE_DC_MOTOR : MOTOR_TYPE_STEPPER;
}

// Utility: decode the stored motor type.
MotorType decodeStoredMotorType(uint8_t rawMode) {
  if ((rawMode & 0xF0) != STEPPERMODE_STORE_MAGIC) {
    return MOTOR_TYPE_DC_MOTOR;
  }
  return sanitizeMotorType((rawMode >> 2) & 0x01);
}

// Returns motor type name.
const __FlashStringHelper* getMotorTypeName(MotorType mode) {
  return (mode == MOTOR_TYPE_DC_MOTOR) ? F("DC_MOTOR") : F("STEPPER");
}
...
void applyRobotControlMode(RobotControlMode mode, bool announce) {
  RobotControlMode newMode = sanitizeRobotControlMode((uint8_t)mode);
...
// Switches robot mode from runtime streams without requiring CFG mode.
...
bool switchRobotModeForRuntime(RobotControlMode mode, bool announce, bool sendSnapshot) {
  RobotControlMode newMode = sanitizeRobotControlMode((uint8_t)mode);
  bool changed = (robotControlMode != newMode);

  if (changed) {
    applyRobotControlMode(newMode, announce);
...
// Loads the robot control mode record from EEPROM.
void loadRobotControlModeFromEEPROM() {
  const uint8_t raw = EEPROM.read(EEPROM_ADDR_ROBOT_MODE);
  const uint8_t marker = EEPROM.read(EEPROM_ADDR_ROBOT_MODE + 1);
  if (marker != ROBOT_MODE_STORE_MAGIC || raw > (uint8_t)ROBOT_MODE_DRONE) {
    applyRobotControlMode(ROBOT_MODE_MANIPULATOR, false);
    EEPROM.write(EEPROM_ADDR_ROBOT_MODE, (uint8_t)ROBOT_MODE_MANIPULATOR);
    EEPROM.write(EEPROM_ADDR_ROBOT_MODE + 1, ROBOT_MODE_STORE_MAGIC);
    return;
  }
  applyRobotControlMode((RobotControlMode)raw, false);
}

// Saves the robot control mode record to EEPROM.
...
// Clears only the vehicle drive vector. Auxiliary outputs such as lights,
// lidar and camera are intentionally left untouched.
void zeroVehicleMotionCommand() {
  vehicleLeftTrackPct = 0;
  vehicleRightTrackPct = 0;
}

// Clears only the drone flight vector. Camera and automation latches are
// handled by the caller because STOP, TAKEOFF, LAND and failsafe differ there.
...
void resetGenericRobotRuntimeState() {
  ...
  vehicleCameraPanDeg = (int)round(clampValue(vehicleProfile.cameraPanDefaultDeg, CAMERA_PAN_MIN_DEG, CAMERA_PAN_MAX_DEG));
  vehicleCameraTiltDeg = (int)round(clampValue(vehicleProfile.cameraTiltDefaultDeg, CAMERA_TILT_MIN_DEG, CAMERA_TILT_MAX_DEG));
...
// Returns one member of the fixed SynROV V1 Manipulator HOME pose.
int16_t getManipulatorHomeMemberDeg(uint8_t memberIdx) {
  if (memberIdx >= MANIP_MEMBER_COUNT) return 0;
  return (int16_t)pgm_read_word_near(kManipulatorHomePose + memberIdx);
}
...
void sanitizeVehicleProcessingProfile(VehicleProcessingProfile &cfg) {
  ...
  cfg.cameraPanDefaultDeg = clampValue(cfg.cameraPanDefaultDeg, CAMERA_PAN_MIN_DEG, CAMERA_PAN_MAX_DEG);
  cfg.cameraTiltDefaultDeg = clampValue(cfg.cameraTiltDefaultDeg, CAMERA_TILT_MIN_DEG, CAMERA_TILT_MAX_DEG);
...
void sanitizeVehicleControlProfile(VehicleControlProfile &cfg) {
  ...
  // Camera limits are fixed by the mechanical/safety contract.
  ...
  cfg.maxSpeedMps = clampValue(cfg.maxSpeedMps, 0.05f, 20.0f);
}
...
void sanitizeDroneControlProfile(DroneControlProfile &cfg) {
  ...
  cfg.maxMotorRpm = clampValue(cfg.maxMotorRpm, 1000.0f, 100000.0f);
}
...
uint8_t sanitizeManipulatorMemberChannel(uint8_t memberIdx, uint8_t ch) {
  if (memberIdx == MANIP_MEMBER_BASE) {
    if (ch <= 1) return ch;
    return DEFAULT_MANIP_BASE_CH;
  }
  if (ch >= DEFAULT_MANIP_UPPER_CH && ch <= LAST_SERVO_CH) return ch;
  switch (memberIdx) {
    case MANIP_MEMBER_UPPER: return DEFAULT_MANIP_UPPER_CH;
    case MANIP_MEMBER_FORE: return DEFAULT_MANIP_FORE_CH;
    case MANIP_MEMBER_FOREARM_ROLL: return DEFAULT_MANIP_FOREARM_ROLL_CH;
    case MANIP_MEMBER_WRIST_PITCH: return DEFAULT_MANIP_WRIST_PITCH_CH;
    case MANIP_MEMBER_WRIST_ROLL: return DEFAULT_MANIP_WRIST_ROT_CH;
    case MANIP_MEMBER_GRIPPER: return DEFAULT_MANIP_GRIPPER_CH;
    default: return DEFAULT_MANIP_UPPER_CH;
  }
}
...
uint8_t getDefaultManipulatorMemberChannel(uint8_t memberIdx) {
  switch (memberIdx) {
    case MANIP_MEMBER_BASE: return DEFAULT_MANIP_BASE_CH;
    case MANIP_MEMBER_UPPER: return DEFAULT_MANIP_UPPER_CH;
    case MANIP_MEMBER_FORE: return DEFAULT_MANIP_FORE_CH;
    case MANIP_MEMBER_FOREARM_ROLL: return DEFAULT_MANIP_FOREARM_ROLL_CH;
    case MANIP_MEMBER_WRIST_PITCH: return DEFAULT_MANIP_WRIST_PITCH_CH;
...
float getManipulatorMemberStartupSafeDeg(uint8_t memberIdx) {
  if (memberIdx >= MANIP_MEMBER_COUNT) return 90.0f;
  return (float)getManipulatorHomeMemberDeg(memberIdx);
}
...
bool setManipulatorBaseChannelFlexible(uint8_t ch) {
  ...
  if (sanitized == 0) {
    if (previousCh == 0) {
      seededBaseDeg = clampValue(readMotorAngle(), MOTOR_INPUT_MIN, MOTOR_INPUT_MAX);
  ...
  if (sanitized == 0) {
    motor.targetAngle = clampValue(seededBaseDeg, MOTOR_INPUT_MIN, MOTOR_INPUT_MAX);
    motor.currentAngle = motor.targetAngle;
    servos.target[0] = motor.targetAngle;
    servos.actual[0] = motor.targetAngle;
...
void sanitizeManipulatorProfile() {
  if (manipProfile.baseChannel > 1) manipProfile.baseChannel = DEFAULT_MANIP_BASE_CH;
  if (manipProfile.upperChannel < DEFAULT_MANIP_UPPER_CH || manipProfile.upperChannel > LAST_SERVO_CH) manipProfile.upperChannel = DEFAULT_MANIP_UPPER_CH;
  if (manipProfile.foreChannel < DEFAULT_MANIP_UPPER_CH || manipProfile.foreChannel > LAST_SERVO_CH) manipProfile.foreChannel = DEFAULT_MANIP_FORE_CH;
  if (manipProfile.forearmRollChannel < DEFAULT_MANIP_UPPER_CH || manipProfile.forearmRollChannel > LAST_SERVO_CH) manipProfile.forearmRollChannel = DEFAULT_MANIP_FOREARM_ROLL_CH;
  if (manipProfile.wristPitchChannel < DEFAULT_MANIP_UPPER_CH || manipProfile.wristPitchChannel > LAST_SERVO_CH) manipProfile.wristPitchChannel = DEFAULT_MANIP_WRIST_PITCH_CH;
  if (manipProfile.wristRotChannel < DEFAULT_MANIP_UPPER_CH || manipProfile.wristRotChannel > LAST_SERVO_CH) manipProfile.wristRotChannel = DEFAULT_MANIP_WRIST_ROT_CH;
  if (manipProfile.gripperChannel < DEFAULT_MANIP_UPPER_CH || manipProfile.gripperChannel > LAST_SERVO_CH) manipProfile.gripperChannel = DEFAULT_MANIP_GRIPPER_CH;
  ...
  for (uint8_t idx = 0; idx < MANIP_MEMBER_COUNT; idx++) {
    ...
    if (used[ch]) {
      ...
      if (fallback <= LAST_SERVO_CH && !used[fallback]) {
        ...
        uint8_t probeStart = (member == MANIP_MEMBER_BASE) ? 0 : DEFAULT_MANIP_UPPER_CH;
...
float getManipulatorMemberMinLimit(uint8_t memberIdx) {
  ...
  if (memberIdx == MANIP_MEMBER_BASE && ch == 0) return MOTOR_INPUT_MIN;
...
float getManipulatorMemberMaxLimit(uint8_t memberIdx) {
  uint8_t ch = getManipulatorMemberChannel(memberIdx);
  if (memberIdx == MANIP_MEMBER_BASE && ch == 0) return MOTOR_INPUT_MAX;
...
float getManipulatorMemberActualServoDeg(uint8_t memberIdx) {
  ...
  if (memberIdx == MANIP_MEMBER_BASE && baseTelemetryUsesPotentiometer()) {
    return clampValue(readMotorAngle(), MOTOR_INPUT_MIN, MOTOR_INPUT_MAX);
  }
  if (ch >= MAX_CHANNELS) return 0.0f;
  if (memberIdx == MANIP_MEMBER_BASE && ch == 0) {
    return clampValue(motor.currentAngle, MOTOR_INPUT_MIN, MOTOR_INPUT_MAX);
  }
  if (memberIdx == MANIP_MEMBER_BASE &&
...
float getManipulatorMemberTargetServoDeg(uint8_t memberIdx) {
  uint8_t ch = getManipulatorMemberChannel(memberIdx);
  if (memberIdx == MANIP_MEMBER_BASE && ch == 0) {
    return clampValue(servos.target[0], MOTOR_INPUT_MIN, MOTOR_INPUT_MAX);
  }
  if (ch >= MAX_CHANNELS) return 0.0f;
...
bool handleVehicleRuntimeCommand(char* cmd) {
  ...
  if (strncmp(cmd, "CAM=", 4) == 0) {
    ...
    vehicleCameraPanDeg = (int)round(clampValue((float)pan, CAMERA_PAN_MIN_DEG, CAMERA_PAN_MAX_DEG));
    vehicleCameraTiltDeg = (int)round(clampValue((float)tilt, CAMERA_TILT_MIN_DEG, CAMERA_TILT_MAX_DEG));
    return true;
  }
...
bool handleDroneRuntimeCommand(char* cmd) {
  ...
  if (strncmp(cmd, "CAM=", 4) == 0) {
    ...
    if (parseStrictCsvLongValues(cmd + 4, camVals, 2, camCount) && camCount == 2) {
      droneCameraPanDeg = constrain((int)camVals[0], (int)CAMERA_PAN_MIN_DEG, (int)CAMERA_PAN_MAX_DEG);
      droneCameraTiltDeg = constrain((int)camVals[1], (int)CAMERA_TILT_MIN_DEG, (int)CAMERA_TILT_MAX_DEG);
      return true;
    }
    bool parsedEnable = false;
...
void updateGenericRobotRuntime() {
  ...
  if (robotControlMode == ROBOT_MODE_VEHICLE) {
    ...
    if (droneAutoTakeoffActive) {
      ...
      droneTelemetryTargetLift = min(targetLift, droneTelemetryTargetLift + max(0.35f, (float)droneTelemetryThrottleCmdFiltered) * liftRate);
      if (measuredAltCm >= (droneAutoTargetAltitudeCm - 6.0f)) {
        droneAutoTakeoffActive = false;
      }
    } else if (droneAutoLandActive) {
      droneTelemetryTargetLift = max(0.0f, droneTelemetryTargetLift - max(0.45f, (float)droneTelemetryThrottleCmdFiltered) * liftRate);
    ...
    droneTelemetryAltitudeCm = max((float)droneAltitudeFromGroundCm, max(0.0f, (float)droneTelemetryLift * 2.0f));
    droneTelemetryY = droneTelemetryAltitudeCm * 0.5f;
  }
}

// Parses motor drive mode value.
...
// Parses motor type value.
bool parseMotorTypeValue(const char* value, MotorType* outMode) {
  if (value == NULL || outMode == NULL) return false;
  if (strcmp(value, "0") == 0 || strcmp(value, "STEPPER") == 0 || strcmp(value, "STEP") == 0) {
    *outMode = MOTOR_TYPE_STEPPER;
    return true;
  }
  if (strcmp(value, "1") == 0 || strcmp(value, "DC") == 0 || strcmp(value, "DC_MOTOR") == 0) {
    *outMode = MOTOR_TYPE_DC_MOTOR;
    return true;
  }
  return false;
}
...
void setMotorType(MotorType mode, bool announce) {
  motorType = sanitizeMotorType((uint8_t)mode);
  configureCh0PinsForCurrentMode();
  releaseMotorCoils();
  motorSetDriverPWM(0);
  if (announce) {
    Serial.print(F("MOTORTYPE="));
    Serial.print((int)motorType);
    Serial.print(F(" ["));
    Serial.print(getMotorTypeName(motorType));
    Serial.println(F("]"));
  }
}

// Sets motor drive mode.
void setMotorDriveMode(MotorDriveMode mode, bool announce) {
  motorDriveMode = sanitizeMotorDriveMode((uint8_t)mode);
  int8_t phaseCount = (int8_t)getMotorPhaseCount();
  if ((int8_t)motor.seq >= phaseCount) {
    motor.seq = PHASE_0;
  }
  applyStepPhase(motor.seq);
  if (announce) {
    Serial.print(F("STEPPERMODE="));
    Serial.print((int)motorDriveMode);
    Serial.print(F(" ["));
    Serial.print(getMotorDriveModeName(motorDriveMode));
    Serial.println(F("]"));
  }
}

// Returns motor mode attempt by index.
MotorDriveMode getMotorModeAttemptByIndex(MotorDriveMode baseMode, uint8_t attemptIndex) {
  const MotorDriveMode order[4] = {
    baseMode,
  ...
  if (baseMode == MOTOR_MODE_FULLSTEP_BIPOLAR) {
    const MotorDriveMode alt[4] = {
      MOTOR_MODE_FULLSTEP_BIPOLAR,
      MOTOR_MODE_HALFSTEP_UNIPOLAR,
      MOTOR_MODE_HALFSTEP_BIPOLAR,
      MOTOR_MODE_FULLSTEP_UNIPOLAR
    };
  ...
  while (n < 4) unique[n++] = MOTOR_MODE_HALFSTEP_UNIPOLAR;
  if (attemptIndex < 4) return unique[attemptIndex];
  return unique[3];
}

// Utility: auto motor advance mode attempt.
bool autoMotorAdvanceModeAttempt() {
  if (autoMotor.modeAttemptIndex >= 3) return false;

  autoMotor.modeAttemptIndex++;
  MotorDriveMode nextMode = getMotorModeAttemptByIndex(autoMotor.originalMode, autoMotor.modeAttemptIndex);

  setMotorDriveMode(nextMode, true);
  ...
  Serial.print(F("AUTOMOTOR RETRY STEPPERMODE="));
  Serial.print((int)motorDriveMode);
  Serial.print(F(" ["));
  Serial.print(getMotorDriveModeName(motorDriveMode));
  Serial.println(F("]"));
