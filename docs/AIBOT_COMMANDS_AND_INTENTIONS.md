...
All AiBot commands use the same SynROV application protocol as Web and ROS. Natural-language words are interpreted in the operator visual frame before the numeric command is produced; the protocol itself never has an AiBot-specific sign convention.
...
| home position | `home` | Cancels active Manipulator autonomy and returns to the V1 initial pose `180,150,70,90,95,130,0`. |
...
| toggle camera | `camera_toggle` | Toggles the Vehicle camera state: yellow indicator, camera channel and Processing camera mode together. |
...
| toggle camera | `camera_toggle` | Toggles the Drone camera state: yellow indicator, camera channel and Processing camera mode together. |
...
| return home | `return_home` | Returns to the Processing-local origin using local navigation pose and telemetry heading. |
