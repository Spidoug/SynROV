...
// Builds the canonical HEX wire frame for both blocking and queued TX.
...
void handleSerial0HexFrame(uint8_t type, const uint8_t* payload, uint16_t len) {
  ...
  // 0x15 — Vehicle stream: [seq(4 LE) left(i8) right(i8) pan(i8) tilt(i8) flags(1)]
  if (type == 0x15 && len >= 9) {
    ...
    vehicleCameraPanDeg = (int)roundf(clampValue((float)(int8_t)payload[base + 2],
                                                  (float)CAMERA_PAN_MIN_DEG,
                                                  (float)CAMERA_PAN_MAX_DEG));
    vehicleCameraTiltDeg = (int)roundf(clampValue((float)(int8_t)payload[base + 3],
                                                   (float)CAMERA_TILT_MIN_DEG,
                                                   (float)CAMERA_TILT_MAX_DEG));
  ...
  // 0x16 — Drone stream: [seq(4 LE) throttle yaw pitch roll strafe fwd flags pan tilt]
  if (type == 0x16 && len >= 13) {
    ...
    droneCameraPanDeg = constrain((int8_t)payload[base + 7], (int)CAMERA_PAN_MIN_DEG, (int)CAMERA_PAN_MAX_DEG);
    droneCameraTiltDeg = constrain((int8_t)payload[base + 8], (int)CAMERA_TILT_MIN_DEG, (int)CAMERA_TILT_MAX_DEG);
...
void sendTelemetryHex() {
  ...
  if (robotControlMode == ROBOT_MODE_MANIPULATOR) {
    ...
    pushI8h(payload, idx, (int8_t)constrain(vehicleCameraPanDeg, (int)CAMERA_PAN_MIN_DEG, (int)CAMERA_PAN_MAX_DEG));
    pushI8h(payload, idx, (int8_t)constrain(vehicleCameraTiltDeg, (int)CAMERA_TILT_MIN_DEG, (int)CAMERA_TILT_MAX_DEG));
    // Downward sonar range in centimeters. The host maps this value to drone_scan_cm and sonar_down_cm.
    pushI16h(payload, idx, (int16_t)tc.sonarCm);
    pushI16h(payload, idx, (int16_t)tc.ina1Current);
    pushI16h(payload, idx, (int16_t)tc.ina2Current);
    ...
    pushI8h(payload, idx, (int8_t)constrain(droneCameraPanDeg, (int)CAMERA_PAN_MIN_DEG, (int)CAMERA_PAN_MAX_DEG));
    pushI8h(payload, idx, (int8_t)constrain(droneCameraTiltDeg, (int)CAMERA_TILT_MIN_DEG, (int)CAMERA_TILT_MAX_DEG));
  }
...
void sendHexBootSnapshot() {
  ...
  if (robotControlMode == ROBOT_MODE_VEHICLE) {
    ...
    sendHexTextLine(String(F("vehicle_motor_max_speed_mps=")) + String(vehicleControlProfile.maxSpeedMps, 3));
    ...
    sendHexTextLine(String(F("vehicle_camera_pan_min_deg="))   + String(CAMERA_PAN_MIN_DEG, 2));
    sendHexTextLine(String(F("vehicle_camera_pan_max_deg="))   + String(CAMERA_PAN_MAX_DEG, 2));
    sendHexTextLine(String(F("vehicle_camera_tilt_min_deg="))  + String(CAMERA_TILT_MIN_DEG, 2));
    sendHexTextLine(String(F("vehicle_camera_tilt_max_deg="))  + String(CAMERA_TILT_MAX_DEG, 2));
  ...
  if (robotControlMode == ROBOT_MODE_DRONE) {
    ...
    sendHexTextLine(String(F("drone_esc_max_motor_rpm=")) + String(droneControlProfile.maxMotorRpm, 1));
...
void printMotorDiagnosticPanel() {
  ...
  Serial.print(F("PIDSTOP=")); Serial.println((float)motorDcPidStopBandDeg, 3);
...
void updateMotorControl() {
  ...
  // half of one raw ADC code after filtering.  A 0.85-code threshold could
  // accept roughly half of one raw ADC count of final error.
  float dcHoldBand = max((float)motorDcPidStopBandDeg,
  ...
  // PIDSTOP/PIDEXIT tuning, but both technologies share the target tracker.
  ...
  if (motorType == MOTOR_TYPE_DC_MOTOR) {
    ...
    // Keep the PID mathematics in the sensor/reference coordinate system and
    // invert only the DC actuator command. This avoids destabilizing the error
    // and derivative signs while giving the requested reversed motor polarity.
    bool pidLogicalForward = (pidEffort > 0.0f);
    motor.directionForward = motorDcPidOutputInverted
      ? !pidLogicalForward : pidLogicalForward;
