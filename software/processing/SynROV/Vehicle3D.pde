...
final float VEH_DEFAULT_CAMERA_TILT_DEG = 0.0f;
...
final float VEH_DEFAULT_MAX_SPEED_MPS = 1.16f;
float vehicleMaxSpeedMps = VEH_DEFAULT_MAX_SPEED_MPS;
...
final float VEH_CAMERA_PAN_LIMIT = 60.0f;
final float VEH_CAMERA_TILT_MIN = -20.0f;
final float VEH_CAMERA_TILT_MAX = 20.0f;
...
float vehicleMaxSceneStep60Hz() {
  return vehicleMaxSpeedMps * sceneUnitsPerMeterForRobotMode(ROBOT_MODE_VEHICLE) / 60.0f;
}

float vehicleTrackCommandAfterDeadband(float command) {
  float normalized = constrain(command, -1.0f, 1.0f);
  float pct = abs(normalized) * 100.0f;
  float deadband = constrain(getSensorFloat("vehicle_motor_deadband_pct", 5.0f), 0.0f, 40.0f);
  return pct < deadband ? 0.0f : normalized;
}

float vehicleTrackCommandToSpeedMps(float command) {
  return vehicleTrackCommandAfterDeadband(command) * vehicleMaxSpeedMps;
}

// Mirrors the firmware's PWM mapping so the UI reports electrical command
// power rather than simply echoing the joystick percentage.
float vehicleTrackCommandToPowerPct(float command) {
  float normalized = vehicleTrackCommandAfterDeadband(command);
  if (abs(normalized) < 0.0001f) return 0.0f;
  float pct = abs(normalized) * 100.0f;
  float pwmMin = constrain(getSensorFloat("vehicle_motor_pwm_min", 36.0f), 0.0f, 255.0f);
  float pwmMax = constrain(getSensorFloat("vehicle_motor_pwm_max", 255.0f), pwmMin, 255.0f);
  float pwm = map(pct, 0.0f, 100.0f, pwmMin, pwmMax);
  return constrain(pwm / 255.0f * 100.0f, 0.0f, 100.0f);
}

float vehicleCameraPanMinDegLimit() { return -VEH_CAMERA_PAN_LIMIT; }
float vehicleCameraPanMaxDegLimit() { return VEH_CAMERA_PAN_LIMIT; }
float vehicleCameraTiltMinDegLimit() { return VEH_CAMERA_TILT_MIN; }
float vehicleCameraTiltMaxDegLimit() { return VEH_CAMERA_TILT_MAX; }
...
void applyDefaultVehicleConfigState() {
  ...
  vehicleNavZ = 0.0f;
  vehicleNavYaw = 0.0f;
  vehicleCameraPanDeg = 0.0f;
  vehicleCameraTiltDeg = 0.0f;
  vehicleMaxSpeedMps = VEH_DEFAULT_MAX_SPEED_MPS;
}
...
JSONObject buildVehicleConfigObject() {
  ...
  motion.setFloat("maxSpeedMps", vehicleMaxSpeedMps);
...
void applyVehicleVisualOffsetsJson(JSONObject offsets) {
  ...
  VEH_CAMERA_DEFAULT_PAN_DEG = constrain(getJsonFloat(offsets, "cameraDefaultPanDeg", VEH_CAMERA_DEFAULT_PAN_DEG), vehicleCameraPanMinDegLimit(), vehicleCameraPanMaxDegLimit());
  VEH_CAMERA_DEFAULT_TILT_DEG = constrain(getJsonFloat(offsets, "cameraDefaultTiltDeg", VEH_CAMERA_DEFAULT_TILT_DEG), vehicleCameraTiltMinDegLimit(), vehicleCameraTiltMaxDegLimit());
...
boolean isVehicleConfigSchemaValid(JSONObject json) {
  if (!isRobotConfigHeader(json, "Vehicle")) return false;
  ...
  if (motion == null || !motion.hasKey("maxSpeedMps")) return false;

  return hasJsonObjects(dims, "body", "track", "rollers");
}

// Loads vehicle config.
void loadVehicleConfig(String filename) {
  JSONObject defaults = buildVehicleConfigObject();
  JSONObject json = loadJsonOrNull(filename);
  if (json == null || !isVehicleConfigSchemaValid(json)) {
    json = defaults;
  ...
  JSONObject motion = getJsonObjectSafe(json, "motion");
  vehicleMaxSpeedMps = constrain(getJsonFloat(motion, "maxSpeedMps", VEH_DEFAULT_MAX_SPEED_MPS), 0.05f, 20.0f);
  ...
  // Camera command state is intentionally never restored from a prior run.
  // Vehicle always starts centered; telemetry or operator input may move it afterwards.
  vehicleCameraPanDeg = constrain(VEH_DEFAULT_CAMERA_PAN_DEG, vehicleCameraPanMinDegLimit(), vehicleCameraPanMaxDegLimit());
  vehicleCameraTiltDeg = constrain(VEH_DEFAULT_CAMERA_TILT_DEG, vehicleCameraTiltMinDegLimit(), vehicleCameraTiltMaxDegLimit());
}

// Loads or creates the vehicle config.
void loadOrCreateVehicleConfig() {
  if (!fileExists(VEHICLE_CONFIG_FILE)) {
    createDefaultVehicleConfig(VEHICLE_CONFIG_FILE);
  }
  loadVehicleConfig(VEHICLE_CONFIG_FILE);
}
...
void applyVehicleTelemetryGeometryFromSensors() {
  ...
  VEH_CAMERA_DEFAULT_PAN_DEG = constrain(getSensorFloat("vehicle_camera_default_pan_deg", VEH_CAMERA_DEFAULT_PAN_DEG), vehicleCameraPanMinDegLimit(), vehicleCameraPanMaxDegLimit());
  VEH_CAMERA_DEFAULT_TILT_DEG = constrain(getSensorFloat("vehicle_camera_default_tilt_deg", VEH_CAMERA_DEFAULT_TILT_DEG), vehicleCameraTiltMinDegLimit(), vehicleCameraTiltMaxDegLimit());
  ...
  vehicleMaxSpeedMps = constrain(getSensorFloat("vehicle_motor_max_speed_mps", vehicleMaxSpeedMps), 0.05f, 20.0f);
}
...
boolean vehicleRuntimeHasTrackMotion() {
  return abs(vehicleTrackCommandAfterDeadband(vehicleLeftTrackCmd)) > 0.0001f || abs(vehicleTrackCommandAfterDeadband(vehicleRightTrackCmd)) > 0.0001f;
}

// Checks the commanded vehicle motion against the shared environment guard.
boolean vehicleRuntimeMotionBlockedByCollision() {
  if (!environmentCollisionActiveForRobotMode(ROBOT_MODE_VEHICLE) || vehicleEmergencyStopLatched()) return false;
  if (!vehicleRuntimeHasTrackMotion()) return false;

  float leftCmd = vehicleTrackCommandAfterDeadband(vehicleLeftTrackCmd);
  float rightCmd = vehicleTrackCommandAfterDeadband(vehicleRightTrackCmd);
  float avgHardware = (leftCmd + rightCmd) * 0.5f;
  float avg = -avgHardware;
  float diff = (rightCmd - leftCmd) * 0.5f;
  ...
  float trackGaugeM = max(0.05f, VEH_TRACK_GAUGE / 100.0f);
  float navYawStep = diff * (2.0f * vehicleMaxSpeedMps / trackGaugeM) / 60.0f;
  float navForwardStep = avg * vehicleMaxSceneStep60Hz() * tractionFactor;
  ...
  return !vehicleEnvironmentMoveAllowed(
    vehicleNavX, vehicleNavZ, vehicleNavYaw,
    proposedX, proposedZ, proposedYaw
  );
}

// Returns the collision-filtered left track command used by physics and hardware.
float vehicleEffectiveLeftTrackCommand() {
  return vehicleRuntimeMotionBlockedByCollision() ? 0.0f : vehicleTrackCommandAfterDeadband(vehicleLeftTrackCmd);
}

// Returns the collision-filtered right track command used by physics and hardware.
float vehicleEffectiveRightTrackCommand() {
  return vehicleRuntimeMotionBlockedByCollision() ? 0.0f : vehicleTrackCommandAfterDeadband(vehicleRightTrackCmd);
}
...
void notifyVehicleRuntimeCollisionBlock() {
  ...
  updateMessage(tr("Vehicle motion blocked by the collision guard."));
}
...
void updateVehiclePhysics() {
  ...
  float tractionFactor = 1.0f - 0.22f * turnBlend;
  float trackGaugeM = max(0.05f, VEH_TRACK_GAUGE / 100.0f);
  float yawRateRadPerSec = diff * (2.0f * vehicleMaxSpeedMps / trackGaugeM);
  float navYawStep = yawRateRadPerSec * dt;
  float navForwardStep = avg * vehicleMaxSceneStep60Hz() * dt * 60.0f * tractionFactor;
  ...
  if (vehicleEnvironmentMoveAllowed(
      vehicleNavX, vehicleNavZ, vehicleNavYaw,
      proposedX, proposedZ, proposedYaw)) {
    vehicleNavYaw = proposedYaw;
    vehicleNavX = proposedX;
    vehicleNavZ = proposedZ;
  } else if (vehicleEnvironmentMoveAllowed(
      vehicleNavX, vehicleNavZ, vehicleNavYaw,
      vehicleNavX, vehicleNavZ, proposedYaw)) {
    // A pure rotation is allowed when it increases clearance, even while the
    // current pose is already intersecting the collision envelope.
    vehicleNavYaw = proposedYaw;
  }
  ...
  boolean firmwareAttitude = robotUsesFirmwareAttitude(ROBOT_MODE_VEHICLE);
  float targetPitchDeg = firmwareAttitude ? 0.0f : constrain(-avg * 4.2f, -5.0f, 5.0f);
  float targetRollDeg = firmwareAttitude ? 0.0f : constrain(diff * 4.6f, -4.5f, 4.5f);
  ...
  if (isLocalTelemetrySelected() || !systemReady || simulationMode) {
...
void drawVehicle3D(boolean advanceCamera) {
  beginRobotScene3D(ROBOT_MODE_VEHICLE, advanceCamera);

  drawVehicleGround();
  drawRobotAttitudeSupportSurface(ROBOT_MODE_VEHICLE);
  applySceneLighting();
  drawCurrentEnvironmentMap();
  drawVehicleRadarProbe();
  drawRobotFrontCameraPanel(ROBOT_MODE_VEHICLE);
  noStroke();
  ...
  scale(VEHICLE_VISUAL_RENDER_SCALE * largeWorldRobotPresentationScale(ROBOT_MODE_VEHICLE));
  float vehicleRenderYaw = VEHICLE_RENDER_ANCHORED ? 0.0f : vehicleNavYaw;
  rotateY(vehicleRenderYaw + radians(vehicleVisualYawDeg));
  rotateZ(radians(vehicleBaseAttitudeRollDeg + vehicleVisualRollDeg));
  rotateX(radians(vehicleBaseAttitudePitchDeg + vehicleVisualPitchDeg));
  ...
  rotateX(radians(constrain(vehicleCameraTiltDeg, vehicleCameraTiltMinDegLimit(), vehicleCameraTiltMaxDegLimit())));
  fill(100, 100, 200);
  sphere(5);
  pushMatrix();
  translate(0, 0, 4.5);
  fill(isRobotCameraViewEnabled(ROBOT_MODE_VEHICLE) ? color(255, 220, 0) : color(80, 80, 70));
  sphere(1);
  ...
  noLights();
  endRobotScene3D("Vehicle3D.pde:scene");
}
...
boolean moveVehicleSafelyTowardFrame(int[] targetFrame, int maxStep, int tolerance) {
  ...
  vehicleCameraPanDeg = constrain(approachFloat(vehicleCameraPanDeg, targetFrame[2], maxStep), vehicleCameraPanMinDegLimit(), vehicleCameraPanMaxDegLimit());
  vehicleCameraTiltDeg = constrain(approachFloat(vehicleCameraTiltDeg, targetFrame[3], maxStep), vehicleCameraTiltMinDegLimit(), vehicleCameraTiltMaxDegLimit());
...
boolean handleVehicleDiagnosticsPanelMousePressed(int mx, int my, int panelX, int panelY, int contentX, int contentW, int gap, int toolsInnerX, int toolsCol2W) {
  ...
  if (pointInRect(mx, my, toolsInnerX + toolsCol2W + gap, diagnosticsRuntimeButtonRowY(panelY, 1), toolsCol2W, DIAGNOSTICS_BUTTON_H)) { diagnosticsTakeLocalControl(); try { toggleVehicleCamera(); } finally { diagnosticsEndLocalControl(); } return true; }
...
void drawVehicleDiagnosticsPanelSections(int panelY, int contentX, int contentW, int gap, int toolsInnerX, int toolsCol2W, int sensorsContentX, int sensorsContentW, int vehicleRuntimeCardY, int vehicleSensorsCardY) {
  drawSectionCard2D(contentX, vehicleRuntimeCardY, contentW, DIAGNOSTICS_VEHICLE_RUNTIME_CARD_H, tr("Vehicle runtime"));
  drawMiniButton2D(toolsInnerX, diagnosticsRuntimeButtonRowY(panelY, 0), toolsCol2W, DIAGNOSTICS_BUTTON_H, tr("Emergency stop"), false);
  drawMiniButton2D(toolsInnerX + toolsCol2W + gap, diagnosticsRuntimeButtonRowY(panelY, 0), toolsCol2W, DIAGNOSTICS_BUTTON_H, tr("Lights"), vehicleLightsEnabled);
  drawMiniButton2D(toolsInnerX, diagnosticsRuntimeButtonRowY(panelY, 1), toolsCol2W, DIAGNOSTICS_BUTTON_H, tr("LiDAR scan"), vehicleLidarScanEnabled);
  drawMiniButton2D(toolsInnerX + toolsCol2W + gap, diagnosticsRuntimeButtonRowY(panelY, 1), toolsCol2W, DIAGNOSTICS_BUTTON_H, tr("Camera"), isRobotCameraViewEnabled(ROBOT_MODE_VEHICLE));
  ...
  drawSectionCard2D(contentX, vehicleSensorsCardY, contentW, DIAGNOSTICS_VEHICLE_SENSORS_CARD_H, tr("Vehicle stability sensors"));
  ...
  drawDiagnosticsFittedTextLeft(tr("Position: firmware/internal odometry by compass"), sensorsContentX, vehicleInfoY + 18, sensorsContentW);
  drawPressureBar2D(sensorsContentX, vehicleInfoY + 42, sensorsContentW, 18, vehicleBatteryPct, 100, tr("Battery"));
  drawDiagnosticsFittedTextLeft(tr("Battery raw / World: ") + nf(getSensorFloat("battery_raw", getSensorFloat("battery", 0)), 1, 0) + " / " + currentEnvironmentWorldSummary(), sensorsContentX, vehicleInfoY + 70, sensorsContentW);
}
