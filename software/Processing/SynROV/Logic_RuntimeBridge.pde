...
final String[] SERIAL_ASCII_CFG_KEYS = {
  ...
  "SENSINT:", "ROBOT:", "ROBOTTYPE:", "MODE:", "MOTORTYPE=", "MOTORTYPE:",
  "MOTORTYPE_NAME:", "STEPPERMODE=", "STEPPERMODE:", "STEPPERMODE_NAME:",
  "MINSTEPUS=", "MAXSTEPUS=", "STOPENTER=", "STOPEXIT=", "PWMFAR=", "PWMNEAR=", "PWMHOLD=",
  "KP=", "KI=", "KD=", "ZEROADC=", "SPANRAW=", "ALPHAANG="
};
...
final String[] SERIAL_ASCII_SENS_KEYS = {
  ...
  "SONAR:", "INA1:", "INA2:", "STEP:", "MOTOR:", "TGT:", "SPD:", "PWMRAW:", "WIN:",
...
int effectiveManipulatorTelemetrySuppressMs() {
  ...
  if (shouldUseExtendedFirmwareTelemetryTransition()) suppressMs = max(suppressMs, 220);
  return suppressMs;
}
...
// Returns whether the latest sensor snapshot includes PWM feedback.
boolean hasCurrentManipulatorPwmTelemetry() {
  if (latestSensors == null) return false;
  for (int i = 0; i < 4; i++) {
    if (!latestSensors.hasKey("pwm_" + (12 + i))) return false;
  }
  return true;
}

// Returns whether an explicit CH12..CH15 flush is pending.
boolean shouldIncludeManipulatorPwmCommand() {
  // Pose packets must not carry PWM by default. CH12..CH15 are manual/force
  // outputs and are sent with dedicated PWM frames only when their own command
  // target changes. This prevents arm member motion from modulating CH12..CH15.
  return pwmNeedsFlush;
}

// Ensures an ARMJ value array always carries the 4 PWM fields for CH12..CH15.
// When 11 values are already present this helper preserves them; callers that
// are only transporting a pose must use ensureManipulatorArmjValuesWithCommandPwm()
// so member movement cannot overwrite the auxiliary PWM targets.
...
// Ensures an ARMJ transport packet carries the current commanded PWM shadow.
// This is the safety boundary that keeps CH12..CH15 unchanged during member
// motion; the PWM shadow itself is changed only by setDuty() or Auto Torque.
int[] ensureManipulatorArmjValuesWithCommandPwm(int[] vals) {
  int[] out = ensureManipulatorArmjValuesWithPwm(vals);
...
// Formats a pose transport packet while freezing CH12..CH15 at the current
// commanded PWM targets.
String formatManipulatorArmjCommand11WithCommandPwm(int[] vals) {
  int[] full = ensureManipulatorArmjValuesWithCommandPwm(vals);
  if (full == null) return "";
  return "ARMJ=" +
    full[0] + "," + full[1] + "," + full[2] + "," +
    full[3] + "," + full[4] + "," + full[5] + "," + full[6] + "," +
...
// Canonicalizes raw ARMJ transport to pose-only form unless PWM is being sent
// by the dedicated PWM path. Manual PWM changes must go through setDuty(); Auto
// Torque writes the PWM targets from its own sensor loop.
String canonicalizeManipulatorArmjCommand11(String cmd) {
  int[] vals = parseManipulatorArmjValues(cmd);
  if (vals == null) return cmd;
  String fixed = formatManipulatorArmjCommand7(vals);
  return fixed.length() > 0 ? fixed : cmd;
}
...
boolean flushManipulatorPwmTargets(String context, boolean echoToMonitor) {
  ...
  if (shouldUseBinaryRuntimeCommands()) {
    ...
    // ASCII fallback has no independent PWM runtime packet, so keep the flush
    // pending. The normal HEX runtime path above is the authoritative path.
    sentAll = false;
  }
...
// Utility: distance from one local manipulator pose to a reference pose.
int manipulatorPoseDistanceToReference(int[] pose, int[] referencePose) {
  if (pose == null || referencePose == null) return 2147483647;
  int n = min(pose.length, min(referencePose.length, GRIPPER_IDX + 1));
  int total = 0;
  for (int i = 0; i < n; i++) {
    int current = normalizeManipulatorTargetValue(i, pose[i]);
...
// Utility: checks whether a candidate pose moves closer to a reference pose.
boolean manipulatorPoseCloserToReference(int[] previousPose, int[] candidatePose, int[] referencePose, int minProgressDeg) {
  int previousDistance = manipulatorPoseDistanceToReference(previousPose, referencePose);
  int candidateDistance = manipulatorPoseDistanceToReference(candidatePose, referencePose);
  return candidateDistance + max(0, minProgressDeg) < previousDistance;
}

// Utility: evaluate local collision for an arbitrary manipulator pose.
boolean manipulatorPoseHasLocalCollision(int[] pose, boolean includeEnvironment) {
  if (pose == null) return false;
  int[] savedAngles = angles.clone();
  boolean savedForceLocalPose = forceLocalManipulatorPoseForCollision;

  for (int i = 0; i < min(angles.length, pose.length); i++) {
    angles[i] = normalizeManipulatorTargetValue(i, pose[i]);
  }

  forceLocalManipulatorPoseForCollision = true;
  collectColliders();
  boolean collided = checkCollisions();
  if (!collided && includeEnvironment && environmentCollisionEnabled) {
    collided = checkEnvironmentCollisionForCurrentPose();
  }

  arrayCopy(savedAngles, angles);
  forceLocalManipulatorPoseForCollision = savedForceLocalPose;
  return collided;
}
...
boolean applyManipulatorPoseTargets(int[] requestedAngles, boolean[] activeMask) {
  ...
  if (blocked && homingStepActive && localCollisionFilteringActive) {
    int[] candidateAngles = angles.clone();
    boolean previousWasColliding = manipulatorPoseHasLocalCollision(previousAngles, environmentCollisionEnabled);
    boolean recoveryProgress = manipulatorPoseCloserToReference(previousAngles, candidateAngles, homingPositions, 1);

    // Match the firmware recovery behavior: once the pose is already in
    // collision, do not trap HOME behind the same collision state. Allow only
    // coherent HOME steps that reduce the distance to the configured HOME pose.
    if (previousWasColliding && recoveryProgress) {
      blocked = false;
      blockedBySelfCollision = false;
      blockedByEnvironmentCollision = false;
    }
  }
...
void setDuty(int channel, int percentage) {
  ...
  if (idx >= 0 && systemReady && !simulationMode && myPort != null && shouldUseBinaryRuntimeCommands()) {
...
void serviceRuntimeConnectionKeepAlive() {
  if (!(systemReady && !simulationMode) || myPort == null) return;
  if (!isHexCommunicationSelected() || !hexRuntimeReady) return;
...
String canonicalizeRuntimeCommandForAsciiTransport(String normalized) {
  ...
  if (upper.startsWith("ARMJ=")) {
    return canonicalizeManipulatorArmjCommand11(cmd);
  }
  return cmd;
}
...
void goHome() {
  ...
  try {
    ...
    // Apply HOME as one coherent pose step. Applying the joints one-by-one made
    // the local collision guard evaluate temporary partial poses that the real
    // manipulator would not intentionally hold. That forced users to raise the
    // HOME tolerance to ~20 degrees just to escape the homing gate in simulation.
    for (int i = 0; i < manipCount; i++) {
      requestedAngles[i] = smoothManipulatorHomeTarget(i, angles[i], homingPositions[i], HOMING_SMOOTH_FACTOR);
      activeMask[i] = true;
    }
    applyManipulatorPoseTargets(requestedAngles, activeMask);
...
boolean manipulatorTelemetryCloseToPose(int[] referencePose, int toleranceDeg) {
  ...
  for (int i = 0; i < n; i++) {
    if (abs(hardwareAngles[i] - referencePose[i]) > toleranceDeg) return false;
  }
  return true;
}
...
boolean manipulatorLocalPoseCloseToPose(int[] referencePose, int toleranceDeg) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (abs(current - target) > toleranceDeg) return false;
  }
  return true;
}
...
void applyManipulatorSensorState() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    boolean shouldSyncInitialHardwarePwm = hasPwmTelemetry && !dutyInitialHardwareSyncDone[i] && channelHasNoLocalOverride &&
      !manipulatorAutoPwmPowerControlEnabled();
    boolean shouldMirrorHardware = hasPwmTelemetry && diagnosticsActiveSlider < 0 && dutyPendingAckPercentages[i] < 0 &&
      dutyTargetsFollowHardware && manipulatorAutoPwmPowerControlEnabled();

    if (shouldSyncInitialHardwarePwm || shouldMirrorHardware) {
