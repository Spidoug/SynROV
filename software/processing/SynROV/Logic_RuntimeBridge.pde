...
void goHome() {
  ...
  try {
    int manipCount = min(homingPositions.length, min(angles.length, GRIPPER_IDX + 1));
    ...
    for (int i = 0; i < manipCount; i++) {
      ...
      requestedAngles[i] = smoothManipulatorHomeTarget(i, commandReference, homingPositions[i], HOMING_SMOOTH_FACTOR);
      activeMask[i] = true;
    }
    applyManipulatorPoseTargets(requestedAngles, activeMask);

    boolean allJointsAtHome = manipulatorLocalPoseCloseToPose(homingPositions, ANGLE_TOLERANCE);
    ...
    boolean homeConfirmedByTelemetry = simulationMode ||
      ...
      manipulatorTelemetryCloseToPose(homingPositions, max(ANGLE_TOLERANCE, 4));

    if (allJointsAtHome && homeConfirmedByTelemetry) {
      int[] finalHomePose = angles.clone();
      boolean[] finalMask = new boolean[GRIPPER_IDX + 1];
      for (int i = 0; i < manipCount; i++) {
        finalHomePose[i] = normalizeManipulatorTargetValue(i, homingPositions[i]);
        finalMask[i] = true;
      }
...
void updatePendingAutoHome() {
  ...
  pendingAutoHomeStartMs = 0;
  goingHome = true;
  homingCommandSource = CONTROL_SOURCE_LOCAL;
  markManipulatorPoseDirty();
