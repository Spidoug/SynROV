...
class SynROVBridge:
    ...
    def manip_home(self) -> None:
        # HOME is asynchronous. Processing owns the physical trajectory and
        # telemetry remains the only source of measured servo state.
    ...
    def drone_flight_status(self) -> Dict[str, Any]:
        ...
        if phase == "landing":
            ready = False
        airborne = bool(flight.get("airborne", altitude > 3.0)) or altitude > 3.0
        if not phase:
    ...
    def send_drone(
        ...
        *,
        ...
        local_flight = dict(flight)
        if bounded_throttle > 0.02 and status["phase"] == "grounded":
            local_flight["phase"] = "taking_off"
            local_flight["flightReady"] = True
        if filtered_by_flight_state:
            local_flight["flightStateFiltered"] = True
        if cam_pan is not None:
            flight["camPan"] = clamp_pan(cam_pan)
            local_flight["camPan"] = flight["camPan"]
        if cam_tilt is not None:
            flight["camTilt"] = clamp_tilt(cam_tilt)
            local_flight["camTilt"] = flight["camTilt"]
        self._merge_control_state("Drone", "flight", local_flight)
        camera_values: Dict[str, Any] = {}
        if cam_pan is not None:
    ...
    def drone_takeoff(self) -> None:
        ...
        if not status["flight_ready"]:
            self._merge_control_state("Drone", "flight", {
                "airborne": bool(status["airborne"]), "flightReady": True, "phase": "taking_off",
    ...
    def set_camera_enabled(self, robot: Any, enabled: bool) -> None:
        self.set_camera(robot, enabled=bool(enabled))

    def set_camera(self, robot: Any, *, pan: Optional[float] = None, tilt: Optional[float] = None, enabled: Optional[bool] = None) -> None:
        name = canonical_robot(robot)
        if name == "Manipulator":
            return
        self.state.control["robot"] = name

        camera_payload: Dict[str, Any] = {"robot": name}
        camera_state: Dict[str, Any] = {}
        if pan is not None:
            camera_payload["pan"] = clamp_pan(pan)
            camera_state["pan"] = camera_payload["pan"]
        if tilt is not None:
            camera_payload["tilt"] = clamp_tilt(tilt)
            camera_state["tilt"] = camera_payload["tilt"]
        if enabled is not None:
            camera_payload["enabled"] = bool(enabled)
            camera_state["enabled"] = bool(enabled)
        if len(camera_payload) == 1:
            return

        self._merge_camera_state(**camera_state)
        if name == "Vehicle":
            drive = {}
            if "pan" in camera_payload:
                drive["camPan"] = camera_payload["pan"]
            if "tilt" in camera_payload:
                drive["camTilt"] = camera_payload["tilt"]
            if drive:
                self._merge_control_state(name, "drive", drive)
        else:
            flight = {}
            if "pan" in camera_payload:
                flight["camPan"] = camera_payload["pan"]
            if "tilt" in camera_payload:
                flight["camTilt"] = camera_payload["tilt"]
            if flight:
                self._merge_control_state(name, "flight", flight)
        self.send_control({"camera": camera_payload}, immediate=True)
