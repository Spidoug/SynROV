...
static final long COLLISION_REFRESH_INTERVAL_MS = 66;
long lastCollisionRefreshMs = 0;


final int SERIAL_ASCII_LINE_MAX = 4096;
final int SERIAL_ASCII_RESYNC_GRACE_MS = 1200;
final int PENDING_APP_SERIAL_LINE_LIMIT = 96;
final int PENDING_APP_TELEMETRY_COALESCE_LIMIT = 12;
...
final String[] SERIAL_ASCII_FRAME_MARKERS = {
  ...
  "MOTORTYPE=", "MOTORTYPE:", "STEPPERMODE=", "STEPPERMODE:",
  "SYNROV", "[MOTOR BASE]", "============================================================"
};
...
void resetPendingManipulatorPoseCommand() {
  ...
  for (int i = 0; i < lastSentManipulatorPose.length && i < manipulatorCommandAnglesDeg.length; i++) {
    lastSentManipulatorPose[i] = getManipulatorCommandTargetInt(i);
  }
...
int effectiveManipulatorTelemetrySuppressMs() {
  return max(manipulatorPoseSendIntervalMs, max(0, manipulatorTelemetrySuppressMs));
}
...
// Returns the PWM command value that must be embedded in ARMJ.
//
// ARMJ is a transport packet: when only joint members change, CH12..CH15 must
// stay equal to the last commanded PWM targets. Do not source these 4 fields
// from live telemetry, because feedback can change while the arm is moving and
// would make the base PWM on CH12/pin 46 vary during member-only commands.
...
// Ensures an explicitly authorized ARMJ value array carries the 4 PWM fields
// for CH12..CH15. Seven-value inputs receive the current commanded PWM shadow;
// eleven-value inputs preserve their explicitly supplied PWM values.
int[] ensureManipulatorArmjValuesWithPwm(int[] vals) {
  if (vals == null) return null;
  if (vals.length == 11) {
    int[] out = new int[11];
    for (int i = 0; i < 7; i++) out[i] = vals[i];
    for (int i = 0; i < 4; i++) out[7 + i] = constrain(vals[7 + i], 0, 100);
    return out;
  }
  if (vals.length != 7) return null;
  int[] out = new int[11];
  for (int i = 0; i < 7; i++) out[i] = vals[i];
  for (int i = 0; i < 4; i++) out[7 + i] = getManipulatorDutyCommandPercentage(i);
  return out;
}

// Formats a pose-only ARMJ command. These 7 values update only the arm joints;
// CH12..CH15 are intentionally omitted so PWM cannot be changed by member motion.
String formatManipulatorArmjCommand7(int[] vals) {
  if (vals == null || vals.length < 7) return "";
  return "ARMJ=" +
    vals[0] + "," + vals[1] + "," + vals[2] + "," +
    vals[3] + "," + vals[4] + "," + vals[5] + "," + vals[6];
}

// Formats an explicitly authorized ARMJ packet with the 4 PWM fields included.
String formatManipulatorArmjCommand11(int[] vals) {
  int[] full = ensureManipulatorArmjValuesWithPwm(vals);
  if (full == null) return "";
  return "ARMJ=" +
...
String buildManipulatorPoseCommand() {
  int baseMember = manipulatorNeutralMembers[BASE_IDX] ? 1000 : round(normalizeAbsoluteAngleDeg(baseServoToForwardYawDeg(getManipulatorCommandTargetDeg(BASE_IDX))));
  int upperMember = manipulatorNeutralMembers[UPPERARM_IDX] ? 1000 : round(upperServoToGroundDeg(getManipulatorCommandTargetDeg(UPPERARM_IDX)));
  int foreMember = manipulatorNeutralMembers[FOREARM_IDX] ? 1000 : round(forearmServoToDeltaDeg(getManipulatorCommandTargetDeg(FOREARM_IDX)));
  int forearmRollMember = manipulatorNeutralMembers[FOREARM_ROLL_IDX] ? 1000 : round(rollServoToDeltaDeg(getManipulatorCommandTargetDeg(FOREARM_ROLL_IDX)));
  int wristPitchMember = manipulatorNeutralMembers[WRIST_VERT_IDX] ? 1000 : round(wristServoToDeltaDeg(getManipulatorCommandTargetDeg(WRIST_VERT_IDX)));
  int wristRollMember = manipulatorNeutralMembers[WRIST_ROT_IDX] ? 1000 : round(rollServoToDeltaDeg(getManipulatorCommandTargetDeg(WRIST_ROT_IDX)));
  int gripMember = manipulatorNeutralMembers[GRIPPER_IDX] ? 1000 : getManipulatorCommandTargetInt(GRIPPER_IDX);

  int[] values = new int[] {
    baseMember, upperMember, foreMember,
    forearmRollMember, wristPitchMember, wristRollMember, gripMember
  };
  return formatManipulatorArmjCommand7(values);
}
...
// Sends pending CH12..CH15 PWM command targets through the dedicated PWM command path.
boolean flushManipulatorPwmTargets(String context, boolean echoToMonitor) {
  if (!pwmNeedsFlush) return true;
  if (!(systemReady && !simulationMode) || myPort == null) return false;

  boolean sentAll = true;
  for (int i = 0; i < 4; i++) {
    int channel = 12 + i;
    int pct = getManipulatorDutyCommandPercentage(i);
    boolean sent = sendManipulatorPwmBinaryCommand(channel, pct, context, echoToMonitor);
    sentAll = sentAll && sent;
    if (sent) {
      dutyPendingAckPercentages[i] = pct;
      dutyPendingAckUntilMs[i] = millis() + DUTY_PENDING_ACK_HOLD_MS;
    }
  }

  if (sentAll) pwmNeedsFlush = false;
  return sentAll;
}
...
void flushPendingManipulatorPoseCommand(boolean force) {
  ...
  if (pendingHardwarePoseSync) {
    ...
    } else if (!force && !manipulatorPoseDirty && !pwmNeedsFlush) {
      return;
    }
  }
  ...
  // Normal firmware collision authority suppresses idle pose re-sends, but HOME
  // is an explicit finite operation and must keep its final target alive until
  // telemetry confirms arrival. Without this, a temporarily rejected/dropped HOME
  // packet can leave local-collision activation waiting forever, most visibly on CH0.
  boolean allowArrivalResend = !firmwareAuthority || goingHome;
  boolean resendPending = allowArrivalResend &&
    !manipulatorPoseDirty &&
    manipulatorPoseResendIntervalMs > 0 &&
    lastHardwareSensorMillis > 0 &&
    !manipulatorTelemetryCloseToCommandedPose(MANIPULATOR_RESEND_POSITION_TOLERANCE_DEG) &&
  ...
  // settling.
  boolean pwmFlushPending = pwmNeedsFlush;
  if (!manipulatorPoseDirty && !force && !resendPending && !pwmFlushPending) return;

  if (pwmFlushPending && !manipulatorPoseDirty && !force && !resendPending) {
    flushManipulatorPwmTargets("manipulator PWM flush", false);
    return;
  }
...
void setManipulatorMemberNeutral(int memberIdx, boolean neutral) {
  ...
  queueManipulatorPoseCommand();
  if (systemReady && myPort != null && isManipulatorSelected && !isRemoteControlSource(commandContextSource)) {
...
// Canonical manipulator command-reference helpers.
// All input sources write the desired pose here; hardwareAngles is feedback.
float getManipulatorCommandTargetDeg(int servoIndex) {
  if (servoIndex < 0 || servoIndex >= manipulatorCommandAnglesDeg.length) return 0.0f;
  return constrain(manipulatorCommandAnglesDeg[servoIndex],
                   (float)servoLimits[servoIndex][0],
                   (float)servoLimits[servoIndex][1]);
}

int getManipulatorCommandTargetInt(int servoIndex) {
  return normalizeManipulatorTargetValue(servoIndex, round(getManipulatorCommandTargetDeg(servoIndex)));
}

void setManipulatorCommandTargetDeg(int servoIndex, float value) {
  if (servoIndex < 0 || servoIndex >= manipulatorCommandAnglesDeg.length) return;
  manipulatorCommandAnglesDeg[servoIndex] = constrain(value,
    (float)servoLimits[servoIndex][0], (float)servoLimits[servoIndex][1]);
}

int[] copyManipulatorCommandPose() {
  int count = min(GRIPPER_IDX + 1, manipulatorCommandAnglesDeg.length);
  int[] pose = new int[count];
  for (int i = 0; i < count; i++) pose[i] = getManipulatorCommandTargetInt(i);
  return pose;
}

void syncManipulatorCommandPoseFromHardware() {
  int count = min(manipulatorCommandAnglesDeg.length, hardwareAngles.length);
  for (int i = 0; i < count; i++) setManipulatorCommandTargetDeg(i, hardwareAngles[i]);
}
...
// Utility: smooth manipulator target with guaranteed convergence.
//
// The normal exponential smoother can stall before the target because of integer
// rounding. With HOMING_SMOOTH_FACTOR = 0.05, any remaining error below about
// 10 degrees rounds to a zero-degree step. That made HOME finish only when the
// HOME angle tolerance was raised to 10+ degrees. HOME needs the same smooth
// behavior for large moves, but it must still take at least one degree per tick
// until the configured HOME pose is actually reached.
int smoothManipulatorHomeTarget(int servoIndex, int current, int target, float factor) {
  factor = constrain(factor, 0.0f, 1.0f);
  int boundedCurrent = normalizeManipulatorTargetValue(servoIndex, current);
  int boundedTarget = normalizeManipulatorTargetValue(servoIndex, target);
  int delta = servoIndex == BASE_IDX
    ? shortestBaseDeltaDeg(boundedCurrent, boundedTarget)
    : boundedTarget - boundedCurrent;

  if (delta == 0) return boundedTarget;

  int stepSize = max(1, round(abs(delta) * factor));
  if (abs(delta) <= stepSize) return boundedTarget;

  return normalizeManipulatorTargetValue(servoIndex, boundedCurrent + (delta > 0 ? stepSize : -stepSize));
}
...
boolean ensureManipulatorPoseSynchronizedBeforeCommand(int[] requestedAngles, boolean[] activeMask, int manipCount) {
  ...
  if (lastHardwareSensorMillis <= 0) {
    updateMessage(tr("Waiting for first manipulator telemetry before sending arm command..."));
    return false;
  }
...
// Clears the latched local-collision motion stop.
void clearManipulatorCollisionMotionHold() {
  manipulatorCollisionHoldLatched = false;
  for (int i = 0; i < manipulatorCollisionBlockedDirection.length; i++) {
    manipulatorCollisionBlockedDirection[i] = 0;
  }
}

// Returns the signed movement direction used by the collision hold. The base is
// circular, so crossing 359 -> 0 is treated as a short positive move.
int manipulatorCollisionMoveDirection(int memberIndex, int fromDeg, int toDeg) {
  float delta = memberIndex == BASE_IDX
    ? normalizeSignedAngleDeg(toDeg - fromDeg)
    : float(toDeg - fromDeg);
  if (abs(delta) < 0.5f) return 0;
  return delta > 0.0f ? 1 : -1;
}

// While collision stop is latched, reject a repeated command only when every
// active change continues in the same blocked direction. Any opposite/different
// move is evaluated normally so the operator can move away from the collision.
boolean manipulatorCollisionHoldRejectsRequest(int[] requestedAngles, boolean[] activeMask, int manipCount) {
  if (!manipulatorCollisionHoldLatched || !isManipulatorLocalCollisionGuardBlockingActive()) return false;

  boolean hasMotion = false;
  boolean hasEscapeCandidate = false;
  for (int i = 0; i < manipCount; i++) {
    if (!activeMask[i]) continue;
    int currentTarget = getManipulatorCommandTargetInt(i);
    int requestedTarget = normalizeManipulatorTargetValue(i, requestedAngles[i]);
    int direction = manipulatorCollisionMoveDirection(i, currentTarget, requestedTarget);
    if (direction == 0) continue;
    hasMotion = true;

    int blockedDirection = i < manipulatorCollisionBlockedDirection.length
      ? manipulatorCollisionBlockedDirection[i]
      : 0;
    if (blockedDirection == 0 || direction != blockedDirection) {
      hasEscapeCandidate = true;
      break;
    }
  }
  return hasMotion && !hasEscapeCandidate;
}

// Stops the real arm at its latest measured pose after a local collision
// prediction. This is sent only on the first block in a collision episode; held
// pro-collision input is then rejected by the latch instead of re-commanding the
// arm every frame and creating oscillation.
void latchManipulatorCollisionMotionHold(
  int[] previousAngles,
  float[] previousCommandAngles,
  boolean[] previousNeutralMembers,
  int[] candidateAngles,
  boolean[] activeMask,
  int manipCount
) {
  boolean firstLatch = !manipulatorCollisionHoldLatched;

  for (int i = 0; i < manipCount && i < manipulatorCollisionBlockedDirection.length; i++) {
    if (!activeMask[i]) continue;
    int from = normalizeManipulatorTargetValue(i, round(previousCommandAngles[i]));
    int direction = manipulatorCollisionMoveDirection(i, from, candidateAngles[i]);
    if (direction != 0) manipulatorCollisionBlockedDirection[i] = direction;
  }
  manipulatorCollisionHoldLatched = true;

  arrayCopy(previousCommandAngles, manipulatorCommandAnglesDeg);
  arrayCopy(previousNeutralMembers, manipulatorNeutralMembers);
  arrayCopy(previousAngles, angles);

  if (!firstLatch) return;

  boolean useMeasuredHold = systemReady && !simulationMode && myPort != null &&
    isManipulatorTelemetryFreshForVisualPose();
  for (int i = 0; i <= GRIPPER_IDX && i < manipulatorCommandAnglesDeg.length; i++) {
    if (i < manipulatorNeutralMembers.length && manipulatorNeutralMembers[i]) continue;
    int holdTarget = useMeasuredHold && i < hardwareAngles.length
      ? normalizeManipulatorTargetValue(i, hardwareAngles[i])
      : normalizeManipulatorTargetValue(i, round(previousCommandAngles[i]));
    setManipulatorCommandTargetDeg(i, holdTarget);
  }
  ...
  if (systemReady && !simulationMode && myPort != null) {
    ...
    for (int i = 0; i <= GRIPPER_IDX && i < manipulatorCommandAnglesDeg.length; i++) {
      if (i < manipulatorNeutralMembers.length && manipulatorNeutralMembers[i]) continue;
      sendServoStatePacket(mapIndexToChannel(i), getManipulatorCommandTargetInt(i));
    }
  }
...
boolean applyManipulatorPoseTargets(int[] requestedAngles, boolean[] activeMask) {
  ...
  if (manipulatorCollisionHoldRejectsRequest(requestedAngles, activeMask, manipCount)) return false;

  int[] previousAngles = angles.clone();
  int[] candidateAngles = previousAngles.clone();
  float[] previousCommandAngles = manipulatorCommandAnglesDeg.clone();
  boolean[] previousNeutralMembers = manipulatorNeutralMembers.clone();
  boolean changed = false;
  boolean[] changedMask = new boolean[GRIPPER_IDX + 1];

  // Candidate collision pose starts from the complete commanded pose, not from
  // measured feedback. This keeps one coherent desired pose while the hardware
  // is still traveling toward earlier commands.
  for (int i = 0; i < manipCount; i++) {
    candidateAngles[i] = getManipulatorCommandTargetInt(i);
  }

  for (int i = 0; i < manipCount; i++) {
    if (!activeMask[i]) continue;

    // activeMask means this joint has an explicit target from the current input
    // source. Therefore a numeric WebSocket target must be allowed to release a
    // member that was previously in neutral; otherwise the HTML control appears
    // connected but cannot take command of that joint again.
    int normalizedTarget = normalizeManipulatorTargetValue(i, requestedAngles[i]);
    candidateAngles[i] = normalizedTarget;
    if (manipulatorNeutralMembers[i]) {
      manipulatorNeutralMembers[i] = false;
      changed = true;
      changedMask[i] = true;
    }
    if (getManipulatorCommandTargetInt(i) != normalizedTarget) {
      setManipulatorCommandTargetDeg(i, normalizedTarget);
      changed = true;
      changedMask[i] = true;
    }
  }
  ...
  boolean localCollisionFilteringActive = isManipulatorLocalCollisionGuardBlockingActive() && !isManipulatorFirmwareCollisionAuthorityActive();

  if (localCollisionFilteringActive) {
    arrayCopy(candidateAngles, angles);
    blocked = evaluateManipulatorSelfCollisionForLocalPose();
    if (!blocked && environmentCollisionActiveForRobotMode(ROBOT_MODE_MANIPULATOR)) {
      invalidateEnvCollisionCache();
      blocked = evaluateManipulatorEnvironmentCollisionForLocalPose();
    }
    arrayCopy(previousAngles, angles);
    invalidateEnvCollisionCache();
  }
  ...
  if (blocked) {
    // Freeze at the measured pose once, then latch the blocked direction. This
    // prevents held keyboard/joystick/web input from repeatedly pushing into
    // the same collision boundary and producing arm jitter.
    latchManipulatorCollisionMotionHold(
      previousAngles, previousCommandAngles, previousNeutralMembers,
      candidateAngles, activeMask, manipCount
    );
    return false;
  }

  // Firmware telemetry owns angles[] only while Firmware is the selected
  // telemetry source. In Local telemetry, the commanded pose must remain the
  // visual authority even with a live serial link; otherwise the real arm moves
  // while the 3D model stays frozen at the previous feedback pose.
  if (simulationMode || myPort == null || !systemReady || isLocalTelemetrySelected()) {
    for (int i = 0; i < manipCount; i++) {
      if (activeMask[i]) angles[i] = getManipulatorCommandTargetInt(i);
    }
  } else {
    arrayCopy(previousAngles, angles);
  }
  ...
  if (systemReady && myPort != null) {
    if (!isManipulatorSelected) selectMode("Manipulator");
    queueManipulatorPoseCommand();
  }
  for (int i = 0; i < manipCount; i++) {
    if (changedMask[i]) sendServoStatePacket(mapIndexToChannel(i), getManipulatorCommandTargetInt(i));
  }
  clearManipulatorCollisionMotionHold();
  lastServoCommandTime = millis();
  return true;
}
...
void setDuty(int channel, int percentage) {
  ...
  if (channel < 12 || channel > 15) {
    updateMessage(tr("Invalid duty channel ") + channel + tr(" (expected 12-15)"));
    return;
  }
  ...
  if (idx >= 0) {
    if (manipulatorAutoPwmPowerControlEnabled()) {
      ...
      resetManipulatorAutoPwmBaselines();
    }
    dutyTargetPercentages[idx] = percentage;
    // Cmd is the slider/host request. Live remains Arduino telemetry only.
    dutyPendingAckPercentages[idx] = percentage;
    dutyPendingAckUntilMs[idx] = millis() + DUTY_PENDING_ACK_HOLD_MS;
    pwmNeedsFlush = true;
  }

  boolean directPwmSent = false;
  if (idx >= 0 && systemReady && !simulationMode && myPort != null) {
    directPwmSent = sendManipulatorPwmBinaryCommand(channel, percentage, "manipulator PWM", false);
    if (directPwmSent) pwmNeedsFlush = false;
  }

  if (isManipulatorSelected && !directPwmSent) {
    queueManipulatorPoseCommand();
    if (!isRemoteControlSource(commandContextSource)) {
      flushPendingManipulatorPoseCommand(false);
    }
  } else if (!isManipulatorSelected) {
    updateMessage(tr("Aux PWM sent directly to the manipulator firmware."));
  }
...
boolean writeBinaryRuntimeBytes(Serial port, byte[] payload, String context, boolean echoToMonitor) {
  ...
  try {
    ...
    updateMessage(context + tr(" failed: ") + detail);
...
boolean sendBinaryRuntimeFrame(int type, byte[] payload, String context, boolean echoToMonitor) {
  ...
  if (port == null) {
    updateMessage(tr("HEX runtime bridge not connected."));
    return false;
  }
...
void markHexRuntimeReady(String sourceLabel) {
  ...
  if (wasReady) return;
  updateMessage(tr("HEX runtime ready on Serial0") + (sourceLabel != null && sourceLabel.length() > 0 ? " (" + sourceLabel + ")" : "") + tr(". Waiting for boot snapshot."));
  requestAutoHomeAfterConnect();
  sendSystemStatus();
}
...
// Sends one manipulator PWM percentage command.
boolean sendManipulatorPwmBinaryCommand(int channel, int percentage, String context, boolean echoToMonitor) {
  if (channel < 12 || channel > 15) return false;
  percentage = constrain(percentage, 0, 100);
  return sendBinaryRuntimeFrame(0x11, new byte[] {
    (byte)(channel & 0xFF),
    (byte)(percentage & 0xFF)
...
boolean sendManipulatorArmjBinaryCommand(int[] vals, String context, boolean echoToMonitor) {
  if (vals == null || vals.length < 7) {
    updateMessage(tr("Invalid ARMJ command for HEX runtime."));
    return false;
  }

  long seq = nextHardwareStreamSequence();
  // Pose-only len=18: [seq 4] [7×i16 joints 14]. CH12..CH15 are deliberately
  // excluded from normal position packets and are updated by 0x11 PWM frames.
...
boolean sendHexRuntimeStreamCommand(String body, String context, boolean echoToMonitor) {
  if (!hexRuntimeReady) {
    ...
    updateMessage(tr("HEX runtime bridge not ready yet."));
    return false;
  }
...
// Sends a lightweight host keepalive so the firmware can distinguish an idle
// connected app from a lost/disconnected host link.
void serviceRuntimeConnectionKeepAlive() {
  if (!(systemReady && !simulationMode) || myPort == null) return;
  if (!hexRuntimeReady) return;
  if (calibrationWindowVisible || serialMonitorSessionActive || hardwareStreamStoppedByExit) return;

  long now = millis();
  if (lastHardwareStreamTxMillis > 0 && (now - lastHardwareStreamTxMillis) < HARDWARE_STREAM_KEEPALIVE_MS) return;
  sendHexRuntimeStreamCommand("PING", "runtime keepalive", false);
}
...
boolean sendHardwareStreamCommand(String body, String context, boolean echoToMonitor) {
  if (!canAcceptCommandSource(commandContextSource, true)) return false;
  if (!((systemReady && !simulationMode) && myPort != null)) {
    updateMessage(tr("Hardware not ready or port closed"));
    return false;
  }
...
// Starts HOME from the one version-1 pose and removes pending target producers
// that could overwrite it on a later frame. With fresh hardware telemetry, HOME
// starts from the measured pose rather than a stale command reference.
void beginManipulatorHome(int source, boolean synchronizeFromHardware) {
  cancelManipulatorTargetAutomation();
  cancelPendingAutoHome();
  pendingAutoHomeStartMs = 0;
  clearManipulatorCollisionMotionHold();

  if (synchronizeFromHardware && systemReady && !simulationMode && myPort != null && lastHardwareSensorMillis > 0) {
    syncManipulatorCommandPoseFromHardware();
  }

  goingHome = true;
  homingCommandSource = source;
...
void goHome() {
  ...
  final int ANGLE_TOLERANCE = max(1, manipulatorHomeAngleToleranceDeg);
  ...
  try {
    int manipCount = min(MANIPULATOR_HOME_POSE.length, min(angles.length, GRIPPER_IDX + 1));
    int[] requestedAngles = copyManipulatorCommandPose();
    boolean[] activeMask = new boolean[GRIPPER_IDX + 1];

    // Apply HOME as one coherent commanded pose. The next HOME increment is based
    // on the previous command reference, not measured feedback. Feedback remains
    // authoritative only for the completion test below. This is essential for CH0:
    // a DC base must keep receiving a progressing target even while the potentiometer
    // is still catching up with the first increment.
    for (int i = 0; i < manipCount; i++) {
      int commandReference = getManipulatorCommandTargetInt(i);
      requestedAngles[i] = smoothManipulatorHomeTarget(i, commandReference, MANIPULATOR_HOME_POSE[i], HOMING_SMOOTH_FACTOR);
      activeMask[i] = true;
    }
    applyManipulatorPoseTargets(requestedAngles, activeMask);

    boolean allJointsAtHome = manipulatorLocalPoseCloseToPose(MANIPULATOR_HOME_POSE, ANGLE_TOLERANCE);
    boolean homingTimedOutForCollision = pendingManipulatorLocalCollisionHomeTimedOut();
    boolean homeConfirmedByTelemetry = simulationMode ||
      !isManipulatorFirmwareTelemetryAuthorityActive() ||
      manipulatorTelemetryCloseToPose(MANIPULATOR_HOME_POSE, max(ANGLE_TOLERANCE, 4));

    if (allJointsAtHome && homeConfirmedByTelemetry) {
      int[] finalHomePose = angles.clone();
      boolean[] finalMask = new boolean[GRIPPER_IDX + 1];
      for (int i = 0; i < manipCount; i++) {
        finalHomePose[i] = normalizeManipulatorTargetValue(i, MANIPULATOR_HOME_POSE[i]);
        finalMask[i] = true;
      }
      applyManipulatorPoseTargets(finalHomePose, finalMask);

      goingHome = false;
      updateMessage(tr("Homing done."));
      completePendingManipulatorLocalCollisionGuardAfterHome();
    } else if (allJointsAtHome && homingTimedOutForCollision) {
      // Do not treat a timeout as homing success. Keep retrying the HOME pose
      // until firmware telemetry confirms the hardware reached it. This avoids
      // enabling the local collision guard over a stale Processing-only pose.
      pendingManipulatorLocalCollisionHomeStartMs = millis();
      markManipulatorPoseDirty();
      updateMessage(tr("Homing waiting for telemetry confirmation; local guard not enabled yet."));
    }
  }
  finally {
    commandContextSource = previousSource;
...
void requestAutoHomeAfterConnect() {
  ...
  if (!isManipulatorSelected && !"Manipulator".equals(detectedRobotType)) {
    cancelPendingAutoHome();
    return;
  }
  ...
  updateMessage(tr("Manipulator homing scheduled..."));
}
...
void updatePendingAutoHome() {
  ...
  if (manipulatorAutoPwmPowerControlEnabled() && !simulationMode && lastHardwareSensorMillis <= 0) return;
  if (millis() < pendingAutoHomeStartMs) return;

  beginManipulatorHome(CONTROL_SOURCE_LOCAL, true);
  updateMessage(tr("Manipulator auto homing..."));
}
...
void beginPortOpen(String portName, int index, boolean fromLastSavedPort) {
  ...
  updateMessage(tr("Opening port: ") + portName);
...
boolean pollPendingPortOpen() {
  ...
  if (!activePortOpenWorker.finished) {
    if (millis() - activePortOpenStartedAt > serialOpenTimeoutMs + serialBootGraceMs + serialReadyHandshakeTimeoutMs + 1000) {
      ...
      updateMessage(tr("Timed out probing port: ") + portName);
      activePortOpenWorker = null;
      activePortOpenStartedAt = 0;
      activeProbePortName = "";
      if (wasLastSaved) {
  ...
  updateMessage(tr("Failed to probe port: ") + portName + " (" + reason + ")");
  testPort = null;
  if (wasLastSaved) {
    initializationStep = 2;
  } else {
...
// Clears transient hardware telemetry/session timing in one place so connect,
// disconnect and full runtime reset cannot drift into different states.
void resetHardwareTelemetrySessionState() {
  lastHardwareSensorMillis = 0;
...
void resetConnectionRuntimeState() {
  ...
  resetHardwareTelemetrySessionState();
  ...
  for (int i = 0; i < dutyPendingAckPercentages.length; i++) {
    dutyPendingAckPercentages[i] = -1;
    dutyPendingAckUntilMs[i] = 0;
  }
  dutyHardwareSnapshotReceived = false;
  pwmNeedsFlush = false;
...
void connect() {
  ...
  if (initializationStep != 0) {
    ...
    updateMessage(tr("Disconnected. System reset."));
    cancelActivePortOpen();
    ...
    updateMessage(tr("Starting connection..."));
...
void activateSimulationFromConnectButton() {
  ...
  serialBannerDetected = false;
  simulationMode();
  updateMessage(tr("Simulation Mode Enabled (double-click Connect)"));
}


// Returns the current serial list without letting a driver/backend failure
// turn a hardware change into a startup NullPointerException.
String[] safeSerialPortList() {
  try {
    String[] found = Serial.list();
    return found == null ? new String[0] : found;
  }
  catch (Throwable t) {
    println("[SynROV][Serial] port enumeration failed: " + (t.getMessage() == null ? t.toString() : t.getMessage()));
    return new String[0];
  }
}
...
void checkLastPort() {
  ...
  updateMessage(tr("Checking last used port..."));
  String lastPort = loadLastPort();
  ports = safeSerialPortList();

  if (lastPort != null) {
    for (int i = 0; i < ports.length; i++) {
      if (ports[i].equals(lastPort)) {
        updateMessage(tr("Attempting to reconnect to last port: ") + lastPort);
        try {
          beginPortOpen(lastPort, i, true);
          return;
        }
        catch (Exception e) {
          updateMessage(tr("Auto-reconnect failed. Testing others"));
        }
      }
    }
  } else {
    updateMessage(tr("No last port saved or found"));
  }
...
void listSerialPorts() {
  ...
  updateMessage(tr("Searching for serial ports..."));
  ports = safeSerialPortList();
  updateMessage(" " + ports.length + tr(" ports found."));
  startWait(500);
  initializationStep = 3;
}
...
void testSerialPorts() {
  ...
  if (portIndex < ports.length) {
    updateMessage(tr("Testing port: ") + ports[portIndex]);
    try {
      beginPortOpen(ports[portIndex], portIndex, false);
    }
    catch (Exception e) {
      updateMessage(tr("Failed to schedule port test: ") + ports[portIndex] + tr(". Trying next..."));
      portIndex++;
      startWait(150);
    }
  } else {
    initializationStep = 6;
    updateMessage(tr("No current SynROV Serial0 port found"));
  }
}
...
void finishSerialConnection(Serial confirmedPort, boolean requestRuntimeAfterConnect) {
  ...
  if (ports != null && portIndex >= 0 && portIndex < ports.length) {
    ...
    updateMessage(tr("Serial connected: ") + ports[portIndex] + " [" + robotDisplayName(detectedRobotType) + "] [ASCII boot / HEX runtime]");
  } else {
    updateMessage(tr("Serial connected [") + robotDisplayName(detectedRobotType) + "] [ASCII boot / HEX runtime]");
  }
  ...
  resetPendingVehicleRuntimeCommand();
  resetPendingDroneRuntimeCommand();
  pendingHardwarePoseSync = "Manipulator".equals(detectedRobotType);
  ...
  try {
    ...
    syncLatestSensorsForActiveTelemetrySource();
    updateMessage(tr("Connection sync recovered from null runtime state."));
  }

  if (requestRuntimeAfterConnect) {
    updateMessage(tr("Boot snapshot captured. Opening HEX runtime on Serial0..."));
...
void handleMainSerialDisconnect(String detail) {
  ...
  cancelPendingAutoHome();
  goingHome = false;
  appendCalibrationSerialLine(tr("[ERROR] Serial0 connection lost") + suffix);
  updateMessage(tr("Serial0 disconnected") + suffix);
  closeSerialPort(myPort, "myPort");
  myPort = null;
  systemReady = false;
  sentReadyRequest = false;
...
void handleProbeSerialFailure(String detail) {
  String suffix = (detail == null || trim(detail).length() == 0) ? "" : (": " + trim(detail));
  updateMessage(tr("Probe failed on candidate port") + suffix);
  closeSerialPort(testPort, "testPort");
  testPort = null;
  activeProbePortName = "";
  sentReadyRequest = false;
...
void servicePendingReadyHandshake() {
  ...
  if (pendingReadyHandshakeStage == READY_HANDSHAKE_STAGE_WAIT_RUNTIME) {
    if (sentReadyRequest && readyRequestTime > 0 && (now - readyRequestTime) >= serialReadyHandshakeTimeoutMs) {
      if (pendingReadyHandshakeAttempts >= READY_HANDSHAKE_MAX_ATTEMPTS) {
        updateMessage(tr("READY? / HEX hello timed out on ") + describeSerialPort(port) + ".");
        cancelPendingReadyHandshake();
        return;
      }
      sentReadyRequest = false;
      ...
      updateMessage(tr("Retrying staged boot handshake on ") + describeSerialPort(port) + "...");
    }
    return;
  }
  ...
  if (pendingReadyHandshakeStage == READY_HANDSHAKE_STAGE_BOOT_REQUEST) {
    if (safeSerialWrite(port, "BOOT?\n", "boot pre-handshake", port == testPort)) {
      ...
      updateMessage(tr("Boot screen requested on ") + describeSerialPort(port) + ".");
    } else {
      cancelPendingReadyHandshake();
    }
    return;
  ...
  if (pendingReadyHandshakeStage == READY_HANDSHAKE_STAGE_READY_REQUEST) {
    if (safeSerialWrite(port, "READY?\n", "ready handshake", port == testPort)) {
      ...
      updateMessage(tr("READY? sent on ") + describeSerialPort(port) + tr(". Sending stable HEX HELLO..."));
    } else {
      cancelPendingReadyHandshake();
    }
    return;
...
void enqueuePendingAppSerialAsciiLine(String line) {
  ...
  synchronized (pendingAppSerialAsciiLines) {
    boolean telemetryLine = line.startsWith("A55A20") || line.startsWith("A55A21");
    if (telemetryLine && pendingAppSerialAsciiLines.size() >= PENDING_APP_TELEMETRY_COALESCE_LIMIT) {
      for (int i = pendingAppSerialAsciiLines.size() - 1; i >= 0; i--) {
        String queued = pendingAppSerialAsciiLines.get(i);
        if (queued != null && (queued.startsWith("A55A20") || queued.startsWith("A55A21"))) {
          pendingAppSerialAsciiLines.remove(i);
        }
      }
    }
    while (pendingAppSerialAsciiLines.size() >= PENDING_APP_SERIAL_LINE_LIMIT) {
      int removeIndex = 0;
      for (int i = 0; i < pendingAppSerialAsciiLines.size(); i++) {
        String queued = pendingAppSerialAsciiLines.get(i);
        if (queued != null && (queued.startsWith("A55A20") || queued.startsWith("A55A21"))) {
          removeIndex = i;
          break;
      ...
      pendingAppSerialAsciiLines.remove(removeIndex);
    }
...
void handleRuntimeBinaryFrame(int type, byte[] payload, int len, String sourceLabel) {
  // Only log non-telemetry frames; 0x20/0x21 are high-rate telemetry frames.
  int t = type & 0xFF;
  if (t != 0x20 && t != 0x21) {
  ...
  if (t == 0x81) {
    ...
    updateMessage(tr("HEX runtime confirmed") + (ackText.length() > 0 ? ": " + ackText : "."));
    return;
  }
  if (t == 0x30) {
    markHexRuntimeReady(sourceLabel);
  ...
  appendCalibrationSerialLine(tr("[INFO] Binary frame received: ") + describeBinaryFrameType(type) + " (" + len + tr(" bytes payload)."));
}
...
// Runtime transport uses newline-terminated ASCII/A55A HEX lines in every phase.
...
void drainBinarySerialPort(Serial port) {
  ...
  try {
    ...
    if (port == calibrationPort) {
      appendCalibrationSerialLine(tr("[ERROR] Dedicated monitor port read failed: ") + detail);
...
void serialEvent(Serial port) {
  ...
  synchronized (serialRxLock) {
    ...
    try {
      ...
      String detail = t.getMessage() == null ? t.toString() : t.getMessage();
      if (port == calibrationPort) {
        appendCalibrationSerialLine(tr("[ERROR] Dedicated monitor port error in serialEvent: ") + detail);
        disconnectCalibrationMonitorPort();
      } else {
        handleMainSerialDisconnect(detail);
      }
...
void simulationMode() {
  ...
  manualRobotModeOverride = false;
  updateMessage(tr("Simulation Mode Enabled"));
  initializationStep = 5;
  beginManipulatorHome(CONTROL_SOURCE_LOCAL, false);
}
...
void parseControlPacket(String raw) {
  ...
  for (String b : blocks) {
    ...
    if (key.equals("CLIENT")) {
      ...
      } else if (value.equalsIgnoreCase("AIBOT") || value.equalsIgnoreCase("AI")) {
        currentControlOwner = CONTROL_SOURCE_AIBOT;
      } else if (value.equalsIgnoreCase("ROS")) {
        currentControlOwner = CONTROL_SOURCE_ROS;
...
void parsePidPacket(String raw) {
  ...
  if (vals.length >= 3) {
    try {
      ...
      if (vals.length >= 5) {
        hardwarePidStopDeg = float(trim(vals[3]));
        hardwarePidExitDeg = float(trim(vals[4]));
      }
      if (vals.length >= 7) {
        hardwareBaseZeroAdc = int(trim(vals[5]));
        hardwareBaseSpanRaw = int(trim(vals[6]));
      }
      if (vals.length >= 8) {
        hardwarePidDirectionInverted = int(trim(vals[7])) != 0;
      }
      sendPidStatePacket();
    }
    catch (Exception e) {
...
JSONObject buildLocalTelemetrySnapshotForRobot(String robotName) {
  ...
  if (normalized.equals("Manipulator")) {
    ...
    sens.setFloat("base_pitch_deg", manipulatorBaseAttitudePitchDeg);
    sens.setFloat("base_roll_deg", manipulatorBaseAttitudeRollDeg);
    ...
    sens.setFloat("vehicle_pitch_deg", vehicleBaseAttitudePitchDeg + vehicleVisualPitchDeg);
    sens.setFloat("vehicle_roll_deg", vehicleBaseAttitudeRollDeg + vehicleVisualRollDeg);
    ...
    sens.setFloat("vehicle_motor_max_speed_mps", vehicleMaxSpeedMps);
    ...
    sens.setFloat("vehicle_camera_pan_min_deg", vehicleCameraPanMinDegLimit());
    sens.setFloat("vehicle_camera_pan_max_deg", vehicleCameraPanMaxDegLimit());
    sens.setFloat("vehicle_camera_tilt_min_deg", vehicleCameraTiltMinDegLimit());
    sens.setFloat("vehicle_camera_tilt_max_deg", vehicleCameraTiltMaxDegLimit());
    ...
    sens.setFloat("heading_deg", normalizeAbsoluteAngleDeg(degrees(droneNavYaw)));
    ...
    sens.setFloat("drone_camera_pan_min_deg", droneCameraPanMinDegLimit());
    sens.setFloat("drone_camera_pan_max_deg", droneCameraPanMaxDegLimit());
    sens.setFloat("drone_camera_tilt_min_deg", droneCameraTiltMinDegLimit());
    sens.setFloat("drone_camera_tilt_max_deg", droneCameraTiltMaxDegLimit());
    ...
    sens.setFloat("drone_esc_max_motor_rpm", droneMaxMotorRpm);
...
JSONObject buildZeroTelemetrySnapshotForRobot(String robotName) {
  ...
  if (normalized.equals("Manipulator")) {
    sens.setFloat("base_deg", MANIPULATOR_HOME_POSE[BASE_IDX]);
    sens.setFloat("base_yaw_deg", baseServoToForwardYawDeg(MANIPULATOR_HOME_POSE[BASE_IDX]));
...
void preserveManipulatorPoseForDisconnect() {
  for (int i = 0; i < min(angles.length, hardwareAngles.length); i++) {
    ...
    setManipulatorCommandTargetDeg(i, preserved);
  }
  resetPendingManipulatorPoseCommand();
  pendingHardwarePoseSync = false;
  manipulatorTelemetrySuppressUntilMs = 0;
...
void resetTelemetryReadoutsForDisconnect() {
  ...
  for (int i = 0; i < dutyPercentages.length; i++) {
    // Hardware is absent: Live is explicitly 0 and Cmd is reset to a safe 0.
    dutyPercentages[i] = 0;
    dutyTargetPercentages[i] = 0;
    if (i < dutyPendingAckPercentages.length) dutyPendingAckPercentages[i] = -1;
    if (i < dutyPendingAckUntilMs.length) dutyPendingAckUntilMs[i] = 0;
  }
  dutyHardwareSnapshotReceived = false;
  pwmNeedsFlush = false;
...
void queueSerialCommand(String command) {
  ...
  if (systemReady && !simulationMode && myPort != null) {
    ...
    updateMessage(tr("Serial0 is not connected"));
  }
}
...
String normalizeRuntimeCommand(String command) {
  ...
  if (upper.startsWith("A55A") || upper.startsWith("FRAME ")) {
    return upper;
  }

  int eq = normalized.indexOf('=');
  if (eq > 0) {
    String key = trim(normalized.substring(0, eq)).toUpperCase();
    String value = trim(normalized.substring(eq + 1));
    if (key.equals("MOTORTYPE") || key.equals("STEPPERMODE")) {
...
// Clears automatic PWM torque references.
void resetManipulatorAutoPwmBaselines() {
  for (int i = 0; i < 4; i++) {
    manipulatorAutoPwmBaselineRaw[i] = -1.0f;
    manipulatorAutoPwmFilteredRaw[i] = 0.0f;
    manipulatorAutoPwmLastDuty[i] = -1;
  }
  manipulatorAutoPwmLastUpdateMs = 0;
}

// Returns the telemetry keys used by each manipulator torque channel.
String[] manipulatorTorqueSensorKeys(int index) {
  if (index == 0) return new String[] {"base_current_ma", "base_torque", "base_torque_raw", "an2", "an_2", "an_1"};
  if (index == 1) return new String[] {"upper_current_ma", "upper_torque", "upper_arm_torque_raw", "an3", "an_3", "an_2"};
  if (index == 2) return new String[] {"fore_current_ma", "forearm_torque", "forearm_torque_raw", "an4", "an_4", "an_3"};
  return new String[] {"forearm_roll_current_ma", "forearm_roll_torque", "forearm_roll_torque_raw", "an5", "an_5", "an_4"};
}

// Reads the latest torque value for one automatic PWM channel.
float readManipulatorTorqueSensorRaw(int index, float fallback) {
  String[] keys = manipulatorTorqueSensorKeys(index);
  if (!hasAnySensorKey(keys)) return fallback;
  return getSensorFloatAny(keys, fallback);
}

// Converts torque response into a holding PWM percentage.
int manipulatorAutoPwmDutyFromTorque(int index, float raw) {
  if (manipulatorAutoPwmBaselineRaw[index] < 0.0f) {
    manipulatorAutoPwmBaselineRaw[index] = raw;
    manipulatorAutoPwmFilteredRaw[index] = raw;
  }
  manipulatorAutoPwmFilteredRaw[index] = lerp(manipulatorAutoPwmFilteredRaw[index], raw, 0.32f);
  float response = abs(manipulatorAutoPwmFilteredRaw[index] - manipulatorAutoPwmBaselineRaw[index]);
  if (response < MANIPULATOR_AUTO_PWM_DEADBAND_RAW) {
    manipulatorAutoPwmBaselineRaw[index] = lerp(manipulatorAutoPwmBaselineRaw[index], manipulatorAutoPwmFilteredRaw[index], 0.02f);
    response = 0.0f;
  }
  float ratio = constrain(response / MANIPULATOR_AUTO_PWM_RANGE_RAW, 0.0f, 1.0f);
  int duty = round(MANIPULATOR_AUTO_PWM_MIN_PCT + ratio * (100 - MANIPULATOR_AUTO_PWM_MIN_PCT));
  if (index < manipulatorNeutralMembers.length && manipulatorNeutralMembers[index]) duty = 0;
  return constrain(duty, 0, 100);
}

// Updates CH12..CH15 from the matching torque sensors while Auto Torque is active.
void updateManipulatorAutoPwmPowerControlFromSensors(boolean force) {
  if (!manipulatorAutoPwmPowerControlEnabled()) return;

  boolean aiBotOwnsTorque = isAiBotControlAuthorityConnected();
  if (aiBotOwnsTorque != aiBotTorqueAuthorityWasConnected) {
    aiBotTorqueAuthorityWasConnected = aiBotOwnsTorque;
    resetManipulatorAutoPwmBaselines();
  }
  // When AiBot is connected it is the sole Auto Torque authority. Processing
  // continues receiving current telemetry, but does not calculate PWM commands.
  if (aiBotOwnsTorque) return;

  if (latestSensors == null || latestSensors.size() == 0) return;
  if (!isManipulatorSelected && !"Manipulator".equals(detectedRobotType)) return;

  long now = millis();
  if (!force && manipulatorAutoPwmLastUpdateMs > 0 && (now - manipulatorAutoPwmLastUpdateMs) < MANIPULATOR_AUTO_PWM_UPDATE_MS) return;
  manipulatorAutoPwmLastUpdateMs = now;

  boolean changed = false;
  for (int i = 0; i < 4; i++) {
    String[] keys = manipulatorTorqueSensorKeys(i);
    if (!hasAnySensorKey(keys)) continue;
    float raw = readManipulatorTorqueSensorRaw(i, manipulatorAutoPwmFilteredRaw[i]);
    int computed = manipulatorAutoPwmDutyFromTorque(i, raw);
    if (!force && manipulatorAutoPwmLastDuty[i] >= 0) {
      computed = round(lerp((float)manipulatorAutoPwmLastDuty[i], (float)computed, 0.45f));
    }
    computed = constrain(computed, 0, 100);
    if (force || manipulatorAutoPwmLastDuty[i] < 0 || abs(computed - dutyTargetPercentages[i]) >= 1) {
      dutyTargetPercentages[i] = computed;
      // Live is updated only by pwm_12..pwm_15 telemetry from the Arduino.
      manipulatorAutoPwmLastDuty[i] = computed;
      changed = true;
    }
  }
  ...
  for (int i = 0; i < dutyChannels.length; i++) {
    sendDutyStatePacket(dutyChannels[i]);
  }
}
...
// Utility: manipulator telemetry close to a reference pose.
boolean manipulatorTelemetryCloseToPose(int[] referencePose, int toleranceDeg) {
  if (referencePose == null || lastHardwareSensorMillis <= 0) return false;
  int n = min(referencePose.length, hardwareAngles.length);
  for (int i = 0; i < n; i++) {
    int target = normalizeManipulatorTargetValue(i, referencePose[i]);
    if (manipulatorAngleErrorDeg(i, hardwareAngles[i], target) > toleranceDeg) return false;
  }
  return true;
}

// Utility: local manipulator pose close to a reference pose.
boolean manipulatorLocalPoseCloseToPose(int[] referencePose, int toleranceDeg) {
  if (referencePose == null) return false;
  int n = min(referencePose.length, angles.length);
  for (int i = 0; i < n; i++) {
    int target = normalizeManipulatorTargetValue(i, referencePose[i]);
    int current = normalizeManipulatorTargetValue(i, angles[i]);
    if (manipulatorAngleErrorDeg(i, current, target) > toleranceDeg) return false;
  }
  return true;
}
...
boolean manipulatorTelemetryCloseToCommandedPose(int toleranceDeg) {
  ...
  for (int i = 0; i < count; i++) {
    if (i < manipulatorNeutralMembers.length && manipulatorNeutralMembers[i]) continue;
    if (manipulatorAngleErrorDeg(i, hardwareAngles[i], getManipulatorCommandTargetInt(i)) > toleranceDeg) return false;
  }
  return true;
}
...
boolean shouldApplyManipulatorTelemetry() {
  return isManipulatorSelected || "Manipulator".equals(detectedRobotType) || hasArmRuntimeTelemetry();
}
...
void applyManipulatorSensorState() {
  ...
  if (pendingHardwarePoseSync) {
    syncManipulatorLocalPoseToHardware(true);
    updateMessage(tr("Manipulator pose synchronized from hardware."));
  }
  ...
  boolean pwmSnapshotPresent = false;
  long pwmNow = millis();
  for (int i = 0; i < 4; i++) {
    boolean hasPwmTelemetry = latestSensors != null && latestSensors.hasKey("pwm_" + (12 + i));
    if (hasPwmTelemetry) {
      int feedbackPct = constrain(round(getSensorFloat("pwm_" + (12 + i), dutyPercentages[i])), 0, 100);
      dutyPercentages[i] = feedbackPct;
      pwmSnapshotPresent = true;

      int pendingPct = (i < dutyPendingAckPercentages.length) ? dutyPendingAckPercentages[i] : -1;
      if (pendingPct >= 0 && (abs(feedbackPct - pendingPct) <= 1 || pwmNow > dutyPendingAckUntilMs[i])) {
        dutyPendingAckPercentages[i] = -1;
        dutyPendingAckUntilMs[i] = 0;
      }
    } else if (!systemReady && !simulationMode) {
      dutyPercentages[i] = 0;
    }
  }
  if (pwmSnapshotPresent) dutyHardwareSnapshotReceived = true;
...
void applyVehicleSensorState() {
  ...
  if (latestSensors.hasKey("vehicle_cam_pan_deg")) vehicleCameraPanDeg = constrain(lerp(vehicleCameraPanDeg, getSensorFloat("vehicle_cam_pan_deg", vehicleCameraPanDeg), 0.28f), vehicleCameraPanMinDegLimit(), vehicleCameraPanMaxDegLimit());
  if (latestSensors.hasKey("vehicle_cam_tilt_deg")) vehicleCameraTiltDeg = constrain(lerp(vehicleCameraTiltDeg, getSensorFloat("vehicle_cam_tilt_deg", vehicleCameraTiltDeg), 0.28f), vehicleCameraTiltMinDegLimit(), vehicleCameraTiltMaxDegLimit());
}
...
void applyDroneSensorState() {
  ...
  if (latestSensors.hasKey("drone_cam_pan_deg")) droneCameraPanDeg = constrain(lerp(droneCameraPanDeg, getSensorFloat("drone_cam_pan_deg", droneCameraPanDeg), 0.28f), droneCameraPanMinDegLimit(), droneCameraPanMaxDegLimit());
  if (latestSensors.hasKey("drone_cam_tilt_deg")) droneCameraTiltDeg = constrain(lerp(droneCameraTiltDeg, getSensorFloat("drone_cam_tilt_deg", droneCameraTiltDeg), 0.28f), droneCameraTiltMinDegLimit(), droneCameraTiltMaxDegLimit());
...
void applySensorState() {
  ...
  if (shouldApplyManipulatorTelemetry()) {
    ...
    updateManipulatorAutoPwmPowerControlFromSensors(false);
  }
}
...
void updateHardwareStreamHealth() {
  ...
  if (!hardwareTelemetryTimeoutLatched && lastHardwareSensorMillis > 0) {
    ...
    updateMessage(tr("Telemetry/stream timeout detected"));
    appendCalibrationSerialLine(tr("[WARN] Telemetry timeout detected"));
...
void processSerialIO() {
  ...
  processPendingAppSerialAsciiLines();
  serviceRuntimeConnectionKeepAlive();
  syncLatestSensorsForActiveTelemetrySource();
  applySensorState();
