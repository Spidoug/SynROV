...
JSONObject buildManipulatorConfigObject() {
  ...
  json.setString("schema", "synrov.robot_config.v1");
  json.setInt("version", SYNROV_CONFIG_FORMAT_VERSION);
...
boolean isManipulatorConfigSchemaValid(JSONObject json) {
  if (!isRobotConfigHeaderV1(json, "Manipulator")) return false;
...
void loadConfig(String filename) {
  ...
  JSONObject json = loadV1JsonOrNull(filename);
...
// Loads or creates the V1 manipulator config.
