...
from .primitives import iter_or_empty, safe_float, to_list
from .perception import pad_float_vector
from .policy import PolicyNormalizer, PolicyOutput
from .protocol import MODEL_SCHEMA, SYNROV_VERSION
from .trainer import TrainingGate, TrainingGateReport


MODEL_POLICY_VERSION = 1
ROBOT_FOREST_POLICY: Dict[str, Dict[str, Any]] = {
    "Manipulator": {"n_estimators": 128, "min_samples_leaf": 2, "max_features": 0.80},
    "Vehicle": {"n_estimators": 144, "min_samples_leaf": 2, "max_features": 0.90},
    "Drone": {"n_estimators": 168, "min_samples_leaf": 3, "max_features": 0.85},
}


def training_sample_weight(row: Dict[str, Any], robot: Any = None) -> float:
    """Return a bounded trust weight for one validated training row.

    Human/teacher samples are deliberately stronger than model-generated data;
    command authority only refines that trust and never lets autonomous samples
    dominate a human demonstration.
    """
    name = canonical_robot(robot or row.get("robot", "Manipulator"))
    quality = safe_float(row.get("quality_score", 0.5), 0.5, 0.0, 1.0)
    source = str(row.get("source", "observe") or "observe").strip().lower()
    confidence = safe_float(row.get("voice_confidence", 0.0), 0.0, 0.0, 1.0)
    command = str(row.get("voice_intent", "") or "").strip().lower()

    # Quality is the common rule. Source class then determines how much a row
    # may influence retraining, preventing feedback loops from autonomous data.
    weight = 0.55 + 0.90 * quality
    if any(token in source for token in ("manual", "operator", "teacher", "guided", "ui", "base", "seed")):
        weight *= 1.35
    elif any(token in source for token in ("voice", "text", "typed")):
        weight *= 1.15 + 0.25 * confidence
    elif any(token in source for token in ("emergency", "safety")):
        weight *= 1.50
    elif any(token in source for token in ("ai", "model", "autonomous", "curiosity", "vision", "music")):
        weight *= 0.58
    else:
        weight *= 0.82

    if command and command != "none":
        authority = safe_float(command_tuning(name, command).authority, 1.0, 0.55, 1.75)
        # Keep tuning subtle: source trust remains the dominant factor.
        weight *= 0.82 + 0.18 * authority
    return safe_float(weight, 1.0, 0.20, 2.50)


def training_weights(rows: Iterable[Dict[str, Any]], robot: Any = None) -> List[float]:
    return [training_sample_weight(row, robot) for row in rows if isinstance(row, dict)]
...
def train_model(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    weights = training_weights(selected, robot)
    forest_policy = dict(ROBOT_FOREST_POLICY.get(robot, ROBOT_FOREST_POLICY["Manipulator"]))
    meta.update({
        "model_policy_version": MODEL_POLICY_VERSION,
        "forest_policy": forest_policy,
        "sample_weight_min": min(weights) if weights else 1.0,
        "sample_weight_max": max(weights) if weights else 1.0,
        "sample_weight_mean": (sum(weights) / len(weights)) if weights else 1.0,
    })
    try:
        ...
        package["model_policy_version"] = MODEL_POLICY_VERSION
    except (ImportError, ModuleNotFoundError) as exc:
        avg = [sum(target[i] * weights[row_i] for row_i, target in enumerate(y)) / max(1e-9, sum(weights)) for i in range(target_dim)]
        package.update({
            "model": None,
            "model_type": "weighted_mean_target",
            "mean_target": avg,
            "warning": str(exc),
            "model_policy_version": MODEL_POLICY_VERSION,
        })
    except (TypeError, ValueError, RuntimeError) as exc:
        # Invalid/unsupported training data uses a deterministic weighted fallback
        # instead of turning one library exception into a runtime-wide failure.
        total_weight = max(1e-9, sum(weights))
        avg = [sum(target[i] * weights[row_i] for row_i, target in enumerate(y)) / total_weight for i in range(target_dim)]
        package.update({
            "model": None,
            "model_type": "weighted_mean_target",
            "mean_target": avg,
