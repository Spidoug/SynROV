// =====================================================================
// SynROV Firmware V1 - Runtime commands and diagnostics
// ---------------------------------------------------------------------
// Purpose:
...
// Reads a stable integer A0 value for calibration/diagnostics.
int16_t readMotorRawADC() {
  // Calibration is performed while the mechanism is stationary. First settle
  // the ADC mux on A0 because telemetry may have just sampled A1..A5. Then use
  // a larger trimmed average so two isolated spikes cannot move a boundary.
  settleMotorFeedbackAdcInput();
  const uint8_t samples = MOTOR_CAL_ADC_SAMPLES;
  long sum = 0;
  int16_t minSample = MOTOR_ADC_MAX;
  int16_t maxSample = MOTOR_ADC_MIN;
  for (uint8_t i = 0; i < samples; i++) {
    int16_t sample = (int16_t)analogRead(MOTOR_FEEDBACK_PIN);
    sum += sample;
  ...
  sum -= minSample;
  sum -= maxSample;
  const long rounded = sum + ((samples - 2) / 2);
  return (int16_t)(rounded / (samples - 2));
}

// Publishes the canonical base-servo controller state for Processing/Web.
void sendPidCalibrationStateLine() {
  Serial.print(F("#PID|"));
  Serial.print(motor.kp, 4); Serial.print(',');
  Serial.print(motor.ki, 4); Serial.print(',');
  Serial.print(motor.kd, 4); Serial.print(',');
  Serial.print((float)motorDcPidStopBandDeg, 3); Serial.print(',');
  Serial.print((float)motorDcPidExitBandDeg, 3); Serial.print(',');
  Serial.print(motorCalibrationZeroADC); Serial.print(',');
  Serial.print(motorCalibrationSpanRaw); Serial.print(',');
  Serial.println(motorDcPidOutputInverted ? 1 : 0);
}
...
void startMotorCalibrationWizard() {
  ...
  Serial.println(F("MOVE TO THE FIRST END OF THE SELECTED ONE-TURN WINDOW AND SEND LIMITMOTORCALIB"));
}
...
void captureMotorCalibrationLimit1() {
  ...
  Serial.println(F("MOVE EXACTLY ONE MECHANICAL TURN TO THE OTHER SELECTED END AND SEND LIMITMOTORCALIB AGAIN"));
}
...
void captureMotorCalibrationLimit2() {
  ...
  if (span < MOTOR_CAL_MIN_SPAN_RAW) {
    Serial.println(F("LIMITMOTORCALIB FAILED - SELECTED ONE-TURN WINDOW IS TOO SMALL"));
    initMotorCalState();
    return;
  }

  // Important for a multi-turn potentiometer: these are ONLY the two ends of
  // the selected mechanical turn. We intentionally do not search for 0 or 1023.
  motorCalibrationZeroADC = low;
  motorCalibrationSpanRaw = high;
  sanitizeMotorRuntimeParams();
  resetMotorFeedbackFilterState();
  readMotorAngle();
  ...
  Serial.print(F("SPANRAW=")); Serial.println(motorCalibrationSpanRaw);
  Serial.print(F("WINDOWADC=")); Serial.print(motorCalibrationZeroADC); Serial.print(F("..")); Serial.println(motorCalibrationSpanRaw);
  Serial.print(F("ADC_WINDOW_USAGE_PCT=")); Serial.println(100.0f * getMotorRawSpan() / 1023.0f, 2);
  Serial.print(F("DEGPERCOUNT=")); Serial.println(getMotorDegPerCount(), 6);
  if (getMotorRawSpan() < MOTOR_CAL_RECOMMENDED_SPAN_RAW) {
    Serial.println(F("WARNING: ONE-TURN WINDOW HAS LESS THAN ~1 RAW ADC COUNT PER LOGICAL DEGREE"));
    Serial.println(F("WARNING: THIS IS A REAL A0 RESOLUTION LIMIT; THE ADC WINDOW WILL NOT BE ARTIFICIALLY ENLARGED"));
  }
  Serial.println(F("LOGICAL_TURN=0..359 (360 POSITIONS, NO WRAP)"));
  sendPidCalibrationStateLine();
...
void printMotorCalibrationStatus() {
  ...
  Serial.print(F("LIMITMOTORCALIB RAWADC_NOW=")); Serial.println(readMotorRawADC());
  if (motorCal.limit1Adc != 0 && motorCal.limit2Adc != 0) {
    int16_t low = min(motorCal.limit1Adc, motorCal.limit2Adc);
    int16_t high = max(motorCal.limit1Adc, motorCal.limit2Adc);
    int32_t candidateSpan = (int32_t)high - (int32_t)low;
    Serial.print(F("LIMITMOTORCALIB CANDIDATE_ZEROADC=")); Serial.println(low);
    Serial.print(F("LIMITMOTORCALIB CANDIDATE_ENDADC=")); Serial.println(high);
    Serial.print(F("LIMITMOTORCALIB CANDIDATE_SPANRAW=")); Serial.println(high);
    Serial.print(F("LIMITMOTORCALIB CANDIDATE_WIDTH=")); Serial.println(candidateSpan);
    Serial.print(F("LIMITMOTORCALIB CANDIDATE_USAGE_PCT=")); Serial.println(100.0f * (float)candidateSpan / 1023.0f, 2);
    if (candidateSpan > 0) {
      Serial.print(F("LIMITMOTORCALIB CANDIDATE_DEGPERCOUNT=")); Serial.println(MOTOR_INPUT_MAX / (float)candidateSpan, 6);
    }
    if (candidateSpan < MOTOR_CAL_RECOMMENDED_SPAN_RAW) {
      Serial.println(F("LIMITMOTORCALIB WARNING: NARROW A0 WINDOW (<~1 RAW COUNT/DEG)"));
    }
  }
}
...
// Utility: returns the number of generated candidates for a stage.
uint8_t autoMotorCandidateCountForStage(AutoMotorStage stage) {
  return (stage == AUTO_STAGE_TORQUE || stage == AUTO_STAGE_BANDS) ? 4 : 0;
}

// Utility: generates one candidate on demand instead of keeping four copies in SRAM.
bool autoMotorBuildCandidate(AutoMotorStage stage, uint8_t index, AutoMotorCandidate& out) {
  ...
  if (stage == AUTO_STAGE_TORQUE) {
    static const int16_t pwmNearDelta[4] PROGMEM = { -24, -8, 12, 28};
    static const int16_t pwmFarDelta[4]  PROGMEM = { -28, -10, 14, 30};
    static const int16_t pwmHoldDelta[4] PROGMEM = { -12, -4, 6, 14};
    static const uint16_t minVals[4] PROGMEM = {2600, 2200, 1800, 1500};
    static const uint16_t maxVals[4] PROGMEM = {9200, 7600, 6200, 5200};
    AutoMotorCandidate c = b;
    const int16_t nearDelta = (int16_t)pgm_read_word(&pwmNearDelta[index]);
    const int16_t farDelta  = (int16_t)pgm_read_word(&pwmFarDelta[index]);
    const int16_t holdDelta = (int16_t)pgm_read_word(&pwmHoldDelta[index]);
    c.pwmNear = (uint16_t)clampValue((int)b.pwmNear + nearDelta, 90, (int)capNear);
    c.pwmFar  = (uint16_t)clampValue((int)b.pwmFar  + farDelta, (int)c.pwmNear, (int)capFar);
    c.pwmHold = (uint16_t)clampValue((int)b.pwmHold + holdDelta, 70, (int)capHold);
    ...
    c.minStepUs = pgm_read_word(&minVals[index]);
    c.maxStepUs = pgm_read_word(&maxVals[index]);
    out = c;
    return true;
  }

  if (stage == AUTO_STAGE_BANDS) {
    ...
    static const int8_t enterDeltaTenths[4] PROGMEM = { -4, -1, 2, 5};
    static const int8_t exitDeltaTenths[4]  PROGMEM = { -7, -2, 4, 9};
    AutoMotorCandidate c = base;
    const float enterDelta = ((int8_t)pgm_read_byte(&enterDeltaTenths[index])) * 0.1f;
    const float exitDelta  = ((int8_t)pgm_read_byte(&exitDeltaTenths[index])) * 0.1f;
    c.stopEnter = clampValue(enterBase + enterDelta, 0.5f, 12.0f);
    c.stopExit = clampValue(exitBase + exitDelta, c.stopEnter + 0.5f, 18.0f);
    out = c;
    return true;
  }
  return false;
...
// Utility: returns the configured test motion without a resident float array.
float autoMotorTestMotionForIndex(AutoMotorStage stage, uint8_t index) {
  if (stage == AUTO_STAGE_TORQUE) {
    static const uint8_t motions[4] PROGMEM = {18, 54, 108, 180};
    return index < 4 ? (float)pgm_read_byte(&motions[index]) : 0.0f;
  }
  if (stage == AUTO_STAGE_BANDS) {
    static const uint8_t motions[3] PROGMEM = {36, 120, 220};
    return index < 3 ? (float)pgm_read_byte(&motions[index]) : 0.0f;
  }
  return 0.0f;
}
...
void autoMotorPrepareStage(AutoMotorStage stage) {
  ...
  if (stage == AUTO_STAGE_TORQUE) autoMotor.motionCount = 4;
  else if (stage == AUTO_STAGE_BANDS) autoMotor.motionCount = 3;
  else autoMotor.motionCount = 0;
...
void autoMotorStart() {
  ...
  if (getMotorRawSpan() < MOTOR_CAL_MIN_SPAN_RAW) {
...
void autoMotorFinalize() {
  ...
  if (autoMotor.best.valid) {
    ...
    configSessionDirty = true;
    Serial.println(F("BEST PARAMETERS ARE READY IN CFG - USE SHOW TO CHECK AND SAVE TO STORE EEPROM"));
  } else {
    setMotorDriveMode(autoMotor.originalMode, false);
    autoMotorApplyCandidate(autoMotor.seed);
    Serial.println(F("AUTOMOTOR FAILED - SEED RESTORED"));
    Serial.println(F("CFG MODE REMAINS ACTIVE - CHECK VALUES, SAVE IF DESIRED, OR EXIT"));
  }
  releaseMotorCoils();
  motorSetDriverPWM(0);
  motorPulse.setSpeed(0.0f);
...
void printAutoMotorStatus() {
  ...
  Serial.print(F("PIDSTOP=")); Serial.println(motorDcPidStopBandDeg, 3);
  Serial.print(F("PIDEXIT=")); Serial.println(motorDcPidExitBandDeg, 3);
  Serial.print(F("PIDDIR=")); Serial.println(motorDcPidOutputInverted ? F("INVERTED") : F("NORMAL"));
  Serial.print(F("PWMFAR=")); Serial.println(movePwmFar);
  Serial.print(F("PWMNEAR=")); Serial.println(movePwmNear);
  Serial.print(F("PWMHOLD=")); Serial.println(holdPwm);
...
void autoMotorUpdate() {
  ...
  if (autoMotor.cachedStage != autoMotor.stage) {
    ...
    autoMotor.candidateCount = autoMotorCandidateCountForStage(autoMotor.stage);
    autoMotor.candidateIndex = 0;
    autoMotor.motionIndex = 0;
    if (autoMotor.candidateCount > 0 && autoMotorBuildCandidate(autoMotor.stage, 0, autoMotor.current)) {
  ...
  if (!autoMotor.motionRunning) {
    if (autoMotor.motionIndex >= autoMotor.motionCount) {
      ...
      autoMotor.motionIndex = 0;
      if (autoMotor.candidateIndex < autoMotor.candidateCount) {
        if (autoMotorBuildCandidate(autoMotor.stage, autoMotor.candidateIndex, autoMotor.current)) {
          autoMotorApplyCandidate(autoMotor.current);
        }
      }
      return;
    ...
    float delta = autoMotorTestMotionForIndex(autoMotor.stage, autoMotor.motionIndex);
    float safeMin = getAutoMotorSafeMinAngle();
    float safeMax = getAutoMotorSafeMaxAngle();
  ...
  if (absError <= holdBandEval) {
    ...
    if ((nowMs - autoMotor.settleStartMs) >= 180UL) {
      ...
      if ((nowMs - autoMotor.settleStartMs) >= 520UL) {
        ...
        bool lowResidual = (holdP2PNow <= (residualAllowance * 0.55f)) && (holdAvgErrNow <= max(0.6f, (float)motorStopEnter));
...
// Utility: stabilize the public/runtime collision flag.
// Unsafe state trips immediately. Clearing requires a continuous safe dwell,
// so a pose sitting on the geometric threshold cannot chatter COLLISION 1/0.
void updateCollisionFlagStable(bool rawCollision) {
  if (!collisionRuntimeEnabled || robotControlMode != ROBOT_MODE_MANIPULATOR) {
    collisionFlag = false;
    collisionFlagClearSinceMs = 0;
    return;
  }
  ...
  if (collisionFlagClearSinceMs == 0UL) {
    collisionFlagClearSinceMs = nowMs;
    return;
  }

  if ((uint32_t)(nowMs - collisionFlagClearSinceMs) >=
      (uint32_t)COLLISION_FLAG_CLEAR_DWELL_MS) {
    collisionFlag = false;
    collisionFlagClearSinceMs = 0;
  }
}
...
void invalidateCollisionPoseCache() {
  ...
  collisionActualSeverityValid = false;
  collisionActualSeverity = 0;
...
uint8_t poseCollisionSeverityCached(float baseDeg, float upperDeg, float foreDeg, float forearmRollDeg, float wristPitchDeg, float wristRotDeg, float gripDeg, bool forceRefresh) {
  ...
  uint32_t solverStartUs = micros();
  uint8_t severity = poseCollisionSeverity(baseDeg, upperDeg, foreDeg, forearmRollDeg, wristPitchDeg, wristRotDeg, gripDeg);
  uint32_t solverElapsedUs = (uint32_t)(micros() - solverStartUs);
  collisionSolverLastUs = solverElapsedUs;
  if (solverElapsedUs > collisionSolverMaxUs) collisionSolverMaxUs = solverElapsedUs;
  collisionSolverEvalCount++;
...
bool collisionGuardAllowsCandidatePose(float baseDeg, float upperDeg, float foreDeg, float forearmRollDeg, float wristPitchDeg, float wristRotDeg, float gripDeg) {
  ...
  if (collisionCandidateDecisionValid &&
    ...
    // A safe TARGET must not clear a collision reported by the physical pose.
    // Only the periodic actual-pose guard is allowed to clear COLL_FLAG.
    return true;
  }
  ...
  if (proposedSeverity == 0u) {
    // Candidate validation must not overwrite or clear the actual collision
    // state. A safe destination does not mean the mechanism has reached it.
    // Candidate validation must not overwrite the actual-pose cache or the
    // last-safe ACTUAL pose. The target may still be far ahead of the hardware.
    allowed = true;
    ...
    updateCollisionFlagStable(true);
    ...
    bool recoveryWithoutSafePose = (!collisionLastSafePoseValid && currentSeverity > 0u && currentSeverity != 0xFFu && proposedSeverity <= currentSeverity);

    if (severityRecovery || recoveryTowardSafe || recoveryWithoutSafePose) {
  ...
  // This cache field describes the RAW candidate geometry, not the debounced
  // public collision flag. A safe candidate remains reusable while COLL_FLAG is
  // waiting through its clear dwell.
  collisionCandidateDecisionCollisionFlag = (proposedSeverity != 0u);
...
void emitCollisionBlockedNotice() {
  ...
  // Keep this hook so guard call sites stay explicit without carrying unused
  // timers or injecting ASCII into the runtime stream.
}
...
bool manipulatorFirmwareHomePoseClose(float toleranceDeg) {
  ...
  for (uint8_t member = 0; member < MANIP_MEMBER_COUNT; member++) {
    ...
    float targetDeg = clampManipulatorMemberTargetToLimits(member, (float)getManipulatorHomeMemberDeg(member));
...
bool serviceManipulatorFirmwareHome(bool force) {
  ...
  for (uint8_t member = 0; member < MANIP_MEMBER_COUNT; member++) {
    ...
    float targetMemberDeg = clampManipulatorMemberTargetToLimits(member, (float)getManipulatorHomeMemberDeg(member));
  ...
  if (manipulatorFirmwareHomePoseClose(MANIPULATOR_FIRMWARE_HOME_COMPLETE_TOLERANCE_DEG)) {
    ...
    for (uint8_t member = 0; member < MANIP_MEMBER_COUNT; member++) {
      finalServoVals[member] = (int)roundf(convertManipulatorMemberToServoDeg(member, (float)getManipulatorHomeMemberDeg(member)));
    }
...
void applyCollisionGuard() {
  ...
  if (!collisionPoseChanged(baseDeg, upperDeg, foreDeg, forearmRollDeg, wristPitchDeg, wristRotDeg, gripDeg)) {
    if (collisionActualSeverityValid) {
      updateCollisionFlagStable(collisionActualSeverity > 0u);
    }
    return;
  }
  ...
  collisionActualSeverity = actualSeverity;
  collisionActualSeverityValid = true;
  updateCollisionFlagStable(actualSeverity > 0u);
...
void updateWristGimbalRuntime() {
  ...
  static unsigned long lastWristGimbalSampleMs = 0;
  unsigned long nowMs = millis();
  if (lastWristGimbalSampleMs != 0UL &&
      (uint32_t)(nowMs - lastWristGimbalSampleMs) < (uint32_t)STABILIZATION_SAMPLE_MS) return;
  lastWristGimbalSampleMs = nowMs;
...
void updateGripPressureModel() {
  ...
  static unsigned long lastGripPressureSampleMs = 0;
  unsigned long nowMs = millis();
  if (lastGripPressureSampleMs != 0UL &&
      (uint32_t)(nowMs - lastGripPressureSampleMs) < (uint32_t)GRIP_PRESSURE_SAMPLE_MS) return;
  lastGripPressureSampleMs = nowMs;
...
void updateArmStabilization() {
  ...
  if (manipulatorBaseMotionQuietingServos()) return;
  static unsigned long lastArmStabilizationSampleMs = 0;
  unsigned long nowMs = millis();
  if (lastArmStabilizationSampleMs != 0UL &&
      (uint32_t)(nowMs - lastArmStabilizationSampleMs) < (uint32_t)STABILIZATION_SAMPLE_MS) return;
  lastArmStabilizationSampleMs = nowMs;
  int16_t ax = 0, ay = 0, az = 0, gx = 0, gy = 0, gz = 0;
...
void printRobotSpecificConfigSummary() {
  ...
  Serial.print(F("vehicle_motor_max_speed_mps=")); Serial.println(vehicleControlProfile.maxSpeedMps, 3);
      ...
      Serial.print(F("vehicle_camera_pan_min_deg=")); Serial.println(CAMERA_PAN_MIN_DEG, 2);
      Serial.print(F("vehicle_camera_pan_max_deg=")); Serial.println(CAMERA_PAN_MAX_DEG, 2);
      Serial.print(F("vehicle_camera_tilt_min_deg=")); Serial.println(CAMERA_TILT_MIN_DEG, 2);
      Serial.print(F("vehicle_camera_tilt_max_deg=")); Serial.println(CAMERA_TILT_MAX_DEG, 2);
    ...
    case ROBOT_MODE_DRONE:
      ...
      Serial.print(F("drone_esc_max_motor_rpm=")); Serial.println(droneControlProfile.maxMotorRpm, 1);
...
void printVehicleConfigPanels() {
  ...
  Serial.println(F("  VEHMAXSPEED=<v>   -> maximum vehicle speed (m/s)"));
...
void printDroneConfigPanels() {
  ...
  Serial.println(F("  DRNMAXRPM=<v>   -> maximum motor speed reference (rpm)"));
...
static void printI2CAddressHex(uint8_t addr) {
  uint8_t hi = (uint8_t)((addr >> 4) & 0x0F);
  uint8_t lo = (uint8_t)(addr & 0x0F);
  Serial.print(F("0x"));
...
void printServoChannelsConfigPanel() {
  ...
  Serial.println(F("    Auto Torque is host-controlled: Processing owns it unless AiBot is connected"));
  Serial.println(F("    CH12..15 Live telemetry always reports the actual firmware PWM outputs"));
...
void printMotorConfigPanel() {
  ...
  if (motorType == MOTOR_TYPE_STEPPER) {
    ...
    Serial.println(F("  MOTORDIAG     -> show motor/collision timing diagnostic block"));
  ...
  if (motorType == MOTOR_TYPE_DC_MOTOR) {
    ...
    Serial.println(F("  PIDSTOP=<deg> -> DC PID hold/arrival entry band"));
    Serial.println(F("  PIDEXIT=<deg>  -> DC PID release/leave band"));
    Serial.println(F("  PIDDIR=NORMAL|INVERTED -> DC PID actuator polarity"));
    Serial.println(F("  PWM channel 12 defines the PID PWM control ceiling on pin 46"));
    Serial.println(F(""));
  }

  Serial.println(F("  ---[BOTH TYPES]---"));
  Serial.println(F("  LIMITMOTORCALIB -> capture the two ends of the selected one-turn A0 window"));
  Serial.println(F("  ZEROADC=<v>     -> lower ADC boundary of the selected turn"));
  Serial.println(F("  SPANRAW=<v>     -> upper ADC boundary of the selected turn"));
...
void printChannelConfig(uint8_t ch) {
  ...
  if (ch == 0) {
    ...
    Serial.println(F("BASE_CAL=ZEROADC_SPANRAW"));
  } else if (channelSupportsCalibration(ch)) {
    ...
    uint8_t value = (idx < 4) ? clampValue(pwmBootPercent[idx], (uint8_t)0, (uint8_t)100) : getDefaultPwmBootPercent(idx);
...
bool isKnownConfigAssignmentCommand(const char* cmd) {
  ...
  return strncmp(cmd, "ROBOT=", 6) == 0 ||
         ...
         strncmp(cmd, "KI=", 3) == 0 ||
         strncmp(cmd, "KD=", 3) == 0 ||
         strncmp(cmd, "PIDSTOP=", 8) == 0 ||
         strncmp(cmd, "PIDEXIT=", 8) == 0 ||
         strncmp(cmd, "PIDDIR=", 7) == 0 ||
...
void handleNumericCommand(const char* prefix, char* cmd, ValueMode mode) {
  ...
  if (mode == APPLY_MIN_LIMIT || mode == APPLY_MAX_LIMIT) {
    ...
    if (ch == 0 && (mode == APPLY_OFFSET || mode == APPLY_SPAN)) {
      if (allowPrimaryAsciiStatusMessages()) Serial.println(F("CH0 USES ZEROADC/SPANRAW; OFFSET0/SPAN0 ARE NOT USED"));
      return;
    }
    if (!channelSupportsCalibration(ch)) {
...
bool applyRuntimeServoTarget(uint8_t ch, float value) {
  ...
  if (!rt.active && rt.configMode) {
    if (ch == 0) {
      // CH0 is not a 0..180 servo. Its actual position always comes from the
      // calibrated A0 one-turn window, even while configuration mode is open.
      readMotorAngle();
      servos.actual[0] = motor.currentAngle;
    } else {
...
void handleConfigCommand(char* cmd) {
  ...
  *eq = '\0';
  ...
  if (strcmp(key, "PWMADDR") == 0 ||
    ...
    if ((strcmp(key, "PWMADDR") == 0 || strcmp(key, "INAADDR") == 0 || strcmp(key, "INA2ADDR") == 0 || strcmp(key, "MPUADDR") == 0) && parsedAddr == 0) {
      printConfigInvalidEcho(rawCmd);
      return;
    }

    if      (strcmp(key, "PWMADDR") == 0)  pca9685Addr = parsedAddr;
  ...
  if (strcmp(key, "PIDDIR") == 0) {
    if (!rt.configMode) {
      if (allowPrimaryAsciiStatusMessages()) Serial.println(F("DENIED: CFG MODE REQUIRED"));
      return;
    }
    if (motorType != MOTOR_TYPE_DC_MOTOR) {
      if (allowPrimaryAsciiStatusMessages()) Serial.println(F("DENIED: PIDDIR REQUIRES MOTORTYPE=DC_MOTOR"));
      return;
    }
    if (strcmp(valueStr, "NORMAL") == 0 || strcmp(valueStr, "0") == 0) {
      motorDcPidOutputInverted = false;
    } else if (strcmp(valueStr, "INVERTED") == 0 || strcmp(valueStr, "1") == 0) {
      motorDcPidOutputInverted = true;
    } else {
      printConfigInvalidEcho(rawCmd);
      return;
    }
  ...
  bool motorCalibrationChanged = false;

  if      (strcmp(key, "PWMADDR") == 0)   pca9685Addr = (uint8_t)clampValue((int)value, 1, 127);
  ...
  else if (strcmp(key, "PIDSTOP") == 0) {
    motorDcPidStopBandDeg = clampValue(value, MOTOR_DC_PID_BAND_MIN_DEG, MOTOR_DC_PID_BAND_MAX_DEG);
    ...
    if (motorDcPidExitBandDeg <= motorDcPidStopBandDeg) {
      motorDcPidStopBandDeg = max(MOTOR_DC_PID_BAND_MIN_DEG, motorDcPidExitBandDeg - 0.6f);
    }
    dcPidChanged = true;
  }
  else if (strcmp(key, "ZEROADC") == 0) {
    long requested = (long)roundf(value);
    long maxZero = (long)motorCalibrationSpanRaw - (long)MOTOR_CAL_MIN_SPAN_RAW;
    if (requested < (long)MOTOR_ADC_MIN || requested > maxZero) {
      Serial.print(F("INVALID ZEROADC - VALID RANGE=")); Serial.print(MOTOR_ADC_MIN); Serial.print(F("..")); Serial.println(maxZero);
      printConfigInvalidEcho(rawCmd);
      return;
    }
    motorCalibrationZeroADC = (int16_t)requested;
    motorCalibrationChanged = true;
  }
  else if (strcmp(key, "SPANRAW") == 0) {
    long requested = (long)roundf(value);
    long minUpper = (long)motorCalibrationZeroADC + (long)MOTOR_CAL_MIN_SPAN_RAW;
    if (requested < minUpper || requested > (long)MOTOR_ADC_MAX) {
      Serial.print(F("INVALID SPANRAW - VALID RANGE=")); Serial.print(minUpper); Serial.print(F("..")); Serial.println(MOTOR_ADC_MAX);
      printConfigInvalidEcho(rawCmd);
      return;
    }
    motorCalibrationSpanRaw = (int32_t)requested;
    motorCalibrationChanged = true;
  }
    ...
    if (!setManipulatorMemberChannel(MANIP_MEMBER_UPPER, (uint8_t)clampValue((int)value, (int)DEFAULT_MANIP_UPPER_CH, (int)LAST_SERVO_CH))) {
      printConfigInvalidEcho(rawCmd);
      return;
    }
  }
  else if (strcmp(key, "FORECH") == 0 || strcmp(key, "FORECHANNEL") == 0) {
    if (!setManipulatorMemberChannel(MANIP_MEMBER_FORE, (uint8_t)clampValue((int)value, (int)DEFAULT_MANIP_UPPER_CH, (int)LAST_SERVO_CH))) {
      printConfigInvalidEcho(rawCmd);
      return;
    }
  }
  else if (strcmp(key, "FOREROLLCH") == 0 || strcmp(key, "FOREARMROLLCH") == 0) {
    if (!setManipulatorMemberChannel(MANIP_MEMBER_FOREARM_ROLL, (uint8_t)clampValue((int)value, (int)DEFAULT_MANIP_UPPER_CH, (int)LAST_SERVO_CH))) {
      printConfigInvalidEcho(rawCmd);
      return;
    }
  }
  else if (strcmp(key, "WRISTPITCHCH") == 0 || strcmp(key, "WRISTVERTCH") == 0) {
    if (!setManipulatorMemberChannel(MANIP_MEMBER_WRIST_PITCH, (uint8_t)clampValue((int)value, (int)DEFAULT_MANIP_UPPER_CH, (int)LAST_SERVO_CH))) {
      printConfigInvalidEcho(rawCmd);
      return;
    }
  }
  else if (strcmp(key, "WRISTROTCH") == 0 || strcmp(key, "WRISTROLLCH") == 0) {
    if (!setManipulatorMemberChannel(MANIP_MEMBER_WRIST_ROLL, (uint8_t)clampValue((int)value, (int)DEFAULT_MANIP_UPPER_CH, (int)LAST_SERVO_CH))) {
      printConfigInvalidEcho(rawCmd);
      return;
    }
  }
  else if (strcmp(key, "GRIPCH") == 0 || strcmp(key, "GRIPPERCH") == 0) {
    if (!setManipulatorMemberChannel(MANIP_MEMBER_GRIPPER, (uint8_t)clampValue((int)value, (int)DEFAULT_MANIP_UPPER_CH, (int)LAST_SERVO_CH))) {
      printConfigInvalidEcho(rawCmd);
      return;
    }
  }
    ...
    vehicleProfile.cameraPanDefaultDeg = clampValue(value, CAMERA_PAN_MIN_DEG, CAMERA_PAN_MAX_DEG);
  }
  else if (strcmp(key, "VEHCAMTILT0") == 0) {
    vehicleProfile.cameraTiltDefaultDeg = clampValue(value, CAMERA_TILT_MIN_DEG, CAMERA_TILT_MAX_DEG);
  }
  ...
  else if (strcmp(key, "VEHMAXSPEED") == 0) {
    vehicleControlProfile.maxSpeedMps = clampValue(value, 0.05f, 20.0f);
  }
  ...
  else if (strcmp(key, "DRNMAXRPM") == 0) {
    droneControlProfile.maxMotorRpm = clampValue(value, 1000.0f, 100000.0f);
  }
  ...
  if (dcPidChanged || motorCalibrationChanged) {
    sendPidCalibrationStateLine();
  }
  releaseInactiveManipulatorChannels();
  noteConfigSessionEdited(false);
...
void handleCommand(char* cmd) {
  ...
  // 0x17 text frames use the current #CMD envelope. Reject a malformed
  // envelope instead of routing a partially tokenized command. Plain ASCII
  // boot/config commands remain valid when they do not use the envelope.
  if (strncmp(cmd, "#CMD|", 5) == 0 && !unwrapStreamCommand(cmd)) return;
  canonicalizeCommandText(cmd);
  if (cmd[0] == '\0') return;
