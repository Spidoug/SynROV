...
// V1 uses its own persistent signatures. No pre-V1 EEPROM block is accepted.
const uint32_t SAFETY_CONFIG_MAGIC = 0x53414631UL; // SAF1
...
// Fast feedback path for the DC servo. The old fixed 0.10 EMA added roughly
...
// cfg.payload.reserved1[0] as
// centi-degrees to avoid changing the EEPROM payload layout.
const float MOTOR_BASE_A0_DEADBAND_DEFAULT_DEG = 0.50f;
...
const float MOTOR_DC_PID_ENTER_BAND_DEFAULT_DEG = 0.20f;
const float MOTOR_DC_PID_EXIT_BAND_DEFAULT_DEG  = 0.90f;
...
// of that selected one-turn window and SPANRAW is (upperBoundary - ZEROADC).
// No CH0 feedback conversion may use the complete 0..1023 ADC range as travel.
...
const int16_t MOTOR_BASE_DEFAULT_ZERO_ADC = 10;
const int32_t MOTOR_BASE_DEFAULT_SPAN_RAW = 302;
...
// this headroom, raw A0 values just outside ZEROADC..ZEROADC+SPANRAW collapse
...
// V1 factory values are loaded whenever the V1 main configuration is absent.
const float MOTOR_DC_KP_DEFAULT = 30.00f;
const float MOTOR_DC_KI_DEFAULT = 5.00f;
const float MOTOR_DC_KD_DEFAULT = 9.00f;
...
const uint32_t EEPROM_MAGIC   = 0x53525631UL; // SRV1
const uint16_t EEPROM_SCHEMA_ID = 1; // SynROV V1 EEPROM schema
...
const uint8_t STEPPERMODE_STORE_MAGIC = 0xB0;
...
struct VehicleControlProfile {
  ...
  PackedGeom16 cameraPanMinDeg;
  PackedGeom16 cameraPanMaxDeg;
  PackedGeom16 cameraTiltMinDeg;
  PackedGeom16 cameraTiltMaxDeg;
...
static const VehicleProcessingProfile kDefaultVehicleProfile PROGMEM = {95.70f, 59.16f, 18.56f, 71.92f, 69.60f, 13.92f, 6.96f, 4.06f, 17.40f, 5.80f, 2.03f, 5.80f, 64, 2.32f, -24.13f, 0.0f, -10.0f, -17.63f, -7.66f, -12.06f, 19.14f};
...
static const VehicleControlProfile kDefaultVehicleControlProfile PROGMEM = {5.0f, 36, 255, 12.0f, false, false, -90.0f, 90.0f, -45.0f, 35.0f, 30, 150, 1800};
static const DroneControlProfile kDefaultDroneControlProfile PROGMEM = {DRONE_ESC_MIN_US, DRONE_ESC_MAX_US, 1080, 900, 200, 200, 135, 1.5f, 34};
...
const uint32_t COMPASS_CAL_MAGIC = 0x43414C31uL; // CAL1
const uint32_t CONFIG_EXTRAS_MAGIC = 0x45585431uL; // EXT1
const uint16_t CONFIG_EXTRAS_SCHEMA_ID = 1;
const uint8_t ROBOT_MODE_STORE_MAGIC = 0xD1;
...
int32_t  motorOneTurnSpanRaw     = MOTOR_BASE_DEFAULT_SPAN_RAW;
...
PackedAngle16 motorDcPidEnterBandDeg    = MOTOR_DC_PID_ENTER_BAND_DEFAULT_DEG;
