...
class DedicatedRobotAI:
    ...
    def intent_dwell_seconds(self, name: str) -> float:
        if name in {"stop", "hover", "land", "takeoff", "home"}:
            return 0.0
        if name.startswith("camera_") or name in {"lights_toggle", "lidar_toggle", "camera_stream_toggle"}:
            return 0.18
        return 0.45
...
class DroneAI(DedicatedRobotAI):
    ...
    def _flight_status(self, bridge: Any) -> Dict[str, Any]:
        ...
        ready = bool(flight.get("flightReady", False))
        airborne = bool(flight.get("airborne", False))
        if not ready:
            sensors = getattr(state, "sensors", {}) or {}
            altitude = 0.0
            ...
            ready = altitude >= 24.0
            airborne = airborne or altitude > 3.0
        if not phase:
    ...
    def tick(
        ...
        # Explicit plans such as "decole, depois avance" wait for the physical
        # flight-ready state before consuming the next queued step.
    ...
    def _execute_intent(self, name: str, bridge: Any) -> CommandResult:
        ...
        if name == "up" and not flight_status["airborne"]:
            bridge.drone_takeoff()
            return CommandResult(True, self.robot, "intent", name, "takeoff_started_for_climb", {"flight": flight_status})
        ...
        mapping: Dict[str, Tuple[float, float, float, float, float, float]] = {
            ...
            # Processing keyboard contract: Q = yaw right (+), E = yaw left (-).
        ...
        # Canonical camera tilt for Vehicle and Drone: R = down (-), F = up (+).
        camera_delta = {
            "camera_left": (-5.0, 0.0),
            "camera_right": (5.0, 0.0),
            "camera_up": (0.0, 5.0),
    ...
    def _tick_mission(
        ...
        if name == "return_home":
            ...
            if distance <= arrival:
                ...
                self.active_mission = None
                return 0.0, {"strategy": "origin_navigation", "strategy_variant": variant, "arrived_home": True, "distance": distance, "units": nav["units"]}
            # Drone forward points toward -Z at yaw=0 in Processing. This angle
            # therefore points the nose directly at the vector back to origin.
            desired_yaw = math.degrees(math.atan2(-dx, -dz))
            yaw_error = _angle_error_deg(desired_yaw, nav["yaw_deg"])
