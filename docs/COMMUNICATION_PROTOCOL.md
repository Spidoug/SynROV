...
| `camera` | `camera` object | Vehicle/Drone camera pan, tilt and enable state. |
...
The camera-enable command is `{"camera":{"robot":"Vehicle|Drone","enabled":true|false}}`. `enabled` is the single operator camera state: it starts/stops the robot-camera channel, controls the yellow camera indicator, and enables/disables the same Processing camera mode used by the diagnostics CAMERA button. Drone firmware camera state is synchronized from this command; camera enable is not embedded in Drone flight packets. Pan and tilt may be supplied in the same `camera` object.

### Direction semantics

Control payloads carry one canonical runtime coordinate system. The protocol never changes a sign according to source, robot UI, transport, or client: a numeric field has the same meaning whether it came from Web, AiBot, ROS, joystick processing, or local controls. Vehicle `drive.steer` and `drive.pivot` are independent; for rotational fields, negative and positive remain fixed protocol coordinates. Human-facing controls translate their visual reference into those canonical values before transmission. For example, a mirrored on-screen yaw button or an AiBot phrase can select the opposite numeric sign without redefining the yaw field itself. Strafe, throttle, camera pan/tilt, and Manipulator absolute targets follow the same rule: visual mapping happens before the packet, and Processing consumes the packet unchanged. See `CONTROL_DIRECTION_AND_SAFE_HOME.md` for the coordinate and visual-reference tables.
