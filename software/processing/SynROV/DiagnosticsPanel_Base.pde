...
// The diagnostics panel starts open. Double-click its title to collapse it into
// a floating button; one click on that button restores the panel.
boolean diagnosticsPanelExpanded = true;
long diagnosticsTitleLastClickMs = 0;
final int DIAGNOSTICS_TITLE_DOUBLE_CLICK_MS = 360;
final int DIAGNOSTICS_TOGGLE_W = 188;
final int DIAGNOSTICS_TOGGLE_H = 32;
...
HashMap<String, ArrayDeque<Float>> diagnosticsSensorHistory = new HashMap<String, ArrayDeque<Float>>();
...
String manipulatorNeutralLabel(int memberIdx) {
  switch(memberIdx) {
  case 0: return tr("Base");
  case 1: return tr("Upper arm");
  case 2: return tr("Forearm");
  case 3: return tr("Forearm roll");
  case 4: return tr("Wrist pitch");
  case 5: return tr("Wrist roll");
  case 6: return tr("Gripper");
  }
  return tr("Joint");
}
...
final int DIAGNOSTICS_MAP_CARD_H = 142;
...
// Returns whether the current viewport has enough room for the full diagnostics panel.
boolean diagnosticsPanelCanFit() {
...
boolean diagnosticsPanelVisible() {
  return diagnosticsPanelExpanded && diagnosticsPanelCanFit();
}

String diagnosticsPanelTitleText() {
  return (isManipulatorSelected ? tr("Manipulator") : (isVehicleSelected ? tr("Vehicle") : "Drone")) + tr(" diagnostics");
}

int diagnosticsToggleButtonX() {
  if (diagnosticsPanelCanFit()) {
    diagnosticsPanelW = preferredDiagnosticsPanelWidth();
    return diagnosticsPanelX() + diagnosticsPanelW - DIAGNOSTICS_TOGGLE_W;
  }
  return max(round(leftSidebarWidth()) + 12, width - diagnosticsPanelMarginRight - DIAGNOSTICS_TOGGLE_W);
}

int diagnosticsToggleButtonY() {
  return diagnosticsPanelCanFit() ? diagnosticsPanelY() + 8 : 12;
}

boolean pointInDiagnosticsToggleButton(int mx, int my) {
  return pointInRect(mx, my, diagnosticsToggleButtonX(), diagnosticsToggleButtonY(), DIAGNOSTICS_TOGGLE_W, DIAGNOSTICS_TOGGLE_H);
}

boolean pointInDiagnosticsPanelTitle(int mx, int my) {
  if (!diagnosticsPanelVisible()) return false;
  return pointInRect(mx, my, diagnosticsPanelX(), diagnosticsPanelY(), diagnosticsPanelW, 38);
}
...
int preferredDiagnosticsPanelHeight() {
  if (isManipulatorSelected) return min(952, height);
  if (isVehicleSelected) return min(822, height);
  if (isDroneSelected) return min(792, height);
...
void ensureDiagnosticsHUD(int w, int h) {
  ...
  if (diagnosticsFont == null) diagnosticsFont = synUiFont;
}
...
void drawStabilityTargetSummaryBlock2D(float x, float y, float w, float h, DiagnosticsStabilityTarget3D target, int idx) {
  ...
  String statusLine = (target.active ? tr("Telemetry active") : tr("No active reference"));
  statusLine += "  ·  " + tr("Vel ") + nf(target.velocityCms, 1, 1) + " cm/s";
  if (!medium) {
...
void drawStabilityTargets3D2D(float x, float y, float w, float h, String label, DiagnosticsStabilityTarget3D[] targets) {
  ...
  if (stackedLayout) {
    ...
    textSize(9.5f);
    text(tr("Pitch/Roll · arrow = Yaw"), cx, max(plotY + 4, maxPlotBottom - 12));
  } else {
    infoY = pad;
    infoH = h - pad * 2;
    float legendH = 16;
    ...
    textAlign(CENTER, TOP);
    textSize(9.5f);
    text(tr("Pitch/Roll · arrow = Yaw"), cx, h - pad - 12);
  }
...
void drawVehicleStabilityTargets2D(float x, float y, float w, float h) {
  ...
  targets[0] = new DiagnosticsStabilityTarget3D(tr("Chassis"), mpu1RollDeg, mpu1PitchDeg, getSensorFloatAny(new String[] {"vehicle_yaw_deg", "heading_deg"}, mpu1YawDeg), mpuVelocityMagCms[0], active, 0);
  targets[1] = new DiagnosticsStabilityTarget3D(tr("Camera/LiDAR"), getSensorFloat("vehicle_cam_roll_deg", mpu2RollDeg), getSensorFloat("vehicle_cam_tilt_deg", mpu2PitchDeg), getSensorFloat("vehicle_cam_pan_deg", mpu2YawDeg), mpuVelocityMagCms[1], active && hasAnySensorKey(new String[] {"mpu2_ax", "vehicle_cam_pan_deg", "vehicle_cam_tilt_deg"}), 1);
  drawStabilityTargets3D2D(x, y, w, h, tr("Vehicle stability targets"), targets);
}
...
void drawDroneStabilityTargets2D(float x, float y, float w, float h) {
  boolean active = diagnosticsHasActiveSensorTelemetry();
  DiagnosticsStabilityTarget3D[] targets = new DiagnosticsStabilityTarget3D[2];
  targets[0] = new DiagnosticsStabilityTarget3D(tr("Flight body"), getSensorFloat("drone_roll_deg", mpu1RollDeg), getSensorFloat("drone_pitch_deg", mpu1PitchDeg), getSensorFloatAny(new String[] {"drone_yaw_deg", "att_yaw_deg", "heading_deg"}, mpu1YawDeg), mpuVelocityMagCms[0], active, 0);
  targets[1] = new DiagnosticsStabilityTarget3D(tr("Camera/Gimbal"), getSensorFloat("drone_cam_roll_deg", mpu2RollDeg), getSensorFloat("drone_cam_tilt_deg", mpu2PitchDeg), getSensorFloat("drone_cam_pan_deg", mpu2YawDeg), mpuVelocityMagCms[1], active && hasAnySensorKey(new String[] {"mpu2_ax", "drone_cam_pan_deg", "drone_cam_tilt_deg"}), 1);
  drawStabilityTargets3D2D(x, y, w, h, tr("Drone stability targets"), targets);
}
...
void drawManipulatorStabilityTargets2D(float x, float y, float w, float h) {
  boolean active = diagnosticsHasActiveSensorTelemetry();
  DiagnosticsStabilityTarget3D[] targets = new DiagnosticsStabilityTarget3D[2];
  targets[0] = new DiagnosticsStabilityTarget3D(tr("Gripper/Wrist"), mpu1RollDeg, mpu1PitchDeg, getSensorFloatAny(new String[] {"wrist_rot_deg", "gimbal_yaw_deg"}, mpu1YawDeg), mpuVelocityMagCms[0], active, 0);
  targets[1] = new DiagnosticsStabilityTarget3D(tr("Arm/Base"), mpu2RollDeg, mpu2PitchDeg, getSensorFloatAny(new String[] {"base_yaw_deg", "heading_deg"}, mpu2YawDeg), mpuVelocityMagCms[1], active && hasAnySensorKey(new String[] {"mpu2_ax", "base_yaw_deg", "heading_deg"}), 1);
  drawStabilityTargets3D2D(x, y, w, h, tr("Grip/arm stability targets"), targets);
}
...
String formatSensorKeyLabel(String key) {
  if (key == null) return tr("sensor");
  String k = trim(key).toLowerCase();
  if (k.equals("mpu1_ax")) return tr("Main accel X raw");
  if (k.equals("mpu1_ay")) return tr("Main accel Y raw");
  if (k.equals("mpu1_az")) return tr("Main accel Z raw");
  if (k.equals("mpu1_gx")) return tr("Main gyro X raw");
  if (k.equals("mpu1_gy")) return tr("Main gyro Y raw");
  if (k.equals("mpu1_gz")) return tr("Main gyro Z raw");
  if (k.equals("mpu2_ax")) return tr("Aux accel X raw");
  if (k.equals("mpu2_ay")) return tr("Aux accel Y raw");
  if (k.equals("mpu2_az")) return tr("Aux accel Z raw");
  if (k.equals("mpu2_gx")) return tr("Aux gyro X raw");
  if (k.equals("mpu2_gy")) return tr("Aux gyro Y raw");
  if (k.equals("mpu2_gz")) return tr("Aux gyro Z raw");
  if (k.equals("heading_deg")) return tr("Compass heading");
  if (k.equals("mag_cal_active")) return tr("Compass calibration active");
  if (k.equals("mag_cal_progress")) return tr("Compass calibration progress");
  if (k.equals("seq")) return tr("Seq");
  if (k.equals("rxseq")) return tr("Rx seq");
  if (k.equals("stamp_ms")) return tr("Stamp ms");
  if (k.equals("link_ms")) return tr("Link ms");
  if (isDroneSelected) {
    if (k.equals("an4") || k.equals("an_4")) return tr("Left distance sensor");
    if (k.equals("an5") || k.equals("an_5")) return tr("Right distance sensor");
    if (k.equals("drone_cam_pan_deg")) return tr("Gimbal pan");
    if (k.equals("drone_cam_tilt_deg")) return tr("Gimbal tilt");
    if (k.equals("sonar_vertical_cm")) return tr("Downward sonar vertical");
    if (k.equals("sonar_ground_ref_cm")) return tr("Sonar ground reference");
    if (k.equals("sonar_height_cm") || k.equals("drone_height_from_sonar_cm")) return tr("Sonar/MPU height");
    if ((k.equals("sonar_cm") || k.equals("range_cm")) && isManipulatorSelected) return tr("Hand sonar distance (< 4 m)");
    if (k.equals("sonar_cm") || k.equals("range_cm") || k.equals("drone_scan_cm") || k.equals("drone_sonar_down_cm") || k.equals("height_cm") || k.equals("alt_cm")) return tr("Downward sonar height (< 4 m)");
    if (k.equals("battery_raw") || k.equals("battery")) return tr("Battery raw");
    if (k.equals("battery_pct") || k.equals("bat_pct")) return tr("Battery percent");
    if (k.equals("ina1_ma")) return tr("Current sensor 1");
    if (k.equals("ina2_ma")) return tr("Current sensor 2");
  }
  if (isVehicleSelected) {
    if (k.equals("an2") || k.equals("an_2")) return tr("Left torque sensor");
    if (k.equals("an3") || k.equals("an_3")) return tr("Right torque sensor");
    if (k.equals("lidar_cm") || k.equals("lidar") || k.equals("range_cm") || k.equals("sonar_cm")) return tr("LiDAR distance");
    if (k.equals("vehicle_cam_pan_deg")) return tr("Camera pan");
    if (k.equals("vehicle_cam_tilt_deg")) return tr("Camera tilt");
    if (k.equals("battery_raw") || k.equals("battery")) return tr("Battery raw");
    if (k.equals("battery_pct") || k.equals("bat_pct")) return tr("Battery percent");
    if (k.equals("ina1_ma")) return tr("Current sensor 1");
    if (k.equals("ina2_ma")) return tr("Current sensor 2");
  ...
  if (isManipulatorSelected) {
    if (k.equals("an2") || k.equals("an_2")) return tr("Base torque sensor");
    if (k.equals("an3") || k.equals("an_3")) return tr("Upper arm torque sensor");
    if (k.equals("an4") || k.equals("an_4")) return tr("Forearm vertical torque sensor");
    if (k.equals("an5") || k.equals("an_5")) return tr("Forearm rotational torque sensor");
    if (k.equals("pwm_12")) return tr("Base force control");
    if (k.equals("pwm_13")) return tr("Upper arm force control");
    if (k.equals("pwm_14")) return tr("Forearm vertical force control");
    if (k.equals("pwm_15")) return tr("Forearm rotation force control");
    if (k.equals("ina1_ma")) return tr("Wrist group current 1");
    if (k.equals("ina2_ma")) return tr("Wrist group current 2");
  }
  String pretty = key.replace('_', ' ');
  if (pretty.length() == 0) return tr("sensor");
...
boolean diagnosticsSensorKeyIsNumeric(JSONObject sens, String key) {
  if (!jsonHasValue(sens, key)) return false;
  float sentinel = Float.NaN;
  return !Float.isNaN(sens.getFloat(key, sentinel));
}

// Utility: diagnostics sensor numeric value.
float diagnosticsSensorNumericValue(JSONObject sens, String key) {
  return getJsonFloat(sens, key, 0.0f);
}
...
void startDiagnosticsSensorRecording() {
  ...
  if (!diagnosticsCanStartSensorRecording()) {
    updateMessage(tr("Connect a robot before using Record Sensors."));
    return;
  }
  ...
  updateMessage(tr("Sensor recording started: ") + diagnosticsSensorRecordFilename);
}
...
void stopDiagnosticsSensorRecording() {
  ...
  updateMessage(tr("Sensor recording saved to ") + diagnosticsSensorRecordFilename);
}
...
void appendDiagnosticsSensorHistorySnapshot(JSONObject sens) {
  ...
  for (int i = 0; i < numericKeys.size(); i++) {
    ...
    ArrayDeque<Float> history = diagnosticsSensorHistory.get(key);
    if (history == null) {
      history = new ArrayDeque<Float>();
      diagnosticsSensorHistory.put(key, history);
    }
    history.addLast(diagnosticsSensorNumericValue(sens, key));
...
void drawDiagnosticsSensorChart(float x, float y, float w, float h, String key) {
  ...
  if (history == null || history.size() < 2) {
    ...
    text(tr("Waiting for data"), x + 10, y + 32);
    return;
  }

  Float[] histArr = history.toArray(new Float[0]);
  ...
  beginShape();
  for (int i = 0; i < histArr.length; i++) {
    float xx = map(i, 0, max(1, histArr.length - 1), left, right);
    float yy = map(histArr[i], minV, maxV, bottom, top);
    vertex(xx, yy);
  }
  endShape();
  strokeWeight(1);
  ...
  fill(uiPrimaryTextColor());
  float latest = histArr[histArr.length - 1];
  textAlign(LEFT, BOTTOM);
  text(tr("Latest: ") + nf(latest, 1, 2), left, y + h - 6);
...
void drawDiagnosticsGraphsWindow() {
  ...
  textSize(14);
  textAlign(LEFT, TOP);
  text(tr("Numeric sensor charts"), winX + 14, winY + 12);
  textSize(11);
  fill(uiSecondaryTextColor());
  text(tr("All numeric sensor streams are shown here, split across pages."), winX + 14, winY + 34);

  int topBtnY = winY + 10;
  int closeBtnW = 74;
  int recBtnW = 104;
  drawMiniButton2D(winX + winW - closeBtnW - 12, topBtnY, closeBtnW, 24, tr("Close"), false);
  drawMiniButton2D(winX + winW - closeBtnW - recBtnW - 20, topBtnY, recBtnW, 24, diagnosticsSensorRecording ? tr("Stop REC") : tr("Record"), diagnosticsSensorRecording, diagnosticsSensorRecordButtonEnabled());
  ...
  if (diagnosticsSensorNumericKeys.isEmpty()) {
    fill(uiSecondaryTextColor());
    textAlign(CENTER, CENTER);
    text(tr("No numeric sensors received yet."), winX + winW * 0.5f, winY + winH * 0.5f);
  }

  int navY = winY + winH - 34;
  drawMiniButton2D(winX + 14, navY, 72, 22, tr("< Prev"), diagnosticsGraphsPage > 0);
  drawMiniButton2D(winX + 94, navY, 72, 22, tr("Next >"), diagnosticsGraphsPage < pageCount - 1);
  fill(uiSecondaryTextColor());
  textAlign(LEFT, CENTER);
  text(tr("Page ") + (diagnosticsGraphsPage + 1) + " / " + pageCount + "  |  " + tr("Sensors: ") + diagnosticsSensorNumericKeys.size(), winX + 182, navY + 11);
...
void toggleEnvironmentScan() {
  ...
  updateMessage(tr(currentModeName()) + tr(" scan: ") + tr(currentEnvironmentAutoScanEnabled() ? "ON" : "OFF"));
}
...
void toggleDroneDownwardSonar() {
  setCurrentEnvironmentAutoScan(!currentEnvironmentAutoScanEnabled());
  saveSoftwareConfigNow();
  updateMessage(tr("Drone downward sonar: ") + tr(currentEnvironmentAutoScanEnabled() ? "ON" : "OFF"));
}
...
void toggleGimbalStab() {
  ...
  if (!sent) {
    updateMessage(tr("Gimbal stabilization command not sent"));
    return;
  }
  gimbalStabEnabled = newState;
  saveSoftwareConfigNow();
  updateMessage(tr("Gimbal stabilization: ") + (gimbalStabEnabled ? "ON" : "OFF"));
}
...
void toggleArmStab() {
  ...
  if (!sent) {
    updateMessage(tr("Arm stabilizer command not sent."));
    return;
  }
  armStabEnabled = newState;
  saveSoftwareConfigNow();
  updateMessage(tr("Arm stabilizer: ") + (armStabEnabled ? "ON" : "OFF"));
}
...
void clearTraceMap() {
  ...
  updateMessage(tr("Scanned map and demo world cleared."));
}

// Utility: remove imported world only.
void removeImportedWorldOnly() {
  if (!hasImportedWorldForRobotMode(currentRobotMode())) {
    updateMessage(tr("No imported world to remove."));
    return;
  }
  clearCurrentImportedWorld();
  updateMessage(tr("Imported world removed. Scanned map preserved."));
}
...
void toggleRemoteCollisionFromPanel() {
  ...
  saveSoftwareConfigNow();
  updateMessage(tr("Environment collision: ") + (environmentCollisionEnabled ? "ON" : "OFF"));
}
...
void toggleVehicleLights() {
  ...
  saveSoftwareConfigNow();
  updateMessage(tr("Vehicle lights: ") + (vehicleLightsEnabled ? "ON" : "OFF"));
  sendSystemStatus();
}
...
void toggleVehicleLidarScan() {
  ...
  saveSoftwareConfigNow();
  updateMessage(tr("Vehicle lidar: ") + (vehicleLidarScanEnabled ? "ON" : "OFF"));
  sendSystemStatus();
}

// Utility: toggle drone camera.
void toggleDroneCamera() {
  toggleSelectedDroneCameraView();
}
...
String diagnosticsCompassSummaryText() {
  if (!diagnosticsHasActiveSensorTelemetry()) return tr("Compass: --");
  String headingText = tr("Compass: ") + nf(normalizeAbsoluteAngleDeg(diagnosticsCurrentCompassHeadingDeg()), 1, 1) + "°";
  if (compassCalibrationActive()) {
    headingText += "  |  " + tr("Cal ") + nf(compassCalibrationProgressPct(), 1, 0) + "%";
  }
  if (worldCompassNorthLockedForRobotMode(currentRobotMode())) {
    headingText += "  |  " + tr("World N ") + nf(worldCompassNorthOffsetDegForRobotMode(currentRobotMode()), 1, 1) + "°";
  }
  return headingText;
}
...
void triggerCompassCalibration() {
  if (compassCalibrationActive()) {
    ...
    updateMessage(tr(currentModeName()) + tr(" compass calibration cancelled"));
    return;
  }
  queueSerialCommand("MAGCAL=1");
  updateMessage(tr(currentModeName()) + tr(" compass calibration started"));
}
...
void diagnosticsTakeLocalControl() {
  updateControlLocks();
  long now = millis();
  boolean takeover = isRemoteControlSource(currentControlOwner);

  currentControlOwner = CONTROL_SOURCE_LOCAL;
  lastLocalControlMillis = now;
  if (takeover) {
    lastWebControlMillis = 0;
    lastAiBotControlMillis = 0;
    rosLastControlMillis = 0;
    notifyControlLockNotice("Local diagnostics takeover: remote control overridden");
    sendSystemStatus();
  } else {
...
boolean handleDiagnosticsPanelMousePressed(int mx, int my) {
  if (!diagnosticsPanelVisible()) {
    if (pointInDiagnosticsToggleButton(mx, my)) {
      diagnosticsPanelExpanded = true;
      diagnosticsTitleLastClickMs = 0;
      return true;
    }
    return false;
  }

  if (pointInDiagnosticsPanelTitle(mx, my)) {
    long nowMs = millis();
    if (diagnosticsTitleLastClickMs > 0 && (nowMs - diagnosticsTitleLastClickMs) <= DIAGNOSTICS_TITLE_DOUBLE_CLICK_MS) {
      diagnosticsPanelExpanded = false;
      diagnosticsGraphsWindowOpen = false;
      diagnosticsTitleLastClickMs = 0;
    } else {
      diagnosticsTitleLastClickMs = nowMs;
    }
    return true;
  }
...
void drawDiagnosticsSlider2D(int x, int y, int w, String label, int channel, int targetValue, int liveValue) {
  ...
  textAlign(RIGHT, TOP);
  textSize(11);
  text(tr("Cmd ") + targetValue + "%  |  " + tr("Live ") + liveValue + "%", x + w, y + 1);
  ...
  // The slider is the command source: its position always represents Cmd.
  // The independent vertical marker is the Arduino-reported Live value.
  int displayValue = targetValue;
  float displayX = map(displayValue, 0, 100, x, x + w);
  noStroke();
  fill(active ? color(70, 140, 255) : color(100, 170, 255));
  rect(x, barY, max(0, displayX - x), DIAGNOSTICS_SLIDER_BAR_H, 5);
  ...
  ellipse(displayX, barY + DIAGNOSTICS_SLIDER_BAR_H / 2, active ? 13 : 11, active ? 13 : 11);
  textSize(12);
}

// Draws the collapsed diagnostics button using the same frame palette as the panel.
void drawDiagnosticsToggleButton() {
  ensureDiagnosticsHUD(DIAGNOSTICS_TOGGLE_W, DIAGNOSTICS_TOGGLE_H);
  int x = diagnosticsToggleButtonX();
  int y = diagnosticsToggleButtonY();
  hint(DISABLE_DEPTH_TEST);
  pushMatrix();
  resetMatrix();
  camera();
  ...
  textFont(diagnosticsFont);
  noLights();
  synDrawFrame2D(x, y, DIAGNOSTICS_TOGGLE_W, DIAGNOSTICS_TOGGLE_H, uiBorderColor(), uiCanvasBackgroundColor(), 10, 1.0f);
  fill(uiPrimaryTextColor());
  drawDiagnosticsFittedTextCentered(diagnosticsPanelTitleText(), x + DIAGNOSTICS_TOGGLE_W / 2, y + DIAGNOSTICS_TOGGLE_H / 2 - 1, DIAGNOSTICS_TOGGLE_W - 16, 11, 9.5f);
  popStyle();
  safePopMatrix("DiagnosticsPanel_Base.pde:toggle");
  hint(ENABLE_DEPTH_TEST);
}

// Draws the expanded panel or its floating restore button.
void drawDiagnosticsPanelOrToggle() {
  if (diagnosticsPanelVisible()) drawDiagnosticsPanel();
  else {
    diagnosticsGraphsWindowOpen = false;
    drawDiagnosticsToggleButton();
  }
}
...
void drawDiagnosticsPanel() {
  ...
  fill(uiPrimaryTextColor());
  textAlign(LEFT, TOP);
  text(diagnosticsPanelTitleText(), panelX + 18, panelY + 14);
  fill(uiSecondaryTextColor());
  textSize(12);

  drawMiniButton2D(contentX, panelY + 42, col2W, DIAGNOSTICS_BUTTON_H, tr("Charts"), diagnosticsGraphsWindowOpen);
  drawMiniButton2D(contentX + col2W + gap, panelY + 42, col2W, DIAGNOSTICS_BUTTON_H, diagnosticsSensorRecording ? tr("Stop recording") : tr("Record sensors"), diagnosticsSensorRecording, diagnosticsSensorRecordButtonEnabled());
  ...
  drawSectionCard2D(contentX, mapCardY, contentW, DIAGNOSTICS_MAP_CARD_H, tr("Map and world"));
  drawMiniButton2D(toolsInnerX, diagnosticsMapButtonRowY(panelY, 0), toolsCol2W, DIAGNOSTICS_BUTTON_H, isManipulatorSelected ? tr("Local collision") : tr("Map collision"), isManipulatorSelected ? (collisionSet || pendingManipulatorLocalCollisionEnableAfterHome || collisionWorldSolidForRobotMode(ROBOT_MODE_MANIPULATOR)) : environmentCollisionActiveForRobotMode(currentRobotMode()));
  drawMiniButton2D(toolsInnerX + toolsCol2W + gap, diagnosticsMapButtonRowY(panelY, 0), toolsCol2W, DIAGNOSTICS_BUTTON_H, tr("Clear map"), false);
  drawMiniButton2D(toolsInnerX, diagnosticsMapButtonRowY(panelY, 1), toolsCol2W, DIAGNOSTICS_BUTTON_H, tr("Load world"), worldImportBusy || currentWorldSourceLabel().length() > 0);
  drawMiniButton2D(toolsInnerX + toolsCol2W + gap, diagnosticsMapButtonRowY(panelY, 1), toolsCol2W, DIAGNOSTICS_BUTTON_H, tr("Remove world"), hasImportedWorldForRobotMode(currentRobotMode()));
  drawMiniButton2D(toolsInnerX, diagnosticsMapButtonRowY(panelY, 2), toolsCol2W, DIAGNOSTICS_BUTTON_H, tr("Adjust world"), isWorldTransformOverlayActive(), hasImportedWorldForRobotMode(currentRobotMode()));
  drawMiniButton2D(toolsInnerX + toolsCol2W + gap, diagnosticsMapButtonRowY(panelY, 2), toolsCol2W, DIAGNOSTICS_BUTTON_H, tr("Save collision"), collisionWorldSaveDialogOpen, currentEnvironmentPointList().size() > 0);
