...
void addEnvironmentPoint(ArrayList<PVector> list, PVector point) {
  ...
  list.add(point.copy());
  while (list.size() > ENVIRONMENT_MAX_POINTS_PER_MAP) {
    list.remove(0);
  }
}
...
boolean manipulatorCollisionAgainst(ArrayList<PVector> points, ArrayList<PVector> samples, float hitRadius) {
  ...
  for (int i = 0; i < samples.size(); i++) {
    ...
    for (int j = 0; j < points.size(); j++) {
      if (PVector.dist(s, points.get(j)) <= hitRadius) return true;
    }
  }
  return false;
}
...
boolean checkEnvironmentCollisionForCurrentPose() {
  ...
  return manipulatorCollisionAgainst(manipulatorEnvironmentPoints, samples, hitRadius) ||
...
void drawEnvironmentPointSet(ArrayList<PVector> points, float alphaScale, float radiusScale) {
  ...
  boolean simpleRender = (points == currentImportedWorldPointList() && points.size() >= IMPORTED_WORLD_SIMPLE_RENDER_THRESHOLD);
  ...
  for (int i = 0; i < points.size(); i += drawStride) {
    ...
    sphere(pointRadius);
    safePopMatrix("EnvironmentMapping.pde:2149");
  }
}
