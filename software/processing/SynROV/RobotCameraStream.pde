...
final float ROBOT_CAMERA_DISPLAY_DISTANCE = 145.0f;
...
void setRobotCameraViewEnabled(int robotMode, boolean enabled) {
  ...
  if (robotMode == ROBOT_MODE_VEHICLE) {
    ...
    // Preserve the existing Drone firmware camera/record flag while the same
    // operator button now also controls the dedicated 9002 live-view channel.
  ...
  if (robotMode == ROBOT_MODE_DRONE) saveSoftwareConfigNow();
