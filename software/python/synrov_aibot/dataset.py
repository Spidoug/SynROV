...
from .perception import finite_float, pad_float_vector, safe_text

SCHEMA_VERSION = "synrov-training-v2"
TARGET_DIMS = {"Manipulator": 7, "Vehicle": 4, "Drone": 6}

# Portuguese/English accent characters that appear in robot-name strings.
_ACCENT_MAP = str.maketrans(
    "áàãâéêíóôõúç",
    "aaaaeeiooouç",
)
_ACCENT_MAP[ord("ç")] = ord("c")
...
def canonical_robot(value: Any, default: str = "Manipulator") -> str:
    ...
    # Normalise Unicode accents and fold to lowercase in a single pass.
    ...
    if text.startswith(("drone", "dro")) or text in {"quad", "uav", "quadcopter"}:
        return "Drone"
    if text.startswith(("manip", "arm", "braco")) or "manipulator" in text or "braco" in text:
        return "Manipulator"
    # Unknown value: recurse on the default once to guard against a bad default.
    if raw.strip().lower() != str(default).strip().lower():
...
class SynROVDatasetValidator:
    ...
    def _signal(self, row: Dict[str, Any]) -> Dict[str, float]:
        def _peak(key: str) -> float:
            vals = [abs(finite_float(v)) for v in row.get(key, [])]
    ...
    def quality_score(self, row: Dict[str, Any]) -> float:
        ...
        voice_conf = finite_float(row.get("voice_confidence", 0.0), 0.0, 0.0, 1.0)

        score = 0.25
        if any(t in source for t in ("manual", "voice", "text", "teacher", "ui", "base")):
            score += 0.30
        if "autonomous" in source or "ai" in source or "model" in source:
            score -= 0.08
    ...
    def sanitize_row(
        ...
        *,
        ...
        clean["timestamp"] = finite_float(row.get("timestamp", time.time()), time.time(), 0.0)
        clean["robot"] = robot
        clean["source"] = safe_text(row.get("source", "observe"), 80) or "observe"
        clean["simulation"] = (
            row.get("simulation")
            if isinstance(row.get("simulation"), (bool, type(None)))
            else None
        )
        ...
        clean["voice_confidence"] = finite_float(row.get("voice_confidence", 0.0), 0.0, 0.0, 1.0)
        clean["telemetry_feat"] = pad_float_vector(row.get("telemetry_feat", []), self.telemetry_dim, 0.0, -1_000_000.0, 1_000_000.0)
...
def read_jsonl(path: Path) -> List[Dict[str, Any]]:
    """Read a JSONL file, silently skipping malformed lines."""
