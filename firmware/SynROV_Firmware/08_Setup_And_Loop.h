// =====================================================================
// SynROV Firmware V1 - Setup and main loop
// ---------------------------------------------------------------------
// Purpose:
...
static void setupBootI2CBus() {
  ...
  // Abort a wedged AVR TWI transaction instead of allowing setup() to block.
...
void setup() {
  ...
  Serial.println(F("[BOOT] I2C DEVICES"));
  rebindI2CDevices();
  Serial.println(rt.pwmReady ? F("[BOOT] PCA9685 READY") : F("[BOOT] PCA9685 ABSENT -> FALLBACK"));
...
void loop() {
  ...
  // CH0 is the only closed-loop actuator that depends on a tight software
  // timeslice. Service it before GPS/Serial/collision work so a heavy collision
  // evaluation from the previous pass cannot add another full parser pass of
  // latency. The normal control-interval gate keeps this call inexpensive.
  if (rt.active) {
    updateMotorControl();
    serviceRuntimeHexTx();
  }
  ...
  if (rt.active) {
    ...
    // A command dispatched above may have changed the CH0 target, and collision
    // validation itself can be CPU-heavy. Re-service immediately afterward; if
    // the control interval has not elapsed this returns at once.
    updateMotorControl();
    autoMotorUpdate();
