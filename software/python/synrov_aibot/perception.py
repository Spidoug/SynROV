...
from .protocol import SNAPSHOT_SCHEMA, SOFTWARE_VERSION
...
def pad_float_vector(
    ...
    out = [safe_float(v, default, lo, hi) for v in islice(iterator, size_i)]
    default_value = safe_float(default, 0.0, lo, hi)
    out.extend([default_value] * (size_i - len(out)))
    return out
...
class SynROVSnapshotBuilder:
    ...
    def _voice(self, voice_ctx: Optional[Dict[str, Any]]) -> VoiceSnapshot:
        ...
        text_age = safe_float(ctx.get("text_age", 999.0), 999.0, 0.0)
        audio_age = safe_float(ctx.get("audio_age", 999.0), 999.0, 0.0)
        audio_level = safe_float(ctx.get("audio_level", 0.0), 0.0, 0.0, 10.0)
        audio_fresh = audio_age <= self.audio_hold_s
        audio_features = pad_float_vector(
            ctx.get("audio_features", []) if audio_fresh else [],
            self.audio_dim,
            0.0,
            -10.0,
            10.0,
        ...
        if text_age > self.voice_hold_s:
            return VoiceSnapshot(
                ...
                audio_level=audio_level if audio_fresh else 0.0,
        ...
        return VoiceSnapshot(
            ...
            confidence=safe_float(ctx.get("conf", ctx.get("confidence", 0.0)), 0.0, 0.0, 1.0),
            text_age_s=text_age,
            audio_age_s=audio_age,
    ...
    def _vision(
        ...
        def _attr(name: str, idx: int, lo: float, hi: float) -> float:
            ...
            return safe_float(raw, 0.0, lo, hi)
    ...
    def build(
        ...
        *,
        ...
        last_telem_ts = safe_float(getattr(state, "last_telemetry_ts", 0.0), 0.0, 0.0)
        ...
        voice_snapshot = self._voice(voice_ctx)
        if audio_feat is not None:
            voice_snapshot = VoiceSnapshot(
                text=voice_snapshot.text,
                norm=voice_snapshot.norm,
                intent=voice_snapshot.intent,
                confidence=voice_snapshot.confidence,
                text_age_s=voice_snapshot.text_age_s,
                audio_age_s=voice_snapshot.audio_age_s,
                audio_level=voice_snapshot.audio_level,
                audio_features=pad_float_vector(audio_feat, self.audio_dim, 0.0, -10.0, 10.0),
            )

        try:
            hint_list = [] if target_hint is None else list(target_hint)
        except Exception:
            hint_list = []

        return SynROVSnapshot(
            timestamp=ts,
            robot=safe_text(robot, 80) or "Manipulator",
            source=safe_text(source, 80) or "unknown",
            telemetry=TelemetrySnapshot(
