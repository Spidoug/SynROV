...
from .safety import MANIP_KEYS, MANIP_POSE_LIMITS, MANIPULATOR_HOME_POSE
from .voice_input import VoiceInput
from .ui_i18n import DEFAULT_LANGUAGE, format_text as ui_format, language_id, language_name, speech_locale, text as ui_text, tr_text
...
PREVIEW_BG = "#d9dee3"
WEBCAM_BG = "#eef1f4"
TEXT_PANEL_BG = "#fbfcfd"
LOG_BG = "#111827"
LOG_FG = "#e5e7eb"
DISCONNECTED_BG = "#475569"
...
class VisualStatusTile(tk.Frame):
    ...
    def __init__(self, parent: tk.Misc, title: str) -> None:
        super().__init__(parent, bg=TILE_BG, highlightthickness=1, highlightbackground=TILE_BORDER)
        self.title = tk.Label(self, text=title, bg=TILE_BG, fg=MUTED, font=("Segoe UI", 8), anchor="w")
        self.title.pack(fill="x", padx=8, pady=(6, 0))
        self.value = tk.Label(self, text="—", bg=TILE_BG, fg=TEXT, font=("Segoe UI", 9, "bold"), anchor="w")
...
class SynROVApp:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self._camera_label_to_index: Dict[str, int] = {}
        self._camera_devices: List[CameraDevice] = []
        ...
        self._ui_language = DEFAULT_LANGUAGE
        self._voice_status_code = "stopped"
        self._activity_text = "Waiting for Processing connection."

        self.uri_var = tk.StringVar(value=DEFAULT_URI)
        # Keep the protocol robot name separate from its localized display label.
        self.robot_var = tk.StringVar(value="Manipulator")
        self.robot_display_var = tk.StringVar(value=ui_text(self._ui_language, "Manipulator"))
        self._robot_label_to_name: Dict[str, str] = {}
        ...
        self.activity_var = tk.StringVar(value=self._activity_text)
        ...
        self.camera_var = tk.StringVar(value=ui_text(self._ui_language, "Auto"))

        self.manip_vars: Dict[str, tk.DoubleVar] = {
            key: tk.DoubleVar(value=MANIPULATOR_HOME_POSE[key])
    ...
    def _build_topbar(self) -> None:
        bar = tk.Frame(self.root, bg=TOPBAR_BG, height=48)
        bar.pack(fill="x")
        bar.pack_propagate(False)
        tk.Label(
            bar, text="SynROV Command Center · AiBot V1", bg=TOPBAR_BG, fg=CARD,
            font=("Segoe UI", 13, "bold"), anchor="w",
        ).pack(side="left", padx=(14, 18))

        tk.Label(bar, text="Processing WS", bg=TOPBAR_BG, fg=TOPBAR_TEXT, font=("Segoe UI", 8)).pack(side="left")
        ...
        tk.Label(bar, text="Robot", bg=TOPBAR_BG, fg=TOPBAR_TEXT, font=("Segoe UI", 8)).pack(side="left")
        self.robot_combo = ttk.Combobox(bar, textvariable=self.robot_display_var, values=(), state="readonly", width=13)
        self.robot_combo.pack(side="left", padx=(6, 12), pady=9)
        self.robot_combo.bind("<<ComboboxSelected>>", self._on_robot_selected)

        tk.Label(bar, textvariable=self.connection_var, bg=TOPBAR_BG, fg=TOPBAR_OK, font=("Segoe UI", 9, "bold")).pack(side="right", padx=14)
        tk.Label(bar, textvariable=self.language_var, bg=TOPBAR_BG, fg=TOPBAR_MUTED, font=("Segoe UI", 8)).pack(side="right", padx=4)
    ...
    def _build_banner(self) -> None:
        ...
        self.banner_title = tk.Label(
            self.banner, text="AiBot V1 ready", bg=ACCENT, fg=CARD,
    ...
    def _build_left_status(self) -> None:
        ...
        self.robot_chip = ttk.Label(chips, text=ui_text(self._ui_language, "Manipulator"), style="CardBody.TLabel")
        self.robot_chip.pack(side="left")
        self.sim_chip = ttk.Label(chips, text=ui_format(self._ui_language, "Telem {count}", count=0), style="CardBody.TLabel")
        self.sim_chip.pack(side="left", padx=6)
        self.frame_chip = ttk.Label(chips, text=ui_format(self._ui_language, "Frames {count}", count=0), style="CardBody.TLabel")
        self.frame_chip.pack(side="left", padx=6)

        self.preview_panel = tk.Label(
            preview, bg=PREVIEW_BG, fg=MUTED, text="Waiting for Processing frame",
            anchor="center", compound="center", height=11,
        )
        self.preview_panel.pack(fill="x", pady=(0, 4))
        self.webcam_panel = tk.Label(
            preview, bg=WEBCAM_BG, fg=MUTED, text="Physical webcam off",
    ...
    def _build_operation_tab(self) -> None:
        ...
        self.camera_combo = ttk.Combobox(camrow, textvariable=self.camera_var, values=(ui_text(self._ui_language, "Auto"),), state="readonly", width=38)
    ...
    def _slider_cell(
        ...
        scale = tk.Scale(
            ...
            showvalue=False, length=205, bg=TILE_BG, highlightthickness=0,
        )
    ...
    def _build_learning_tab(self) -> None:
        ...
        self.learned_text = tk.Text(learned, height=15, wrap="word", state="disabled", bg=TEXT_PANEL_BG, relief="flat")
    ...
    def _build_log_tab(self) -> None:
        ...
        self.state_text = tk.Text(state_card, height=10, wrap="word", state="disabled", bg=TEXT_PANEL_BG, relief="flat")
        self.state_text.pack(fill="x", pady=(4, 0))
        log_card = self._card(self.tab_log, "Log")
        log_card.pack(fill="both", expand=True)
        self.log_text = tk.Text(log_card, wrap="word", state="disabled", bg=LOG_BG, fg=LOG_FG, insertbackground=CARD, relief="flat")
        self.log_text.pack(fill="both", expand=True, pady=(4, 0))

    def _configure_main_window(self) -> None:
        """Create a visible, freely resizable V1 main window."""
        screen_w = max(900, int(self.root.winfo_screenwidth()))
        screen_h = max(600, int(self.root.winfo_screenheight()))
        width = min(1320, max(900, screen_w - 80))
    ...
    def _on_robot_selected(self, _event: Any = None) -> None:
        selected_label = self.robot_display_var.get()
        robot = canonical_robot(self._robot_label_to_name.get(selected_label, selected_label))
    ...
    def _apply_camera_selection(self) -> None:
        label = self.camera_var.get() or ui_text(self._ui_language, "Auto")
        ...
        if self.webcam_var.get() and device is None:
            self._log(ui_text(self._ui_language, "Physical webcam unavailable; port 9002 remains on the Processing-generated stream."))
    ...
    def _camera_device_label(self, device: CameraDevice) -> str:
        size = f" · {device.width}x{device.height}" if device.width > 0 and device.height > 0 else ""
        return f"{ui_text(self._ui_language, 'Camera')} {device.index} · {device.backend_name}{size}"

    def _refresh_camera_combo_labels(self) -> None:
        auto_label = ui_text(self._ui_language, "Auto")
        mapping: Dict[str, int] = {auto_label: WebcamSource.AUTO_INDEX}
        for device in self._camera_devices:
            mapping[self._camera_device_label(device)] = device.index
        ...
        self.camera_var.set(next((label for label, idx in mapping.items() if idx == current), auto_label))
    ...
    def _consume_camera_scan(self) -> None:
        ...
        self._refresh_camera_combo_labels()
        if self.webcam_var.get():
            self._apply_camera_selection()
    ...
    def _send_manip_manual(self, _event: Any = None) -> None:
        ...
        self._set_activity("Manipulator manual control sent.")

    def _home_manipulator(self) -> None:
        for key in MANIP_KEYS:
            self.manip_vars[key].set(MANIPULATOR_HOME_POSE[key])
    ...
    def _send_vehicle_manual(self, _event: Any = None) -> None:
        ...
        self._set_activity("Vehicle manual control sent.")
    ...
    def _send_drone_manual(self, _event: Any = None) -> None:
        ...
        self._set_activity("Drone manual control sent.")
    ...
    def _send_command(self, text: Optional[str] = None) -> None:
        ...
        self._set_activity(f"{result.robot}: {phrase} → {result.kind}:{result.name or '—'}")
    ...
    def _execute_selected_intention(self, _event: Any = None) -> None:
        ...
        if not selected:
            return
        item_id = selected[0]
        values = self.intent_tree.item(item_id, "values")
        tags = set(self.intent_tree.item(item_id, "tags") or ())
        if not values or "learned" in tags:
            return
    ...
    def _format_voice_status(self, status: str) -> str:
        ...
        if code.startswith("listening:"):
            ...
            return ui_format(lang, "Listening · {device}", device=device)
        if code.startswith("heard:"):
            phrase = code.split(":", 1)[1].strip()
            return ui_format(lang, "Recognized: {phrase}", phrase=phrase)
        if code.startswith("language:"):
            return ui_text(lang, "Microphone language synchronized")
        if code.startswith("microphone_error:"):
            detail = code.split(":", 1)[1].strip()
            return ui_format(lang, "Microphone error: {detail}", detail=detail)
        if code.startswith("recognition_error:"):
            detail = code.split(":", 1)[1].strip()
            return ui_format(lang, "Recognition unavailable: {detail}", detail=detail)
        mapping = {
            "stopped": "Voice stopped",
            "calibrating": "Calibrating ambient noise…",
            "recognizing": "Recognizing phrase…",
            "unrecognized": "Phrase not recognized",
            "overflow": "Audio overflow; continuing…",
        }
        return ui_text(lang, mapping.get(code, code))

    # -------------------------------------------------------------- intentions
    def _refresh_intentions(self) -> None:
        robot = canonical_robot(self.robot_var.get())
        lang = self._ui_language
        self.intentions_title.configure(text=ui_format(lang, "Commands and intentions — {robot}", robot=robot))
        for item in self.intent_tree.get_children():
            self.intent_tree.delete(item)
        for spec in all_specs_for_robot(robot):
            kind = ui_text(lang, "Mission") if spec.kind == "mission" else ui_text(lang, "Command")
            self.intent_tree.insert("", "end", values=(kind, spec.command(lang), spec.intent, spec.description(lang)), tags=(spec.kind,))
        for phrase, item in sorted(self.runtime.registry.learned_aliases(robot).items()):
            self.intent_tree.insert(
                "", "end",
                values=(ui_text(lang, "Learned"), phrase, item.get("name", ""), ui_text(lang, "Alias learned this session.")),
                tags=("learned",),
            )
    ...
    def _learn_new_alias(self) -> None:
        ...
        selected = self.intent_tree.selection()
        if not selected:
            messagebox.showinfo(ui_text(self._ui_language, "New learning"), ui_text(self._ui_language, "Select an intention or mission for the active robot."), parent=self.root)
            return
        item_id = selected[0]
        values = self.intent_tree.item(item_id, "values")
        tags = set(self.intent_tree.item(item_id, "tags") or ())
        if not values or "learned" in tags:
            messagebox.showinfo(ui_text(self._ui_language, "New learning"), ui_text(self._ui_language, "Select an original intention or mission."), parent=self.root)
            return
        phrase = simpledialog.askstring(ui_text(self._ui_language, "New learning"), ui_text(self._ui_language, "New phrase for this intention:"), parent=self.root)
        if not phrase:
            return
        kind = "mission" if "mission" in tags else "intent"
        intent = str(values[2])
        if self.runtime.learn_alias(phrase, kind, intent):
            self._log(ui_format(self._ui_language, "[{robot}] Learned: {phrase} → {kind}:{intent}", robot=robot, phrase=phrase, kind=kind, intent=intent))
            self._refresh_intentions()
        else:
            messagebox.showerror(ui_text(self._ui_language, "New learning"), ui_text(self._ui_language, "Could not register this phrase."), parent=self.root)
    ...
    def _refresh_learned_text(self) -> None:
        ...
        if not lines:
            lines = [ui_text(self._ui_language, "No new phrase has been taught for this robot yet.")]
    ...
    def _refresh_robot_combo_labels(self) -> None:
        """Localize robot labels without changing the protocol-facing names."""
        mapping: Dict[str, str] = {}
        labels: List[str] = []
        for robot_name in ROBOTS:
            label = ui_text(self._ui_language, robot_name)
            # Fall back to the canonical name if two translations collide.
            if label in mapping and mapping[label] != robot_name:
                label = robot_name
            mapping[label] = robot_name
            labels.append(label)
        self._robot_label_to_name = mapping
        if hasattr(self, "robot_combo"):
            self.robot_combo.configure(values=tuple(labels))
        canonical = canonical_robot(self.robot_var.get())
        self.robot_display_var.set(next((label for label, name in mapping.items() if name == canonical), canonical))

    def _set_activity(self, english_text: str) -> None:
        self._activity_text = str(english_text)
        self.activity_var.set(ui_text(self._ui_language, self._activity_text))
    ...
    def _apply_ui_language(self, language: str, *, force: bool = False) -> None:
        ...
        self.root.title(ui_text(lang, APP_TITLE))
        self._refresh_robot_combo_labels()
        self._refresh_camera_combo_labels()
        if hasattr(self, "notebook"):
            tab_texts = (
                ui_text(lang, "Operation"),
                ui_text(lang, "Intentions"),
                ui_text(lang, "Learning"),
                ui_text(lang, "Log / State"),
            )
            for index, label in enumerate(tab_texts):
                self.notebook.tab(index, text=label)
        if hasattr(self, "intent_tree"):
            headings = {
                "type": ui_text(lang, "Type"),
                "command": ui_text(lang, "Command"),
                "intent": ui_text(lang, "Intention"),
                "description": ui_text(lang, "Description"),
            }
            for key, label in headings.items():
                self.intent_tree.heading(key, text=label)
            self._refresh_intentions()
        self.voice_status_var.set(self._format_voice_status(self._voice_status_code))
        self.activity_var.set(ui_text(lang, self._activity_text))
        prefix = ui_text(lang, "Language")
    ...
    def _update_preview(self, panel: tk.Label, image: Any, size: tuple[int, int], attr: str, empty_text: str) -> None:
        ...
        try:
            ...
            setattr(self, attr, None)
            panel.configure(image="", text=ui_text(self._ui_language, "Frame unavailable"))
    ...
    def _refresh_visuals(self) -> None:
        ...
        self.connection_var.set(ui_text(self._ui_language, "Connected") if connected else ui_text(self._ui_language, "Disconnected"))
        self.connect_button.configure(text=ui_text(self._ui_language, "Disconnect") if connected else ui_text(self._ui_language, "Connect"))
        language = str(status.get("language") or "")
        prefix = ui_text(self._ui_language, "Language")
        self.language_var.set(f"{prefix}: {language_name(language) if language else '—'}")
        robot_label = ui_text(self._ui_language, robot)
        connection_label = ui_text(self._ui_language, "ONLINE" if connected else "OFFLINE")
        self.banner_title.configure(text=f"{robot_label} · {connection_label}")
        self.banner.configure(bg=ACCENT if connected else DISCONNECTED_BG)
        self.banner_title.configure(bg=ACCENT if connected else DISCONNECTED_BG)
        self.banner_detail.configure(bg=ACCENT if connected else DISCONNECTED_BG)

        self.tile_conn.set(connection_label, "ok" if connected else "bad")
        ...
        self.tile_ai.set(ui_text(self._ui_language, "ON" if self.runtime.adaptive_enabled else "OFF"), "ok" if self.runtime.adaptive_enabled else "warn")
        camera_label = ui_text(self._ui_language, "USB") if self.runtime.webcam.enabled else str(cam9002.get("source", cam9002.get("last_source", "fallback")))
        ...
        self.live_status.configure(text=f"{ui_text(self._ui_language, 'Activity')}: {self.activity_var.get()}")
        self.robot_chip.configure(text=robot_label)
        self.sim_chip.configure(text=ui_format(self._ui_language, "Telem {count}", count=state.telemetry_counter))
        self.frame_chip.configure(text=ui_format(self._ui_language, "Frames {count}", count=state.frame_counter))

        self._update_preview(
            self.preview_panel, state.last_frame_pil, self.PREVIEW_SIZE, "_preview_photo",
            ui_text(self._ui_language, "Waiting for Processing frame"),
        )
        self._update_preview(
            ...
            ui_text(self._ui_language, "Physical webcam off"),
        )
        ...
        connected_text = ui_text(self._ui_language, "connected") if connected else ui_text(self._ui_language, "disconnected")
        heading_text = ui_text(self._ui_language, "available") if heading_available else ui_text(self._ui_language, "no reading")
        webcam_text = ui_text(self._ui_language, "active") if self.runtime.webcam.enabled else ui_text(self._ui_language, "inactive")
        lines = [
            ui_format(self._ui_language, "Version: {version}", version=status.get("softwareVersion")),
            ui_format(self._ui_language, "Active robot: {robot}", robot=robot_label),
            ui_format(self._ui_language, "Processing: {status}", status=connected_text),
            ui_format(self._ui_language, "Telemetry heading: {heading:.1f}° ({availability}) · source={source}", heading=heading, availability=heading_text, source=telemetry.get("heading_source", "n/a")),
            ui_format(self._ui_language, "Catalog: {commands} commands · {missions} missions", commands=catalog.get("commands", 0), missions=catalog.get("missions", 0)),
            ui_format(self._ui_language, "Gimbal: pan {pan_min}°…{pan_max}° · tilt {tilt_min}°…{tilt_max}°", pan_min=camera.get("pan_min_deg"), pan_max=camera.get("pan_max_deg"), tilt_min=camera.get("tilt_min_deg"), tilt_max=camera.get("tilt_max_deg")),
            ui_format(self._ui_language, "Webcam: {status} · index={index} · backend={backend}", status=webcam_text, index=camera.get("active_index"), backend=camera.get("backend")),
            ui_format(self._ui_language, "Video 9002: {source}", source=source),
            ui_format(self._ui_language, "Adaptive AI: {status} · protection={protection} · shadow={shadow}", status=ui_text(self._ui_language, "ON" if self.runtime.adaptive_enabled else "OFF"), protection=self.runtime.learning_protection, shadow=self.runtime.shadow_validation),
            ui_format(self._ui_language, "Last action: {kind}:{name} · {reason}", kind=last.kind, name=last.name or "-", reason=last.reason or "ok"),
        ]
    ...
    def _poll(self) -> None:
        ...
        while True:
            ...
            if kind == "log":
                ...
                self._set_activity("Processing connected. AI is synchronizing sensors and active robot.")
                self._log(str(payload or "connected"))
            elif kind == "disconnected":
                self._set_activity("Processing disconnected.")
                self._log(str(payload or "disconnected"))
            elif kind == "command_result" and payload is not None:
                self._set_activity(f"{payload.robot}: {payload.kind}:{payload.name or '—'}")
