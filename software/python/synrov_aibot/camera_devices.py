...
class CameraDevice:
    ...
    def label_pt(self) -> str:
        ...
        return f"Câmera {self.index} · {self.backend_name}{size}"
...
class WebcamSource:
    ...
    def start(self, index: int = AUTO_INDEX, devices: Optional[Sequence[CameraDevice]] = None) -> CameraDevice:
        ...
        if wanted >= 0:
            ...
            if not candidates:
                if platform.system().lower() == "linux" and not any(os.path.exists(f"/dev/video{i}") for i in range(10)):
                    self.last_error = "nenhuma câmera física disponível"
        ...
        self.last_error = (
            f"não foi possível abrir a câmera {wanted}" if wanted >= 0
            else "nenhuma câmera física disponível"
        )
        raise RuntimeError(self.last_error)
